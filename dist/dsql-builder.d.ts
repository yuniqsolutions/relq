...
/**
 * AWS Aurora DSQL Schema Validation Error Codes
 *
 * Complete registry of error codes and warning codes for
 * DSQL dialect schema validation. Each code maps to a specific
 * incompatibility with AWS Aurora DSQL's PostgreSQL subset.
 *
 * @module dsql-schema/errors
 *
 * @remarks
 * **Code ranges:**
 * | Range                | Category                          |
 * |----------------------|-----------------------------------|
 * | DSQL-TYPE-001–014    | Blocked column types              |
 * | DSQL-LIMIT-001–004   | Column type size limits           |
 * | DSQL-MOD-001–003     | Column modifier restrictions      |
 * | DSQL-CONS-001–007    | Constraint restrictions           |
 * | DSQL-IDX-001–007     | Index type restrictions           |
 * | DSQL-IDX-LIMIT-001–005 | Index limit restrictions        |
 * | DSQL-TBL-001–007     | Table feature restrictions        |
 * | DSQL-FN-001–008      | Function/procedure restrictions   |
 * | DSQL-TRIG-001–003    | Trigger restrictions              |
 * | DSQL-VIEW-001–004    | View restrictions                 |
 * | DSQL-EXT-001–004     | Extension restrictions            |
 * | DSQL-SEQ-001–005     | Sequence restrictions             |
 * | DSQL-DB-001–006      | Database/schema limits            |
 * | DSQL-TXN-001–005     | Transaction restrictions          |
 * | DSQL-MISC-001–006    | Miscellaneous restrictions        |
 *
 * @since 1.1.0
 */
/**
 * Severity level for a DSQL validation message.
 *
 * - `error`: Schema will not work on DSQL. Must be fixed.
 * - `warning`: Schema may work but behavior differs or feature is not enforced.
 *
 * @since 1.1.0
 */
export type DsqlValidationSeverity = "error" | "warning";
/**
 * A single DSQL schema validation message.
 *
 * @example
 * ```typescript
 * const msg: DsqlValidationMessage = {
 *     code: 'DSQL-TYPE-001',
 *     severity: 'error',
 ...
 * @since 1.1.0
 */
export interface DsqlValidationMessage {
	/** Unique rule code (e.g. 'DSQL-TYPE-001'). */
	code: string;
	/** Severity level. */
	severity: DsqlValidationSeverity;
...
/**
 * Aggregated DSQL validation result.
 *
 * @since 1.1.0
 */
export interface DsqlValidationResult {
	/** True if there are zero errors (warnings are OK). */
	valid: boolean;
	/** Error-severity messages. */
	errors: DsqlValidationMessage[];
	/** Warning-severity messages. */
	warnings: DsqlValidationMessage[];
	/** All messages combined. */
	all: DsqlValidationMessage[];
}
/**
 * Auto-fix descriptor for a DSQL validation rule.
 *
 * When a schema element triggers a DSQL error, the auto-fix describes
 * the transformation needed to make it DSQL-compatible.
 *
 * @example
 * ```typescript
 * const fix: DsqlAutoFix = {
 *     description: 'Replace SERIAL with UUID + gen_random_uuid()',
 *     originalType: 'serial',
 *     replacementType: 'uuid',
 *     additionalChanges: [
 *         'Add .default(DEFAULT.genRandomUuid())',
 *         'Add .primaryKey()',
 *     ],
 * };
 * ```
...
/**
 * Static definition of a DSQL validation rule.
 *
 * @since 1.1.0
 */
export interface DsqlErrorDef {
	/** Unique rule code. */
	code: string;
	/** Severity. */
	severity: DsqlValidationSeverity;
...
export declare const DSQL_TYPE_ERRORS: Record<string, DsqlErrorDef>;
export declare const DSQL_LIMIT_WARNINGS: Record<string, DsqlErrorDef>;
export declare const DSQL_MOD_ERRORS: Record<string, DsqlErrorDef>;
export declare const DSQL_CONS_RULES: Record<string, DsqlErrorDef>;
export declare const DSQL_IDX_RULES: Record<string, DsqlErrorDef>;
export declare const DSQL_IDX_LIMIT_RULES: Record<string, DsqlErrorDef>;
export declare const DSQL_TBL_RULES: Record<string, DsqlErrorDef>;
export declare const DSQL_FN_RULES: Record<string, DsqlErrorDef>;
export declare const DSQL_TRIG_RULES: Record<string, DsqlErrorDef>;
export declare const DSQL_VIEW_RULES: Record<string, DsqlErrorDef>;
export declare const DSQL_EXT_RULES: Record<string, DsqlErrorDef>;
export declare const DSQL_SEQ_RULES: Record<string, DsqlErrorDef>;
export declare const DSQL_DB_RULES: Record<string, DsqlErrorDef>;
export declare const DSQL_TXN_RULES: Record<string, DsqlErrorDef>;
export declare const DSQL_MISC_RULES: Record<string, DsqlErrorDef>;
/**
 * All DSQL validation rules combined into a single lookup table.
 *
 * @since 1.1.0
 */
export declare const DSQL_ALL_RULES: Record<string, DsqlErrorDef>;
/**
 * Create an aggregated validation result from a list of messages.
 *
 * @since 1.1.0
 */
export declare function createDsqlValidationResult(messages: DsqlValidationMessage[]): DsqlValidationResult;
/**
 * Look up a DSQL rule definition by code.
 *
 * @since 1.1.0
 */
export declare function lookupDsqlRule(code: string): DsqlErrorDef | undefined;
/**
 * Create a validation message from a rule code with location context.
 *
 * @since 1.1.0
 */
export declare function createDsqlMessage(code: string, location?: {
	tableName?: string;
	columnName?: string;
	indexName?: string;
	functionName?: string;
}): DsqlValidationMessage;
/**
 * Format a single DSQL validation message for display.
 *
 * @since 1.1.0
 */
export declare function formatDsqlMessage(msg: DsqlValidationMessage): string;
/**
 * Format a complete DSQL validation result for display.
 *
 * @since 1.1.0
 */
export declare function formatDsqlValidationResult(result: DsqlValidationResult): string;
/**
 * AWS Aurora DSQL Quotas and Limits
 *
 * All numeric limits, size constraints, and quotas that DSQL
 * enforces on clusters, databases, tables, indexes, columns,
 * transactions, and connections.
 *
 * @module dsql-schema/limits
 *
 * @since 1.1.0
 */
/**
 * DSQL cluster and database-level limits.
 *
 * @since 1.1.0
 */
export declare const DSQL_DATABASE_LIMITS: {
	/** Maximum databases per cluster (always 1: "postgres"). */
	readonly MAX_DATABASES: 1;
	/** Maximum schemas per database. */
	readonly MAX_SCHEMAS: 10;
	/** Maximum tables per database. */
	readonly MAX_TABLES: 1000;
	/** Maximum views per database. */
	readonly MAX_VIEWS: 5000;
	/** Maximum storage per cluster (bytes). Default 10 TiB; up to 256 TiB configurable. */
	readonly MAX_STORAGE_BYTES: number;
};
/**
 * DSQL table-level limits.
 *
 * @since 1.1.0
 */
export declare const DSQL_TABLE_LIMITS: {
	/** Maximum columns per table. */
	readonly MAX_COLUMNS: 255;
	/** Maximum indexes per table. */
	readonly MAX_INDEXES: 24;
	/** Maximum columns in a primary key or index. */
	readonly MAX_INDEX_COLUMNS: 8;
	/** Maximum row size in bytes. */
	readonly MAX_ROW_SIZE_BYTES: number;
	/** Maximum column size for non-indexed columns (bytes). */
	readonly MAX_COLUMN_SIZE_BYTES: number;
	/** Maximum combined primary key column size (bytes). */
	readonly MAX_PK_SIZE_BYTES: 1024;
	/** Maximum combined index column size (bytes). */
	readonly MAX_INDEX_SIZE_BYTES: 1024;
	/** Maximum view definition size (bytes). */
	readonly MAX_VIEW_DEF_BYTES: number;
};
/**
 * DSQL column type size limits (tighter than PostgreSQL).
 *
 * @since 1.1.0
 */
export declare const DSQL_TYPE_LIMITS: {
	/** Maximum VARCHAR length in bytes. PG: 10,485,760. */
	readonly MAX_VARCHAR_LENGTH: 65535;
	/** Maximum CHAR length in bytes. PG: 10,485,760. */
	readonly MAX_CHAR_LENGTH: 4096;
	/** Maximum BPCHAR length in bytes. */
	readonly MAX_BPCHAR_LENGTH: 4096;
	/** Maximum TEXT size in bytes. PG: ~1 GB. */
	readonly MAX_TEXT_SIZE_BYTES: number;
	/** Maximum BYTEA size in bytes. PG: ~1 GB. */
	readonly MAX_BYTEA_SIZE_BYTES: number;
	/** Maximum NUMERIC precision. PG: 1000. */
	readonly MAX_NUMERIC_PRECISION: 38;
	/** Maximum NUMERIC scale. PG: 16383. */
	readonly MAX_NUMERIC_SCALE: 37;
};
/**
 * DSQL connection limits.
 *
 * @since 1.1.0
 */
export declare const DSQL_CONNECTION_LIMITS: {
	/** Maximum concurrent connections per cluster. Configurable. */
	readonly MAX_CONNECTIONS: 10000;
	/** Connection rate limit (connections per second). */
	readonly CONNECTION_RATE: 100;
	/** Connection burst limit. */
	readonly CONNECTION_BURST: 1000;
	/** Maximum idle connection time (seconds). */
	readonly MAX_IDLE_SECONDS: 3600;
	/** Maximum IAM token lifetime (seconds). */
	readonly MAX_TOKEN_LIFETIME_SECONDS: 604800;
	/** Default IAM token lifetime (seconds). */
	readonly DEFAULT_TOKEN_LIFETIME_SECONDS: 900;
};
/**
 ...
 * @since 1.1.0
 */
export declare const DSQL_TRANSACTION_LIMITS: {
	/** Maximum mutated rows per write transaction. */
	readonly MAX_MUTATED_ROWS: 3000;
	/** Maximum transaction duration (seconds). */
	readonly MAX_DURATION_SECONDS: 300;
	/** Maximum DDL statements per transaction. */
	readonly MAX_DDL_PER_TRANSACTION: 1;
	/** Maximum write transaction data size (bytes). */
	readonly MAX_DATA_SIZE_BYTES: number;
	/** Maximum query memory per transaction (bytes). */
	readonly MAX_QUERY_MEMORY_BYTES: number;
	/** Fixed isolation level. */
	readonly ISOLATION_LEVEL: "REPEATABLE READ";
	/** Serialization failure error code (requires retry). */
	readonly SERIALIZATION_FAILURE_CODE: "40001";
};
/**
 * Column types that cannot be used in DSQL indexes.
 *
 * @since 1.1.0
 */
export declare const DSQL_NON_INDEXABLE_TYPES: ReadonlySet<string>;
/**
 * Index methods supported by DSQL (only B-tree).
 *
 * @since 1.1.0
 */
export declare const DSQL_SUPPORTED_INDEX_METHODS: ReadonlySet<string>;
/**
 * Index methods NOT supported by DSQL.
 *
 * @since 1.1.0
 */
export declare const DSQL_BLOCKED_INDEX_METHODS: ReadonlySet<string>;
/**
 * AWS Aurora DSQL Type Compatibility Map
 *
 * Maps every PostgreSQL column type to its DSQL support status.
 * DSQL supports approximately 19 of PostgreSQL's 60+ types (~28% coverage).
 *
 * @module dsql-schema/type-mappings
 *
 * @since 1.1.0
 */
/**
 * DSQL compatibility status for a PostgreSQL column type.
 *
 * - `supported`: Fully works in DSQL.
 * - `unsupported`: Blocked as a column type in DSQL.
 * - `warning`: Works but has tighter limits than PostgreSQL.
 *
 * @since 1.1.0
 */
export type DsqlTypeStatus = "supported" | "unsupported" | "warning";
/**
 * A single DSQL type compatibility mapping.
 *
 * @since 1.1.0
 */
export interface DsqlTypeMapping {
	/** PostgreSQL type name (uppercase canonical). */
	pgType: string;
	/** DSQL compatibility status. */
	status: DsqlTypeStatus;
	/** DSQL error/warning code if not fully supported. */
	errorCode?: string;
	/** Recommended alternative for unsupported types. */
	alternative?: string;
...
/**
 * Complete mapping of PostgreSQL types to DSQL compatibility.
 *
 * @remarks
 * DSQL supports approximately 19 column types:
 * - Numeric: smallint, integer, bigint, real, double precision, numeric
 * - Character: char, varchar, bpchar, text
 * - Date/Time: date, time, timetz, timestamp, timestamptz, interval
 * - Boolean: boolean
 * - Binary: bytea
 * - UUID: uuid
 * - Enum: user-defined enums via CREATE TYPE AS ENUM
 *
 * @since 1.1.0
 */
export declare const DSQL_TYPE_MAP: Record<string, DsqlTypeMapping>;
/**
 * Look up a DSQL type mapping by SQL type string.
 * Case-insensitive, strips parameterized suffixes and array brackets.
 *
 * @since 1.1.0
 */
export declare function getDsqlTypeMapping(sqlType: string): DsqlTypeMapping | undefined;
/**
 * Check if a type is supported in DSQL.
 *
 * @since 1.1.0
 */
export declare function isDsqlTypeSupported(sqlType: string): boolean;
/**
 * Get all unsupported types grouped by category.
 *
 * @since 1.1.0
 */
export declare function getDsqlUnsupportedTypesByCategory(): Record<string, DsqlTypeMapping[]>;
/**
 * Validate a column type for DSQL compatibility.
 *
 * @param sqlType - The SQL type string (e.g. 'JSONB', 'VARCHAR(255)', 'SERIAL')
 ...
 * @example
 * ```typescript
 * validateDsqlColumnType('JSONB', 'events', 'data');
 * // [{ code: 'DSQL-TYPE-002', message: 'JSON/JSONB column types are not supported...' }]
 *
 * validateDsqlColumnType('VARCHAR(100000)', 'users', 'bio');
 * // [{ code: 'DSQL-LIMIT-001', message: 'VARCHAR length exceeds DSQL maximum...' }]
 *
 * validateDsqlColumnType('INTEGER');
 * // [] (fully supported)
 * ```
 *
 * @since 1.1.0
 */
export declare function validateDsqlColumnType(sqlType: string, tableName?: string, columnName?: string): DsqlValidationMessage[];
/**
 * Validate multiple column types at once.
 *
 ...
 * @since 1.1.0
 */
export declare function validateDsqlColumnTypes(columns: Array<{
	sqlType: string;
	tableName?: string;
	columnName?: string;
}>): DsqlValidationMessage[];
/**
 * Check if a specific column type is supported on DSQL.
 *
 ...
 * @since 1.1.0
 */
export declare function checkDsqlTypeSupport(sqlType: string): {
	supported: boolean;
...
/**
 * Get the recommended DSQL alternative for an unsupported type.
 *
 ...
 * @since 1.1.0
 */
export declare function getDsqlAlternative(sqlType: string): string | null;
/**
 * Get all blocked (unsupported) DSQL type names.
 *
 * @returns Array of uppercase SQL type names that are blocked in DSQL
 *
 * @since 1.1.0
 */
export declare function getDsqlBlockedTypes(): string[];
/**
 * Constraint definition input for DSQL validation.
 *
 * @since 1.1.0
 */
export interface DsqlConstraintInput {
	/** Constraint type: 'PRIMARY KEY' | 'UNIQUE' | 'CHECK' | 'FOREIGN KEY' | 'EXCLUSION'. */
	type: string;
	/** Optional constraint name. */
	name?: string;
	...
	/** ON UPDATE action. */
	onUpdate?: string;
	/** Whether this is an inline column REFERENCES. */
	isInlineReference?: boolean;
}
/**
 * Validate a single constraint for DSQL compatibility.
 *
 * @param input - Constraint definition
 * @param tableName - Optional table name for error context
 * @returns Array of validation messages
 *
 * @example
 * ```typescript
 * validateDsqlConstraint({ type: 'FOREIGN KEY', onDelete: 'CASCADE' }, 'orders');
 * // [{ code: 'DSQL-CONS-001', severity: 'warning', ... },
 * //  { code: 'DSQL-CONS-003', severity: 'warning', ... }]
 *
 * validateDsqlConstraint({ type: 'EXCLUSION' }, 'bookings');
 * // [{ code: 'DSQL-CONS-005', severity: 'error', ... }]
 * ```
 *
 * @since 1.1.0
 */
export declare function validateDsqlConstraint(input: DsqlConstraintInput, tableName?: string): DsqlValidationMessage[];
/**
 * Validate multiple constraints at once.
 *
 * @since 1.1.0
 */
export declare function validateDsqlConstraints(constraints: DsqlConstraintInput[], tableName?: string): DsqlValidationMessage[];
/**
 * Constraint features that are blocked or produce warnings in DSQL.
 *
 * @since 1.1.0
 */
export declare const DSQL_CONSTRAINT_RULES: {
...
/**
 * Index definition input for DSQL validation.
 *
 * @since 1.1.0
 */
export interface DsqlIndexInput {
	/** Optional index name. */
	name?: string;
	...
	columns?: string[];
	/** Column types corresponding to columns (for non-indexable check). */
	columnTypes?: string[];
	/** Whether the index is CONCURRENTLY created. */
	concurrently?: boolean;
	/** Operator class specifications. */
	operatorClass?: string;
}
/**
 * Validate a single index for DSQL compatibility.
 *
 * @param input - Index definition
 * @param tableName - Optional table name for error context
 * @param tableIndexCount - Total number of indexes on the table (for limit check)
 * @returns Array of validation messages
 *
 * @example
 * ```typescript
 * validateDsqlIndex({ method: 'gin', columns: ['tags'] }, 'posts');
 * // [{ code: 'DSQL-IDX-001', severity: 'error', ... }]
 *
 * validateDsqlIndex({ columns: Array(9).fill('col') }, 'wide_table');
 * // [{ code: 'DSQL-IDX-LIMIT-002', severity: 'error', ... }]
 * ```
 *
 * @since 1.1.0
 */
export declare function validateDsqlIndex(input: DsqlIndexInput, tableName?: string, tableIndexCount?: number): DsqlValidationMessage[];
/**
 * Validate multiple indexes at once.
 *
 * @since 1.1.0
 */
export declare function validateDsqlIndexes(indexes: DsqlIndexInput[], tableName?: string): DsqlValidationMessage[];
/**
 * Check if an index method is supported in DSQL.
 *
 * @since 1.1.0
 */
export declare function isDsqlIndexMethodSupported(method: string): boolean;
/**
 * Table definition input for DSQL validation.
 *
 * @since 1.1.0
 */
export interface DsqlTableInput {
	/** Table name. */
	name?: string;
	/** Whether table is TEMPORARY. */
	temporary?: boolean;
	/** Whether table is UNLOGGED. */
	unlogged?: boolean;
	/** Whether table uses INHERITS. */
	inherits?: string;
	/** Whether table uses PARTITION BY. */
	partitionBy?: string;
	/** Whether table specifies TABLESPACE. */
	tablespace?: string;
	/** Whether table has WITH (storage_parameters). */
	withStorageParams?: boolean;
	/** Whether table has ON COMMIT clause. */
	onCommit?: string;
	/** Number of columns in the table. */
	columnCount?: number;
}
/**
 * Validate a table definition for DSQL compatibility.
 *
 * @param input - Table definition
 * @returns Array of validation messages
 *
 * @example
 * ```typescript
 * validateDsqlTable({ temporary: true, name: 'temp_data' });
 * // [{ code: 'DSQL-TBL-001', severity: 'error', ... }]
 *
 * validateDsqlTable({ partitionBy: 'RANGE (created_at)', name: 'events' });
 * // [{ code: 'DSQL-TBL-005', severity: 'error', ... }]
 * ```
 *
 * @since 1.1.0
 */
export declare function validateDsqlTable(input: DsqlTableInput): DsqlValidationMessage[];
/**
 * Validate the total number of tables against DSQL limits.
 *
 * @param tableCount - Total number of tables in the database
 * @returns Validation messages if limit exceeded
 *
 * @since 1.1.0
 */
export declare function validateDsqlTableCount(tableCount: number): DsqlValidationMessage[];
/**
 * Validate the total number of schemas against DSQL limits.
 *
 * @param schemaCount - Total number of schemas in the database
 * @returns Validation messages if limit exceeded
 *
 * @since 1.1.0
 */
export declare function validateDsqlSchemaCount(schemaCount: number): DsqlValidationMessage[];
/**
 * Table features that are blocked in DSQL.
 *
 * @since 1.1.0
 */
export declare const DSQL_BLOCKED_TABLE_FEATURES: readonly [
	"TEMPORARY",
	"UNLOGGED",
	"INHERITS",
	"TABLESPACE",
...
/**
 * Function/procedure definition input for DSQL validation.
 *
 * @since 1.1.0
 */
export interface DsqlFunctionInput {
	/** Function or procedure name. */
	name?: string;
	/** Language (e.g. 'sql', 'plpgsql', 'plpython3u', 'c'). */
	language?: string;
	/** Whether this is a CREATE PROCEDURE (vs CREATE FUNCTION). */
	isProcedure?: boolean;
	/** Whether this is a DO $$ block. */
	isDoBlock?: boolean;
	/** Whether the return type is TRIGGER. */
	returnsTrigger?: boolean;
}
/**
 * Validate a function/procedure definition for DSQL compatibility.
 *
 ...
 * @example
 * ```typescript
 * validateDsqlFunction({ name: 'audit_log', language: 'plpgsql' });
 * // [{ code: 'DSQL-FN-001', severity: 'error', ... }]
 *
 * validateDsqlFunction({ isDoBlock: true });
 * // [{ code: 'DSQL-FN-007', severity: 'error', ... }]
 *
 * validateDsqlFunction({ name: 'compute', language: 'sql' });
 * // [] (allowed)
 * ```
 *
 * @since 1.1.0
 */
export declare function validateDsqlFunction(input: DsqlFunctionInput): DsqlValidationMessage[];
/**
 * Validate multiple functions at once.
 *
 * @since 1.1.0
 */
export declare function validateDsqlFunctions(functions: DsqlFunctionInput[]): DsqlValidationMessage[];
/**
 * Check if a language is supported in DSQL.
 *
 * @since 1.1.0
 */
export declare function isDsqlLanguageSupported(language: string): boolean;
/**
 * All blocked languages in DSQL.
 *
 * @since 1.1.0
 */
export declare const DSQL_BLOCKED_LANGUAGES: readonly [
	"plpgsql",
	"plpython3u",
	"plpythonu",
	"plpython2u",
...
/**
 * Trigger definition input for DSQL validation.
 *
 * @since 1.1.0
 */
export interface DsqlTriggerInput {
	/** Trigger name. */
	name?: string;
	/** Whether this is a CREATE TRIGGER. */
	isCreate?: boolean;
	/** Whether this is a DROP TRIGGER. */
	isDrop?: boolean;
}
/**
 * Validate a trigger for DSQL compatibility.
 * All triggers produce errors since DSQL does not support triggers.
 *
 * @param input - Trigger definition
 * @param tableName - Optional table name for context
 * @returns Array of validation messages (always at least one error)
 *
 * @example
 * ```typescript
 * validateDsqlTrigger({ name: 'audit_trigger', isCreate: true }, 'users');
 * // [{ code: 'DSQL-TRIG-001', severity: 'error', ... }]
 * ```
 *
 * @since 1.1.0
 */
export declare function validateDsqlTrigger(input: DsqlTriggerInput, tableName?: string): DsqlValidationMessage[];
/**
 * Validate multiple triggers at once.
 *
 * @since 1.1.0
 */
export declare function validateDsqlTriggers(triggers: DsqlTriggerInput[], tableName?: string): DsqlValidationMessage[];
/**
 * Sequence definition input for DSQL validation.
 *
 * @since 1.1.0
 */
export interface DsqlSequenceInput {
	/** Sequence name. */
	name?: string;
	/** Whether this is a CREATE SEQUENCE. */
	isCreate?: boolean;
}
/**
 * Validate a sequence for DSQL compatibility.
 * All sequences produce errors since DSQL does not support sequences.
 *
 * @param input - Sequence definition
 * @returns Array of validation messages
 *
 * @example
 * ```typescript
 * validateDsqlSequence({ name: 'users_id_seq', isCreate: true });
 * // [{ code: 'DSQL-SEQ-001', severity: 'error', ... }]
 * ```
 *
 * @since 1.1.0
 */
export declare function validateDsqlSequence(input: DsqlSequenceInput): DsqlValidationMessage[];
/**
 * Validate multiple sequences at once.
 *
 * @since 1.1.0
 */
export declare function validateDsqlSequences(sequences: DsqlSequenceInput[]): DsqlValidationMessage[];
/**
 * Check if a SQL expression uses sequence functions (nextval, currval, setval, lastval).
 *
 * @param expression - SQL expression string
 * @returns Array of validation messages for any sequence function usage
 *
 * @example
 * ```typescript
 * validateDsqlSequenceExpression("nextval('users_id_seq')");
 * // [{ code: 'DSQL-SEQ-002', ... }]
 * ```
 *
 * @since 1.1.0
 */
export declare function validateDsqlSequenceExpression(expression: string, tableName?: string, columnName?: string): DsqlValidationMessage[];
/**
 * All blocked sequence functions in DSQL.
 *
 * @since 1.1.0
 */
export declare const DSQL_BLOCKED_SEQUENCE_FUNCTIONS: readonly [
	"nextval",
	"currval",
	"setval",
	"lastval"
...
/**
 * View definition input for DSQL validation.
 *
 * @since 1.1.0
 */
export interface DsqlViewInput {
	/** View name. */
	name?: string;
	/** Whether this is a MATERIALIZED VIEW. */
	materialized?: boolean;
	/** Whether this is a REFRESH MATERIALIZED VIEW. */
	isRefresh?: boolean;
	/** View definition SQL (for size check). */
	definition?: string;
}
/**
 * Validate a view definition for DSQL compatibility.
 *
 ...
 * @example
 * ```typescript
 * validateDsqlView({ materialized: true, name: 'mv_report' });
 * // [{ code: 'DSQL-VIEW-001', severity: 'error', ... }]
 *
 * validateDsqlView({ name: 'active_users' });
 * // [] (regular views are supported)
 * ```
 *
 * @since 1.1.0
 */
export declare function validateDsqlView(input: DsqlViewInput): DsqlValidationMessage[];
/**
 * Validate the total number of views against DSQL limits.
 *
 * @param viewCount - Total number of views in the database
 * @returns Validation messages if limit exceeded
 *
 * @since 1.1.0
 */
export declare function validateDsqlViewCount(viewCount: number): DsqlValidationMessage[];
/**
 * Validate multiple views at once.
 *
 * @since 1.1.0
 */
export declare function validateDsqlViews(views: DsqlViewInput[]): DsqlValidationMessage[];
/**
 * Scan a SQL string for miscellaneous DSQL-blocked features.
 *
 ...
 * @example
 * ```typescript
 * validateDsqlMiscSql('LISTEN channel_name');
 * // [{ code: 'DSQL-MISC-001', severity: 'error', ... }]
 *
 * validateDsqlMiscSql('CREATE EXTENSION postgis');
 * // [{ code: 'DSQL-EXT-001', severity: 'error', ... }]
 * ```
 *
 * @since 1.1.0
 */
export declare function validateDsqlMiscSql(sql: string, tableName?: string): DsqlValidationMessage[];
/**
 * Validate extension usage for DSQL compatibility.
 *
 * @param extensionNames - Array of extension names to validate
 * @returns Array of validation messages (all will be errors)
 *
 * @example
 * ```typescript
 * validateDsqlExtensions(['postgis', 'pgvector']);
 * // [{ code: 'DSQL-EXT-001', ... }, { code: 'DSQL-EXT-001', ... }]
 * ```
 *
 * @since 1.1.0
 */
export declare function validateDsqlExtensions(extensionNames: string[]): DsqlValidationMessage[];
/**
 * Validate column modifier for DSQL — identity columns and collation.
 *
 * @since 1.1.0
 */
export declare function validateDsqlColumnModifier(input: {
	generatedAsIdentity?: boolean;
	defaultNextval?: boolean;
	collation?: string;
	tableName?: string;
	columnName?: string;
}): DsqlValidationMessage[];
/**
 * All blocked miscellaneous features in DSQL.
 *
 * @since 1.1.0
 */
export declare const DSQL_BLOCKED_MISC_FEATURES: readonly [
	"LISTEN",
	"NOTIFY",
	"pg_advisory_lock",
	"TRUNCATE",
...
/**
 * AWS Aurora DSQL Schema Builder
 *
 * Exports only DSQL-compatible column types, table definitions,
 * and schema features. Unsupported PostgreSQL features are intentionally
 * NOT exported to provide compile-time safety.
 *
 * @module relq/dsql-builder
 *
 * @example
 * ```typescript
 * import { defineTable, text, integer, uuid, genRandomUuid } from 'relq/dsql-builder';
 *
 * export const users = defineTable('users', {
 *   id: uuid().primaryKey().default(genRandomUuid()),
 *   name: text().notNull(),
 *   age: integer(),
 *   metadata: text(), // No JSONB column type — store JSON as text, cast at query time
 * });
 * ```
 *
 * @remarks
 * **Not available in this builder (use `relq/pg-builder` instead):**
 * - JSON/JSONB column types (use text() and cast with ::jsonb at query time)
 * - SERIAL/BIGSERIAL/SMALLSERIAL (use uuid().default(genRandomUuid()))
 * - Array column types (runtime-only in DSQL)
 * - Geometric types, Range/Multirange types, XML type, MONEY type
 * - Network types (INET runtime-only, CIDR/MACADDR/MACADDR8 blocked)
 * - Bit string types, Text search types, OID types, Internal types
 * - Triggers (completely blocked)
 * - Sequences (completely blocked)
 * - Materialized views
 * - Partitioning (DSQL auto-partitions)
 * - Non-B-tree indexes (GIN, GiST, SP-GiST, BRIN, Hash)
 * - Extensions (none available)
 * - PL/pgSQL functions (SQL-only)
 * - DEFERRABLE constraints, EXCLUSION constraints
 * - Foreign keys (accepted but NOT enforced)
 *
 * @since 1.1.0
 */
/**
 * DSQL dialect identifier.
 *
 * Use this when configuring Relq to target AWS Aurora DSQL.
 *
 * @example
 * ```typescript
 * import { DIALECT } from 'relq/dsql-builder';
 * console.log(DIALECT); // 'dsql'
 * ```
 */
export declare const DIALECT: "dsql";
/**
 * Validate a schema specifically for DSQL compatibility.
 *
 * @param schema - The schema to validate
 * @returns Validation result with DSQL-specific errors and suggestions
 *
 * @since 1.1.0
 */
export declare function validateForDSQL(schema: SchemaInput): ValidationResult;
/**
 * Features NOT supported on AWS Aurora DSQL.
 *
 * DSQL supports approximately 35-40% of PostgreSQL 16 features.
 * This constant documents all PostgreSQL features that are unavailable.
 *
 * @since 1.1.0
 */
export declare const DSQL_UNSUPPORTED_FEATURES: {
	/** Column types not available on DSQL */
	...
	/** Index methods not available on DSQL (only B-tree) */
	...
	/** Schema features not available on DSQL */
	...
	/** Non-indexable types in DSQL */
	readonly nonIndexableTypes: readonly [
		"bytea",
		"interval",
		"timetz"
	];
	...
	readonly alternatives: {
		readonly SERIAL: "uuid().default(genRandomUuid())";
		readonly JSON: "text() \u2014 cast with col::jsonb at query time";
		readonly JSONB: "text() \u2014 cast with col::jsonb at query time";
		readonly XML: "text()";
		readonly MONEY: "numeric(19, 4)";
		readonly INET: "varchar(45)";
		readonly CIDR: "text()";
		readonly MACADDR: "varchar(17)";
		readonly BIT: "bytea() or text()";
		readonly TSVECTOR: "text() + external search service";
		readonly POINT: "Two numeric columns";
		readonly INT4RANGE: "Two integer columns (lower, upper)";
	};
};
