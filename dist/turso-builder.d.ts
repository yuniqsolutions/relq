...
/**
 * Creates a 32-bit float vector column (F32_BLOB).
 *
 * This is the **recommended default** for most embedding use cases.
 * 4 bytes per dimension, standard float32 precision.
 *
 * @param dimensions - Number of vector dimensions (e.g. 1536 for OpenAI ada-002)
 * @param columnName - Optional explicit database column name
 * @returns SQLiteColumnBuilder typed as `number[]`
 *
 * @example
 * ```typescript
 * import { sqliteTable, integer, text, vectorF32 } from 'relq/turso-builder';
 *
 * const documents = sqliteTable('documents', {
 *   id: integer().primaryKey().autoincrement(),
 *   title: text().notNull(),
 *   embedding: vectorF32(1536).notNull(),
 * });
 * ```
 *
 * @remarks
 * **Storage:** `F32_BLOB(n)` — 4 bytes per dimension
 *
 * **Dialect Support:**
 * | Dialect     | Support    | Notes                                |
 * |-------------|------------|--------------------------------------|
 * | Turso       | ✅ Full     | Native libSQL vector type            |
 * | SQLite      | ❌ None     | Requires libSQL fork                 |
 * | PostgreSQL  | ❌ None     | Use pgvector `vector(n)` instead     |
 *
 * @since 1.1.0
 * @see {@link vectorF64} for higher precision
 * @see {@link vectorF16} for lower storage cost
 */
export declare function vectorF32(dimensions: number, columnName?: string): SQLiteColumnBuilder<number[]>;
/**
 * Creates a 64-bit float vector column (F64_BLOB).
 *
 * Highest precision vector type. 8 bytes per dimension.
 * Use when precision is critical and storage is not a concern.
 *
 ...
 * @example
 * ```typescript
 * import { vectorF64 } from 'relq/turso-builder';
 *
 * const preciseEmbeddings = vectorF64(768).notNull();
 * ```
 *
 * @remarks
 * **Storage:** `F64_BLOB(n)` — 8 bytes per dimension
 *
 * **Dialect Support:**
 * | Dialect     | Support    | Notes                                |
 * |-------------|------------|--------------------------------------|
 ...
 * @since 1.1.0
 */
export declare function vectorF64(dimensions: number, columnName?: string): SQLiteColumnBuilder<number[]>;
/**
 * Creates a 16-bit float vector column (F16_BLOB).
 *
 * Half-precision float. 2 bytes per dimension.
 * Good balance between storage and precision for most embeddings.
 *
 * @param dimensions - Number of vector dimensions
 * @param columnName - Optional explicit database column name
 * @returns SQLiteColumnBuilder typed as `number[]`
 ...
 * @example
 * ```typescript
 * import { vectorF16 } from 'relq/turso-builder';
 *
 * const compactEmbeddings = vectorF16(384).notNull();
 * ```
 *
 * @remarks
 * **Storage:** `F16_BLOB(n)` — 2 bytes per dimension
 *
 * **Dialect Support:**
 * | Dialect     | Support    | Notes                                |
 * |-------------|------------|--------------------------------------|
 ...
 * @since 1.1.0
 */
export declare function vectorF16(dimensions: number, columnName?: string): SQLiteColumnBuilder<number[]>;
/**
 * Creates a brain float 16-bit vector column (FLOATB16 / FB16_BLOB).
 *
 * Brain float (bfloat16) uses 8 bits for exponent and 7 for mantissa,
 * giving the same range as float32 but with reduced precision.
 * 2 bytes per dimension. Popular in ML inference.
 *
 * @param dimensions - Number of vector dimensions
 * @param columnName - Optional explicit database column name
 * @returns SQLiteColumnBuilder typed as `number[]`
 ...
 * @example
 * ```typescript
 * import { vectorBF16 } from 'relq/turso-builder';
 *
 * const mlEmbeddings = vectorBF16(1024).notNull();
 * ```
 *
 * @remarks
 * **Storage:** `FLOATB16(n)` — 2 bytes per dimension
 *
 * **Dialect Support:**
 * | Dialect     | Support    | Notes                                |
 * |-------------|------------|--------------------------------------|
 ...
 * @since 1.1.0
 */
export declare function vectorBF16(dimensions: number, columnName?: string): SQLiteColumnBuilder<number[]>;
/**
 * Creates an 8-bit quantized vector column (FLOAT8 / F8_BLOB).
 *
 * Quantized to 8 bits per dimension (1 byte each).
 * Significant storage savings with moderate precision loss.
 * Good for large-scale similarity search where recall is acceptable.
 *
 * @param dimensions - Number of vector dimensions
 * @param columnName - Optional explicit database column name
 * @returns SQLiteColumnBuilder typed as `number[]`
 ...
 * @example
 * ```typescript
 * import { vectorF8 } from 'relq/turso-builder';
 *
 * const quantizedEmbeddings = vectorF8(1536).notNull();
 * ```
 *
 * @remarks
 * **Storage:** `FLOAT8(n)` — 1 byte per dimension
 *
 * **Dialect Support:**
 * | Dialect     | Support    | Notes                                |
 * |-------------|------------|--------------------------------------|
 ...
 * @since 1.1.0
 */
export declare function vectorF8(dimensions: number, columnName?: string): SQLiteColumnBuilder<number[]>;
/**
 * Creates a binary 1-bit vector column (FLOAT1BIT / F1BIT_BLOB).
 *
 * Each dimension is a single bit. 1/8 byte per dimension.
 * Maximum compression. Useful for binary hash-based similarity
 * or as a first-pass filter before re-ranking with higher precision.
 *
 * @param dimensions - Number of vector dimensions (should be multiple of 8)
 * @param columnName - Optional explicit database column name
 * @returns SQLiteColumnBuilder typed as `number[]`
 *
 * @example
 * ```typescript
 * import { vectorBinary } from 'relq/turso-builder';
 *
 * const binaryHash = vectorBinary(2048).notNull();
 * ```
 *
 * @remarks
 * **Storage:** `FLOAT1BIT(n)` — 1/8 byte per dimension
 *
 * **Dialect Support:**
 * | Dialect     | Support    | Notes                                |
 * |-------------|------------|--------------------------------------|
 ...
 * @since 1.1.0
 */
export declare function vectorBinary(dimensions: number, columnName?: string): SQLiteColumnBuilder<number[]>;
/**
 * Turso/libSQL DiskANN Vector Index
 *
 * libSQL provides approximate nearest neighbor (ANN) search via DiskANN
 * vector indexes. These are created on vector columns (F32_BLOB, etc.)
 * and support cosine similarity and L2 (Euclidean) distance metrics.
 *
 * ```sql
 * -- Default cosine similarity
 * CREATE INDEX idx ON docs(libsql_vector_idx(embedding));
 *
 ...
 * ```
 *
 * @module sqlite-schema/turso/vector-index
 * @since 1.1.0
 */
/** Distance metric for vector similarity search. */
export type VectorMetric = "cosine" | "l2";
/** Compression type for neighbor vectors in the DiskANN index. */
export type VectorCompression = "float8" | "float16" | "float32" | "float1bit";
/**
 * Configuration for a DiskANN vector index.
 *
 * @example
 * ```typescript
...
export interface VectorIndexConfig {
	 ...
	 * - `'l2'` — Euclidean distance, best for spatial/geometric data
	 *
	 * @default 'cosine'
	 */
	metric?: VectorMetric;
	/**
	 * Compression for neighbor vectors stored in the index.
	 * Lower precision = smaller index, slightly lower recall.
	 *
	 * @default undefined (no compression, uses column type)
	 */
...
/**
 * Generate a CREATE INDEX SQL statement for a DiskANN vector index.
 *
 * @param config - Vector index configuration
 * @returns Complete CREATE INDEX SQL string
 *
 * @example
 * ```typescript
 * import { generateVectorIndexSQL } from 'relq/turso-builder';
 *
 * const sql = generateVectorIndexSQL({
 *   name: 'idx_docs_embedding',
 *   table: 'documents',
 *   column: 'embedding',
 *   metric: 'cosine',
 ...
 * | Dialect     | Support    | Notes                                   |
 * |-------------|------------|-----------------------------------------|
 * | Turso       | ✅ Full     | Native DiskANN vector index             |
 * | SQLite      | ❌ None     | No vector index support in stock SQLite |
 * | PostgreSQL  | ❌ None     | Use pgvector HNSW/IVFFlat indexes       |
 *
 * @since 1.1.0
 */
export declare function generateVectorIndexSQL(config: VectorIndexConfig): string;
/**
 * Generate a `vector_top_k()` SQL fragment for nearest neighbor search.
 *
 * Returns a table-valued function reference that can be joined with
 * the source table to retrieve the top K nearest neighbors.
 *
 * @param indexName - Name of the vector index
 * @param vectorParam - SQL parameter or literal for the query vector
 * @param k - Number of nearest neighbors to return
 * @returns SQL fragment string
 *
 * @example
 * ```typescript
 * import { vectorTopK } from 'relq/turso-builder';
 *
 * // In a raw SQL query:
 * const fragment = vectorTopK('idx_docs_embedding', '?', 10);
 * // → "vector_top_k('idx_docs_embedding', ?, 10)"
 *
 * // Full query pattern:
 * // SELECT d.* FROM vector_top_k('idx_docs_embedding', ?, 10) AS v
 * // JOIN documents d ON d.rowid = v.id;
 * ```
 *
 * @since 1.1.0
 */
export declare function vectorTopK(indexName: string, vectorParam: string, k: number): string;
/**
 * Generate a `vector_distance_cos()` SQL expression for cosine distance.
 *
 * @param column - Column name containing the vector
 * @param vectorParam - SQL parameter or literal for the comparison vector
 * @returns SQL expression string
 *
 * @example
 * ```typescript
 * import { vectorDistanceCos } from 'relq/turso-builder';
 *
 * const expr = vectorDistanceCos('embedding', '?');
 * // → "vector_distance_cos(\"embedding\", ?)"
 * ```
 *
 * @since 1.1.0
 */
export declare function vectorDistanceCos(column: string, vectorParam: string): string;
/**
 * Generate a `vector_distance_l2()` SQL expression for Euclidean distance.
 *
 * @param column - Column name containing the vector
 * @param vectorParam - SQL parameter or literal for the comparison vector
 * @returns SQL expression string
 ...
 * @since 1.1.0
 */
export declare function vectorDistanceL2(column: string, vectorParam: string): string;
/**
 * Wrap a JavaScript array as a `vector32('[...]')` SQL literal.
 *
 * @param values - Array of numbers representing the vector
 * @returns SQL literal string like `vector32('[0.1, 0.2, ...]')`
 *
 * @example
 * ```typescript
 * import { toVector32 } from 'relq/turso-builder';
 *
 * const literal = toVector32([0.1, 0.2, 0.3]);
 * // → "vector32('[0.1,0.2,0.3]')"
 * ```
 *
 * @since 1.1.0
 */
export declare function toVector32(values: number[]): string;
/**
 * Wrap a JavaScript array as a `vector64('[...]')` SQL literal.
 *
 * @param values - Array of numbers representing the vector
 * @returns SQL literal string like `vector64('[0.1, 0.2, ...]')`
 *
 * @since 1.1.0
 */
export declare function toVector64(values: number[]): string;
