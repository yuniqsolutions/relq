...
export declare const DEFAULT: {
	...
	/**
	 * Generate MD5 hash of a random UUID. Produces a 32-char hex string.
	 * Common pattern for generating short unique text identifiers.
	 * @example id: varchar(32).default(DEFAULT.md5RandomUuid())
	 */
	readonly md5RandomUuid: () => DefaultValue;
	/**
...
export declare const generatedFn: GeneratedFn;
export declare function createColumn<T>(type: string): ColumnBuilder<T>;
export declare function createColumnWithName<T>(type: string, columnName?: string): ColumnBuilder<T>;
/** Format value for SQL */
export declare function formatGenValue(val: ChainableExpr | GeneratedExpr | string | number | null): string;
/** Create a chainable expression with all arithmetic methods */
export declare function chainableExpr(sql: string): ChainableExpr;
/** Helper to format value for SQL in FluentGenExpr */
export declare function formatFluentValue(val: FluentGenExpr | GeneratedExpr | string | number | boolean | null): string;
...
export declare function createCaseBuilder(): CaseBuilder;
/**
...
/**
 * Creates a MONEY column for currency values.
 *
 * MONEY is a fixed-precision type for currency storage with locale-aware
 * formatting. It stores values with 2 decimal places (cents).
 *
 * @param columnName - Optional database column name override
 * @returns ColumnBuilder for MONEY type
 *
 * @example
 * ```typescript
 * const table = defineTable('invoices', {
 *   total: money('total'),  // e.g., '$1,234.56'
 *   tax: money('tax'),
 * });
 * ```
 *
 ...
 * | Dialect     | Support | Notes                                    |
 * |-------------|---------|------------------------------------------|
 * | PostgreSQL  | ✅ Full  | Locale-aware formatting                  |
 * | CockroachDB | ❌ None  | Use DECIMAL(19,4) instead                |
 * | Nile        | ✅ Full  | PG15 compatible                          |
 * | AWS DSQL    | ❌ None  | Use DECIMAL instead                      |
 * | SQLite      | ❌ None  | Use REAL or INTEGER (cents)              |
 * | MySQL       | ❌ None  | Use DECIMAL(19,4)                        |
 *
 * **Recommendation:** Many developers prefer DECIMAL for currency to avoid
 * locale dependency and ensure cross-database compatibility.
 *
 * @since 1.0.0
 * @see {@link decimal} for locale-independent currency storage
 */
...
/**
 * Creates a POINT column for 2D geometric points.
 *
 * @param columnName - Optional database column name
 * @returns ColumnBuilder for POINT type
 *
 * @example
 * ```typescript
 * location: point('location'),
 * // Value: { x: 40.7128, y: -74.0060 }
 * ```
 *
 * @remarks
 * **Dialect Support:**
 * | Dialect     | Support | Notes                                    |
 * |-------------|---------|------------------------------------------|
 * | PostgreSQL  | ✅ Full  | Native geometric type                    |
 * | CockroachDB | ❌ None  | Use JSONB for GeoJSON                    |
 * | Nile        | ✅ Full  | PG15 compatible                          |
 * | AWS DSQL    | ❌ None  | Use JSONB for coordinates                |
 * | SQLite      | ❌ None  | Use TEXT or JSONB                        |
 * | MySQL       | ⚠️ Spatial | POINT spatial type                     |
 *
 * @since 1.0.0
 */
...
/**
 * Creates a LINE column for infinite lines (ax + by + c = 0).
 *
 * @remarks
...
/**
 * Creates a LSEG column for finite line segments.
 *
 * @remarks
 * **Dialect Support:** PostgreSQL ✅ | CockroachDB ❌ | Nile ✅ | DSQL ❌
 * @since 1.0.0
...
/**
 * Creates a BOX column for rectangular boxes.
 *
 * @remarks
 * **Dialect Support:** PostgreSQL ✅ | CockroachDB ❌ | Nile ✅ | DSQL ❌
 * @since 1.0.0
...
/**
 * Creates a PATH column for geometric paths (open or closed).
 *
 * @remarks
 * **Dialect Support:** PostgreSQL ✅ | CockroachDB ❌ | Nile ✅ | DSQL ❌
 * @since 1.0.0
...
/**
 * Creates a POLYGON column for closed geometric paths.
 *
 * @remarks
 * **Dialect Support:** PostgreSQL ✅ | CockroachDB ❌ | Nile ✅ | DSQL ❌
 * @since 1.0.0
...
/**
 * Creates a CIRCLE column for circles (center point + radius).
 *
 * @remarks
 * **Dialect Support:** PostgreSQL ✅ | CockroachDB ❌ | Nile ✅ | DSQL ❌
 * @since 1.0.0
...
/**
 * Creates an XML column for storing XML data.
 *
 * @param columnName - Optional database column name
 * @returns ColumnBuilder for XML type
 *
 * @remarks
 * **Dialect Support:**
 * | Dialect     | Support | Notes                                    |
 * |-------------|---------|------------------------------------------|
 * | PostgreSQL  | ✅ Full  | Native XML with XPath, XSLT              |
 * | CockroachDB | ❌ None  | Use TEXT instead                         |
 * | Nile        | ❌ None  | Use TEXT instead                         |
 * | AWS DSQL    | ❌ None  | Use TEXT instead                         |
 * | SQLite      | ❌ None  | Use TEXT                                 |
 * | MySQL       | ❌ None  | Use TEXT                                 |
 *
 * @since 1.0.0
...
/**
 * Creates an INT4RANGE column for ranges of integer values.
 *
 * Range types represent a range of values with lower and upper bounds.
 * Bounds can be inclusive `[` `]` or exclusive `(` `)`.
 *
 * @returns ColumnBuilder for INT4RANGE type
 *
 * @example
 * ```typescript
 * const table = defineTable('reservations', {
 *   seats: int4range().notNull(),  // e.g., '[1,10)' - seats 1-9
 * });
 *
 * // Insert: '[1, 100]' includes 1 and 100
 * // Insert: '(1, 100)' excludes 1 and 100
 * // Insert: '[1, 100)' includes 1, excludes 100
 * ```
 *
 * @remarks
 * **Dialect Support:**
 * | Dialect     | Support | Notes                                    |
 * |-------------|---------|------------------------------------------|
 * | PostgreSQL  | ✅ Full  | Native range types with GiST indexing    |
 * | CockroachDB | ❌ None  | Not supported                            |
 * | Nile        | ✅ Full  | PG15 compatible                          |
 * | AWS DSQL    | ❌ None  | Use composite/JSON workaround            |
 * | SQLite      | ❌ None  | No range types                           |
 * | MySQL       | ❌ None  | No range types                           |
 *
 * @since 1.0.0
 * @see {@link int8range} for bigint ranges
 * @see {@link numrange} for numeric/decimal ranges
 */
export declare const int4range: () => ColumnBuilder<string, ColumnConfig<string>>;
/**
 * Creates an INT8RANGE column for ranges of bigint values.
 *
 * @returns ColumnBuilder for INT8RANGE type
 *
 * @example
 * ```typescript
 * const table = defineTable('time_blocks', {
 *   tickRange: int8range().notNull(),  // For large integer ranges
 * });
 * ```
 *
 * @remarks
 ...
 *
 * @since 1.0.0
 * @see {@link int4range} for integer ranges
 */
export declare const int8range: () => ColumnBuilder<string, ColumnConfig<string>>;
/**
 * Creates a NUMRANGE column for ranges of numeric/decimal values.
 *
 * Useful for continuous numeric ranges like price ranges, measurement ranges.
 *
 * @returns ColumnBuilder for NUMRANGE type
 *
 * @example
 * ```typescript
 * const table = defineTable('products', {
 *   priceRange: numrange().notNull(),  // e.g., '[9.99, 29.99]'
 * });
 * ```
 *
 * @remarks
...
/**
 * Creates a TSRANGE column for ranges of timestamp without time zone.
 *
 * Useful for scheduling, time slots, and period tracking.
 *
 * @returns ColumnBuilder for TSRANGE type
 *
 * @example
 * ```typescript
 * const table = defineTable('events', {
 *   duration: tsrange().notNull(),  // Event time range
 * });
 *
 * // Use exclusion constraint to prevent overlapping:
 * // EXCLUDE USING GIST (room WITH =, duration WITH &&)
 * ```
 *
 * @remarks
 * **Dialect Support:**
 * | Dialect     | Support | Notes                                    |
 * |-------------|---------|------------------------------------------|
 * | PostgreSQL  | ✅ Full  | Native, supports exclusion constraints   |
 * | CockroachDB | ❌ None  | Not supported                            |
 * | Nile        | ✅ Full  | PG15 compatible                          |
 * | AWS DSQL    | ❌ None  | Use two timestamp columns instead        |
 * | SQLite      | ❌ None  | No range types                           |
 * | MySQL       | ❌ None  | No range types                           |
 *
 * @since 1.0.0
 * @see {@link tstzrange} for timestamp with time zone ranges
 */
export declare const tsrange: () => ColumnBuilder<string, ColumnConfig<string>>;
/**
 * Creates a TSTZRANGE column for ranges of timestamp with time zone.
 *
 * Preferred over TSRANGE when dealing with time zones or global applications.
 *
 * @returns ColumnBuilder for TSTZRANGE type
 *
 * @example
 * ```typescript
 * const table = defineTable('bookings', {
 *   period: tstzrange().notNull(),  // Booking period with timezone
 * });
 *
 * // Query for overlapping: WHERE period && '[2024-01-01, 2024-01-31]'
 * ```
 *
 * @remarks
 * **Dialect Support:**
 * | Dialect     | Support | Notes                                    |
 * |-------------|---------|------------------------------------------|
 * | PostgreSQL  | ✅ Full  | Native, timezone-aware                   |
 * | CockroachDB | ❌ None  | Not supported                            |
 * | Nile        | ✅ Full  | PG15 compatible                          |
 * | AWS DSQL    | ❌ None  | Use two timestamptz columns instead      |
 * | SQLite      | ❌ None  | No range types                           |
 * | MySQL       | ❌ None  | No range types                           |
 *
 * @since 1.0.0
 * @see {@link tsrange} for timestamp without time zone ranges
 */
export declare const tstzrange: () => ColumnBuilder<string, ColumnConfig<string>>;
/**
 * Creates a DATERANGE column for ranges of dates.
 *
 * Useful for date periods like fiscal years, seasons, availability.
 *
 * @returns ColumnBuilder for DATERANGE type
 *
 * @example
 * ```typescript
 * const table = defineTable('rentals', {
 *   rentalPeriod: daterange().notNull(),  // e.g., '[2024-01-01, 2024-01-15)'
 * });
 *
 * // Check if date is in range: WHERE '2024-01-10' <@ rental_period
 * ```
 *
 * @remarks
 * **Dialect Support:**
 * | Dialect     | Support | Notes                                    |
 * |-------------|---------|------------------------------------------|
 * | PostgreSQL  | ✅ Full  | Native date range support                |
 * | CockroachDB | ❌ None  | Not supported                            |
 * | Nile        | ✅ Full  | PG15 compatible                          |
 * | AWS DSQL    | ❌ None  | Use two date columns instead             |
 * | SQLite      | ❌ None  | No range types                           |
 * | MySQL       | ❌ None  | No range types                           |
 *
 * @since 1.0.0
...
/**
 * Creates an INT4MULTIRANGE column for multiple integer ranges.
 *
 * Multirange types can hold multiple disjoint ranges in a single value,
 * automatically merging adjacent/overlapping ranges.
 *
 * @returns ColumnBuilder for INT4MULTIRANGE type
 *
 * @example
 * ```typescript
 * const table = defineTable('schedules', {
 *   availableSlots: int4multirange(),  // e.g., '{[1,5), [10,15)}'
 * });
 * ```
 *
 * @remarks
 ...
 * | Dialect     | Support | Notes                                    |
 * |-------------|---------|------------------------------------------|
 * | PostgreSQL  | ✅ Full  | PostgreSQL 14+ only                      |
 * | CockroachDB | ❌ None  | Not supported                            |
 * | Nile        | ✅ Full  | PG15 compatible                          |
 * | AWS DSQL    | ❌ None  | Not supported                            |
 * | SQLite      | ❌ None  | No multirange types                      |
 * | MySQL       | ❌ None  | No multirange types                      |
 *
 * @since 1.0.0
 * @see {@link int4range} for single integer ranges
 */
export declare const int4multirange: () => ColumnBuilder<string, ColumnConfig<string>>;
/**
 * Creates an INT8MULTIRANGE column for multiple bigint ranges.
 *
 * @returns ColumnBuilder for INT8MULTIRANGE type
 *
 * @remarks
 * **Dialect Support:**
 * | Dialect     | Support | Notes                                    |
 ...
 *
 * @since 1.0.0
 * @see {@link int8range} for single bigint ranges
 */
export declare const int8multirange: () => ColumnBuilder<string, ColumnConfig<string>>;
/**
 * Creates a NUMMULTIRANGE column for multiple numeric ranges.
 *
 * @returns ColumnBuilder for NUMMULTIRANGE type
 *
 * @remarks
 * **Dialect Support:**
 * | Dialect     | Support | Notes                                    |
 ...
 *
 * @since 1.0.0
 * @see {@link numrange} for single numeric ranges
 */
export declare const nummultirange: () => ColumnBuilder<string, ColumnConfig<string>>;
/**
 * Creates a TSMULTIRANGE column for multiple timestamp ranges.
 *
 * @returns ColumnBuilder for TSMULTIRANGE type
 *
 * @example
 * ```typescript
 * const table = defineTable('availability', {
 *   openHours: tsmultirange(),  // Multiple time slots
 * });
 * ```
 *
 * @remarks
 ...
 *
 * @since 1.0.0
 * @see {@link tsrange} for single timestamp ranges
 */
export declare const tsmultirange: () => ColumnBuilder<string, ColumnConfig<string>>;
/**
 * Creates a TSTZMULTIRANGE column for multiple timestamp with timezone ranges.
 *
 * @returns ColumnBuilder for TSTZMULTIRANGE type
 *
 * @remarks
 * **Dialect Support:**
 * | Dialect     | Support | Notes                                    |
 ...
 *
 * @since 1.0.0
 * @see {@link tstzrange} for single timestamptz ranges
 */
export declare const tstzmultirange: () => ColumnBuilder<string, ColumnConfig<string>>;
/**
 * Creates a DATEMULTIRANGE column for multiple date ranges.
 *
 * @returns ColumnBuilder for DATEMULTIRANGE type
 *
 * @example
 * ```typescript
 * const table = defineTable('holidays', {
 *   closedDates: datemultirange(),  // Multiple date ranges
 * });
 * ```
 *
 * @remarks
 ...
 *
 * @since 1.0.0
 * @see {@link daterange} for single date ranges
 */
export declare const datemultirange: () => ColumnBuilder<string, ColumnConfig<string>>;
/**
 * Creates an OID (Object Identifier) column.
 *
 * OID is an unsigned 4-byte integer used internally by PostgreSQL for
 * referencing system catalog objects. Rarely used directly in application code.
 *
 * @returns ColumnBuilder for OID type
 *
 * @example
 * ```typescript
 * const table = defineTable('audit_log', {
 *   tableOid: oid(),  // Reference to pg_class.oid
 * });
 * ```
 *
 * @remarks
 ...
 * | Dialect     | Support | Notes                                    |
 * |-------------|---------|------------------------------------------|
 * | PostgreSQL  | ✅ Full  | Internal type, system catalogs           |
 * | CockroachDB | ✅ Full  | For compatibility                        |
 * | Nile        | ✅ Full  | PG15 compatible                          |
 * | AWS DSQL    | ⚠️ Limited | May have restrictions                  |
 * | SQLite      | ❌ None  | No OID type, use INTEGER                 |
 * | MySQL       | ❌ None  | No OID type, use INT UNSIGNED            |
 *
 * @since 1.0.0
 */
export declare const oid: () => ColumnBuilder<number, ColumnConfig<number>>;
/**
 * Creates a REGCLASS column for table/relation identifiers.
 *
 * REGCLASS is an alias for OID with automatic name resolution.
 * Input accepts table names that are resolved to their OID.
 *
 * @returns ColumnBuilder for REGCLASS type
 *
 * @example
 * ```typescript
 * const table = defineTable('table_references', {
 *   targetTable: regclass().notNull(),  // 'users'::regclass
 * });
 * ```
 *
 * @remarks
 ...
 * | Dialect     | Support | Notes                                    |
 * |-------------|---------|------------------------------------------|
 * | PostgreSQL  | ✅ Full  | Auto-resolves table names to OID         |
 * | CockroachDB | ⚠️ Partial | Limited system catalog access         |
 * | Nile        | ⚠️ Partial | May have restrictions                  |
 * | AWS DSQL    | ❌ None  | Not supported                            |
 * | SQLite      | ❌ None  | No equivalent                            |
 * | MySQL       | ❌ None  | No equivalent                            |
 *
 * @since 1.0.0
 * @see {@link oid} for raw object identifiers
 */
export declare const regclass: () => ColumnBuilder<string, ColumnConfig<string>>;
/**
 * Creates a REGPROC column for function identifiers.
 *
 * REGPROC is an alias for OID with automatic function name resolution.
 *
 * @returns ColumnBuilder for REGPROC type
 *
 * @remarks
 * **Dialect Support:**
 * | Dialect     | Support | Notes                                    |
 * |-------------|---------|------------------------------------------|
 * | PostgreSQL  | ✅ Full  | Auto-resolves function names to OID      |
 * | CockroachDB | ⚠️ Partial | Limited system catalog access         |
 * | Nile        | ⚠️ Partial | May have restrictions                  |
 * | AWS DSQL    | ❌ None  | Not supported                            |
 * | SQLite      | ❌ None  | No equivalent                            |
...
/**
 * Creates a REGTYPE column for type identifiers.
 *
 * REGTYPE is an alias for OID with automatic type name resolution.
 *
 * @returns ColumnBuilder for REGTYPE type
 *
 * @remarks
 * **Dialect Support:**
 * | Dialect     | Support | Notes                                    |
 * |-------------|---------|------------------------------------------|
 * | PostgreSQL  | ✅ Full  | Auto-resolves type names to OID          |
 * | CockroachDB | ⚠️ Partial | Limited system catalog access         |
 * | Nile        | ⚠️ Partial | May have restrictions                  |
 * | AWS DSQL    | ❌ None  | Not supported                            |
 * | SQLite      | ❌ None  | No equivalent                            |
...
/**
 * Creates a PG_LSN (Log Sequence Number) column.
 *
 * PG_LSN is used for tracking Write-Ahead Log positions.
 * Useful for replication monitoring and point-in-time recovery.
 *
 * @returns ColumnBuilder for PG_LSN type
 *
 * @example
 * ```typescript
 * const table = defineTable('replication_status', {
 *   lastLsn: pgLsn().notNull(),  // e.g., '0/1A2B3C4D'
 * });
 * ```
 *
 * @remarks
 ...
 * | Dialect     | Support | Notes                                    |
 * |-------------|---------|------------------------------------------|
 * | PostgreSQL  | ✅ Full  | WAL position tracking                    |
 * | CockroachDB | ❌ None  | Different replication model              |
 * | Nile        | ⚠️ Limited | Internal use only                      |
 * | AWS DSQL    | ❌ None  | Not supported                            |
 * | SQLite      | ❌ None  | No equivalent                            |
 * | MySQL       | ❌ None  | Use GTID for replication tracking        |
 *
 * @since 1.0.0
 */
export declare const pgLsn: () => ColumnBuilder<string, ColumnConfig<string>>;
/**
 * Creates a PG_SNAPSHOT column for transaction snapshots.
 *
 * PG_SNAPSHOT stores a point-in-time view of transaction IDs,
 * used for MVCC visibility rules. Primarily for advanced replication.
 *
 * @returns ColumnBuilder for PG_SNAPSHOT type
 *
 * @remarks
 * **Dialect Support:**
 * | Dialect     | Support | Notes                                    |
 * |-------------|---------|------------------------------------------|
 * | PostgreSQL  | ✅ Full  | PostgreSQL 13+ (replaces txid_snapshot)  |
 * | CockroachDB | ❌ None  | Different MVCC implementation            |
 * | Nile        | ⚠️ Limited | Internal use only                      |
 * | AWS DSQL    | ❌ None  | Not supported                            |
 * | SQLite      | ❌ None  | No equivalent                            |
 * | MySQL       | ❌ None  | No equivalent                            |
...
/**
 * Creates a CITEXT (case-insensitive text) column.
 *
 * Requires the `citext` extension: `CREATE EXTENSION citext;`
 * Case-insensitive comparisons without using LOWER() everywhere.
 *
 * @param columnName - Optional database column name override
 * @returns ColumnBuilder for CITEXT type
 *
 * @example
 * ```typescript
 * const table = defineTable('users', {
 *   email: citext('email').notNull().unique(),  // Email comparisons case-insensitive
 * });
 *
 ...
 * | Dialect     | Support | Notes                                    |
 * |-------------|---------|------------------------------------------|
 * | PostgreSQL  | ✅ Full  | Requires citext extension                |
 * | CockroachDB | ❌ None  | Use COLLATE "en_US" or manual LOWER()    |
 * | Nile        | ⚠️ Ext   | Extension must be enabled                |
 * | AWS DSQL    | ❌ None  | Use TEXT with application-level compare  |
 * | SQLite      | ⚠️ COLLATE | Use `COLLATE NOCASE`                  |
 * | MySQL       | ⚠️ COLLATE | Use case-insensitive collation        |
 *
 * @since 1.0.0
 */
export declare const citext: (columnName?: string) => ColumnBuilder<string, ColumnConfig<string>>;
/**
 * Creates an LTREE (label tree) column for hierarchical data.
 *
 * Requires the `ltree` extension: `CREATE EXTENSION ltree;`
 * Stores tree-like structures as dot-separated label paths.
 *
 * @param columnName - Optional database column name override
 * @returns ColumnBuilder for LTREE type
 *
 * @example
 * ```typescript
 * const table = defineTable('categories', {
 *   path: ltree('path').notNull(),  // e.g., 'root.electronics.phones.apple'
 * });
 *
 * // Query descendants: WHERE path <@ 'root.electronics'
 * // Query ancestors: WHERE 'root.electronics.phones' <@ path
 * ```
 *
 * @remarks
 * **Dialect Support:**
 * | Dialect     | Support | Notes                                    |
 * |-------------|---------|------------------------------------------|
 * | PostgreSQL  | ✅ Full  | Requires ltree extension, GiST indexable |
 * | CockroachDB | ❌ None  | Use TEXT with materialized path pattern  |
 * | Nile        | ⚠️ Ext   | Extension must be enabled                |
 * | AWS DSQL    | ❌ None  | Use TEXT with application-level logic    |
 * | SQLite      | ❌ None  | Use TEXT with recursive CTEs             |
 * | MySQL       | ❌ None  | Use TEXT with nested set or closure      |
 *
 * @since 1.0.0
 * @see {@link lquery} for querying ltree columns
 * @see {@link ltxtquery} for text search on ltree
 */
export declare const ltree: (columnName?: string) => ColumnBuilder<string, ColumnConfig<string>>;
/**
 * Creates an LQUERY column for ltree path queries.
 *
 * LQUERY is a query language for ltree pattern matching.
 * Supports wildcards: `*` (any label), `_` (single label), `{a,b}` (alternatives).
 *
 * @param columnName - Optional database column name override
 * @returns ColumnBuilder for LQUERY type
 *
 * @example
 * ```typescript
 * // Pattern: 'root.*.phones' matches 'root.electronics.phones'
 * // Pattern: 'root.{electronics,appliances}.*' matches multiple paths
 * ```
 *
 * @remarks
 * **Dialect Support:**
 * | Dialect     | Support | Notes                                    |
 * |-------------|---------|------------------------------------------|
 * | PostgreSQL  | ✅ Full  | Requires ltree extension                 |
 * | CockroachDB | ❌ None  | Not supported                            |
 * | Nile        | ⚠️ Ext   | Extension must be enabled                |
 * | AWS DSQL    | ❌ None  | Not supported                            |
 * | SQLite      | ❌ None  | Not supported                            |
 * | MySQL       | ❌ None  | Not supported                            |
 *
 * @since 1.0.0
 * @see {@link ltree} for label tree data
 */
export declare const lquery: (columnName?: string) => ColumnBuilder<string, ColumnConfig<string>>;
/**
 * Creates an LTXTQUERY column for full-text search on ltree.
 *
 * Combines ltree path matching with text search capabilities.
 *
 * @param columnName - Optional database column name override
 * @returns ColumnBuilder for LTXTQUERY type
 *
 * @remarks
 * **Dialect Support:**
 * | Dialect     | Support | Notes                                    |
...
/**
 * Creates an HSTORE column for key-value storage.
 *
 * Requires the `hstore` extension: `CREATE EXTENSION hstore;`
 * Stores key-value pairs as a single column value.
 *
 * @param columnName - Optional database column name override
 * @returns ColumnBuilder for HSTORE type with Record<string, string | null> TypeScript type
 *
 * @example
 * ```typescript
 * const table = defineTable('products', {
 *   attributes: hstore('attributes'),  // '"color"=>"red", "size"=>"large"'
 * });
 *
 * // Query: WHERE attributes -> 'color' = 'red'
 * // Check key exists: WHERE attributes ? 'color'
 * ```
 *
 * @remarks
 * **Dialect Support:**
 * | Dialect     | Support | Notes                                    |
 * |-------------|---------|------------------------------------------|
 * | PostgreSQL  | ✅ Full  | Requires hstore extension, GIN indexable |
 * | CockroachDB | ❌ None  | Use JSONB instead                        |
 * | Nile        | ⚠️ Ext   | Extension must be enabled                |
 * | AWS DSQL    | ❌ None  | Use JSONB or TEXT                        |
 * | SQLite      | ❌ None  | Use JSON1 extension                      |
 * | MySQL       | ❌ None  | Use JSON type                            |
 *
 * **Note:** Consider using JSONB instead for new projects as it's more
 * flexible and doesn't require an extension.
 *
 * @since 1.0.0
 * @see {@link jsonb} for JSON storage (recommended alternative)
 */
...
/**
 * Creates a CUBE column for multi-dimensional points.
 *
 * Requires the `cube` extension: `CREATE EXTENSION cube;`
 * Used for n-dimensional point storage and distance calculations.
 *
 * @param columnName - Optional database column name override
 * @returns ColumnBuilder for CUBE type
 *
 * @example
 * ```typescript
 * const table = defineTable('colors', {
 *   rgb: cube('rgb'),  // (255, 128, 0) - 3D point
 * });
 *
 * // Distance query: ORDER BY rgb <-> '(255, 0, 0)' (Euclidean distance)
 * ```
 *
 * @remarks
 * **Dialect Support:**
 * | Dialect     | Support | Notes                                    |
 * |-------------|---------|------------------------------------------|
 * | PostgreSQL  | ✅ Full  | Requires cube extension                  |
 * | CockroachDB | ❌ None  | Use array of floats                      |
 * | Nile        | ⚠️ Ext   | Extension must be enabled                |
 * | AWS DSQL    | ❌ None  | Use array or JSON                        |
 * | SQLite      | ❌ None  | Use JSON array                           |
 * | MySQL       | ❌ None  | Use JSON array                           |
 *
 * @since 1.0.0
 */
export declare const cube: (columnName?: string) => ColumnBuilder<number[], ColumnConfig<number[]>>;
/**
 * Creates a SEMVER column for semantic versioning.
 *
 * Requires the `semver` extension: `CREATE EXTENSION semver;`
 * Proper sorting and comparison of semantic version strings.
 *
 * @param columnName - Optional database column name override
 * @returns ColumnBuilder for SEMVER type
 *
 * @example
 * ```typescript
 * const table = defineTable('packages', {
 *   version: semver('version').notNull(),  // '1.2.3', '2.0.0-beta.1'
 * });
 *
 * // Query: WHERE version > '1.0.0' AND version < '2.0.0'
 * ```
 *
 * @remarks
 * **Dialect Support:**
 * | Dialect     | Support | Notes                                    |
 * |-------------|---------|------------------------------------------|
 * | PostgreSQL  | ✅ Full  | Requires semver extension                |
 * | CockroachDB | ❌ None  | Use TEXT with app-level parsing          |
 * | Nile        | ⚠️ Ext   | Extension must be enabled                |
 * | AWS DSQL    | ❌ None  | Use TEXT with app-level parsing          |
 * | SQLite      | ❌ None  | Use TEXT with app-level parsing          |
 * | MySQL       | ❌ None  | Use VARCHAR with app-level parsing       |
 *
 * @since 1.0.0
 */
export declare const semver: (columnName?: string) => ColumnBuilder<string, ColumnConfig<string>>;
/**
 * TypeScript type representing a vector/embedding for pgvector.
 *
 * Vectors are stored as dense arrays of floating-point numbers.
 * Common embedding dimensions: 384, 768, 1024, 1536, 3072.
 *
 * @example
 * ```typescript
 * const embedding: VectorArray = [0.1, 0.2, 0.3, ...]; // 1536 dimensions
 * ```
 */
export type VectorArray = number[];
/**
 * TypeScript type for sparse vectors in pgvector.
 *
 * Sparse vectors store only non-zero values with their indices.
 * Format: `{indices: [i1, i2, ...], values: [v1, v2, ...], dimensions: n}`
 *
 * @example
 * ```typescript
 * // Sparse vector with 3 non-zero values in 1000 dimensions
...
/**
 * Creates a pgvector VECTOR column for storing dense embeddings.
 *
 * Requires the `pgvector` extension: `CREATE EXTENSION vector;`
 * Used for AI/ML embeddings from models like OpenAI, Cohere, HuggingFace.
 *
 * @param dimensions - Number of dimensions (e.g., 1536 for OpenAI ada-002)
 * @param columnName - Optional database column name override
 * @returns ColumnBuilder for VECTOR type with proper TypeScript inference
 *
 * @example
 * ```typescript
 * const table = defineTable('documents', {
 ...
 * | CockroachDB | ❌ None  | Not supported - use ARRAY or JSONB       |
 * | Nile        | ⚠️ Ext   | Extension may need to be enabled         |
 * | AWS DSQL    | ❌ None  | Not supported                            |
 * | SQLite      | ❌ None  | Use sqlite-vec extension separately      |
 * | MySQL       | ❌ None  | Not supported                            |
 *
 * **Index Types (pgvector 0.5+):**
 * - `ivfflat`: Faster build, good for < 1M vectors
 * - `hnsw`: Slower build, better recall for large datasets
 *
 * **Distance Operators:**
 ...
 *
 * @since 1.0.0
 * @see {@link halfvec} for half-precision vectors (memory efficient)
 * @see {@link sparsevec} for sparse vectors
 */
export declare const vector: (dimensions: number, columnName?: string) => ColumnBuilder<VectorArray, ColumnConfig<VectorArray>>;
/**
 * Creates a pgvector HALFVEC column for half-precision embeddings.
 *
 * Requires pgvector 0.7+ and the extension: `CREATE EXTENSION vector;`
 * Uses 16-bit floats instead of 32-bit, reducing storage by ~50%.
 *
 * @param dimensions - Number of dimensions
 * @param columnName - Optional database column name override
 * @returns ColumnBuilder for HALFVEC type with proper TypeScript inference
 *
 * @example
 * ```typescript
 * const table = defineTable('documents', {
 *   id: serial().primaryKey(),
 *
 *   // Half-precision embedding (50% smaller storage)
 *   embedding: halfvec(1536, 'embedding').notNull(),
 * });
 *
 * // HNSW index with halfvec
 ...
 * | Dialect     | Support | Notes                                    |
 * |-------------|---------|------------------------------------------|
 * | PostgreSQL  | ✅ Full  | Requires pgvector 0.7+ extension         |
 * | CockroachDB | ❌ None  | Not supported                            |
 * | Nile        | ⚠️ Ext   | Extension may need to be enabled         |
 * | AWS DSQL    | ❌ None  | Not supported                            |
 * | SQLite      | ❌ None  | Not supported                            |
 ...
 * - ⚠️ Slightly lower precision (usually negligible for embeddings)
 *
 * @since 1.0.0
 * @see {@link vector} for full-precision vectors
 * @see {@link sparsevec} for sparse vectors
 */
export declare const halfvec: (dimensions: number, columnName?: string) => ColumnBuilder<VectorArray, ColumnConfig<VectorArray>>;
/**
 * Creates a pgvector SPARSEVEC column for sparse embeddings.
 *
 * Requires pgvector 0.7+ and the extension: `CREATE EXTENSION vector;`
 * Efficient for vectors with many zero values (e.g., TF-IDF, BM25, SPLADE).
 *
 * @param dimensions - Maximum number of dimensions
 * @param columnName - Optional database column name override
 * @returns ColumnBuilder for SPARSEVEC type with proper TypeScript inference
 *
 * @example
 * ```typescript
 * const table = defineTable('documents', {
 *   id: serial().primaryKey(),
 *
 *   // SPLADE sparse embedding (up to 30522 dimensions, mostly zeros)
 *   sparse_embedding: sparsevec(30522, 'sparse_embedding'),
 *
 *   // TF-IDF sparse vector
 *   tfidf: sparsevec(10000),
 * });
 * ```
 *
 * @remarks
 ...
 *
 * **Use Cases:**
 * - SPLADE embeddings (sparse neural retrieval)
 * - TF-IDF vectors
 * - BM25 sparse representations
 * - Any embedding with many zero values
 *
 * **PostgreSQL format:** `{1:0.5, 5:0.3, 999:0.1}/1000`
 * (indices:values/dimensions)
 *
 * @since 1.0.0
 * @see {@link vector} for dense vectors
 * @see {@link halfvec} for half-precision dense vectors
 */
export declare const sparsevec: (dimensions: number, columnName?: string) => ColumnBuilder<SparseVector, ColumnConfig<SparseVector>>;
/**
 * GeoJSON-compatible geometry type for PostGIS.
 *
 * This interface matches the GeoJSON specification (RFC 7946) and is used
 * as the TypeScript type for PostGIS geometry and geography columns.
 *
 * @see {@link https://datatracker.ietf.org/doc/html/rfc7946} GeoJSON RFC
 */
...
/**
 * Creates a PostGIS GEOMETRY column for spatial data.
 *
 * Requires the `postgis` extension: `CREATE EXTENSION postgis;`
 * Uses Cartesian (planar) coordinate system for calculations.
 *
 * @param columnName - Optional database column name override
 * @param srid - Spatial Reference System Identifier (e.g., 4326 for WGS84)
 * @param geometryType - Specific geometry type constraint
 * @returns ColumnBuilder for GEOMETRY type
 *
 * @example
 * ```typescript
 * const table = defineTable('locations', {
 *   // Generic geometry (any type)
 *   shape: geometry('shape'),
 *
 *   // Point with WGS84 (GPS coordinates)
 ...
 * });
 *
 * // Create spatial index: CREATE INDEX ON locations USING GIST (location);
 * // Query by distance: WHERE ST_DWithin(location, point, 1000)
 * ```
 *
 * @remarks
 * **Dialect Support:**
 * | Dialect     | Support | Notes                                    |
 * |-------------|---------|------------------------------------------|
 * | PostgreSQL  | ✅ Full  | Requires PostGIS extension               |
 * | CockroachDB | ⚠️ Limited | Native spatial support, no PostGIS    |
 * | Nile        | ⚠️ Ext   | Extension must be enabled                |
 * | AWS DSQL    | ❌ None  | Not supported                            |
 * | SQLite      | ⚠️ SpatiaLite | Requires SpatiaLite extension       |
 * | MySQL       | ⚠️ Native | Native spatial types, different syntax  |
 *
 * **Geometry vs Geography:**
 * - Use `geometry` for projected data (meters, local coordinate systems)
 * - Use `geography` for lat/lon data requiring accurate Earth distances
 *
 * @since 1.0.0
 * @see {@link geography} for geodetic calculations
 * @see {@link geoPoint} for simple point columns
 */
...
/**
 * Creates a PostGIS GEOGRAPHY column for geodetic calculations.
 *
 * Requires the `postgis` extension: `CREATE EXTENSION postgis;`
 * Uses spherical math for accurate distance calculations on Earth's surface.
 *
 * @param columnName - Optional database column name override
 * @param srid - Spatial Reference System Identifier (default: 4326 for WGS84)
 * @param geometryType - Specific geometry type constraint
 * @returns ColumnBuilder for GEOGRAPHY type
 *
 * @example
 * ```typescript
 * const table = defineTable('stores', {
 *   // Store location with accurate Earth distance calculations
 *   location: geography('location', 4326, 'POINT'),
 * });
 *
 * // Query stores within 5km radius (distance in meters):
 ...
 * |-------------|---------|------------------------------------------|
 * | PostgreSQL  | ✅ Full  | Requires PostGIS extension               |
 * | CockroachDB | ⚠️ Limited | Native geography, subset of functions |
 * | Nile        | ⚠️ Ext   | Extension must be enabled                |
 * | AWS DSQL    | ❌ None  | Not supported                            |
 * | SQLite      | ❌ None  | SpatiaLite uses geometry only            |
 * | MySQL       | ❌ None  | No geography type                        |
 *
 * **When to use GEOGRAPHY:**
 * - Calculating real-world distances (km, miles)
 * - Working with GPS coordinates (latitude/longitude)
 * - Queries spanning large areas where Earth curvature matters
 *
 * @since 1.0.0
 * @see {@link geometry} for projected/planar calculations
 */
...
/**
 * Creates a PostGIS POINT geometry column (shorthand).
 *
 * Convenience function for the common case of storing point locations.
 * Equivalent to `geometry(name, srid, 'POINT')`.
 *
 * @param columnName - Optional database column name override
 * @param srid - Spatial Reference System Identifier (default: 4326 for WGS84)
 * @returns ColumnBuilder for GEOMETRY(POINT) type
 *
 * @example
 * ```typescript
 * const table = defineTable('poi', {
 *   location: geoPoint('location'),        // GEOMETRY(POINT, 4326)
 *   localPoint: geoPoint('local', 32632),  // GEOMETRY(POINT, 32632) - UTM zone
 * });
 * ```
 *
 * @remarks
 ...
 * |-------------|---------|------------------------------------------|
 * | PostgreSQL  | ✅ Full  | Requires PostGIS extension               |
 * | CockroachDB | ⚠️ Limited | Native GEOMETRY support                |
 * | Nile        | ⚠️ Ext   | Extension must be enabled                |
 * | AWS DSQL    | ❌ None  | Not supported                            |
 * | SQLite      | ⚠️ SpatiaLite | Requires SpatiaLite extension       |
 * | MySQL       | ⚠️ POINT | Native POINT type                       |
 *
 * @since 1.0.0
 * @see {@link geometry} for full geometry type control
 */
...
/**
 * Creates a PostGIS BOX2D column for 2D bounding boxes.
 *
 * BOX2D represents a 2-dimensional bounding rectangle.
 * Useful for spatial index bounds and extent calculations.
 *
 * @param columnName - Optional database column name override
 * @returns ColumnBuilder for BOX2D type
 *
 * @example
 * ```typescript
 * const table = defineTable('map_tiles', {
 *   bounds: box2d('bounds'),  // e.g., 'BOX(0 0, 100 100)'
 * });
 * ```
 *
 * @remarks
 ...
 * |-------------|---------|------------------------------------------|
 * | PostgreSQL  | ✅ Full  | Requires PostGIS extension               |
 * | CockroachDB | ❌ None  | Use geometry with BOX function           |
 * | Nile        | ⚠️ Ext   | Extension must be enabled                |
 * | AWS DSQL    | ❌ None  | Not supported                            |
 * | SQLite      | ❌ None  | Not supported                            |
 * | MySQL       | ❌ None  | Use Envelope() on geometry               |
 *
 * @since 1.0.0
 */
export declare const box2d: (columnName?: string) => ColumnBuilder<string, ColumnConfig<string>>;
/**
 * Creates a PostGIS BOX3D column for 3D bounding boxes.
 *
 * BOX3D represents a 3-dimensional bounding box including Z coordinates.
 * Used for 3D spatial data and volumetric calculations.
 *
 * @param columnName - Optional database column name override
 * @returns ColumnBuilder for BOX3D type
 *
 * @example
 * ```typescript
 * const table = defineTable('buildings', {
 *   volume: box3d('volume'),  // e.g., 'BOX3D(0 0 0, 100 100 50)'
 * });
 * ```
 *
 * @remarks
 ...
 *
 * @since 1.0.0
 * @see {@link box2d} for 2D bounding boxes
 */
...
export interface ParsedPolicy {
	...
	table: string;
	/** SQL command the policy applies to */
	command: "ALL" | "SELECT" | "INSERT" | "UPDATE" | "DELETE";
	/** Whether this is a PERMISSIVE (default) or RESTRICTIVE policy */
	permissive: boolean;
	/** Roles the policy applies to (empty = PUBLIC) */
	roles: string[];
	/** USING expression (for existing rows) */
	using?: string;
	/** WITH CHECK expression (for new rows) */
	withCheck?: string;
...
/**
 * PostgreSQL (native) - full feature support
 */
export declare const POSTGRES_FEATURES: DialectFeatureSet;
/**
 ...
 * Nile feature set
 */
export declare const NILE_FEATURES: DialectFeatureSet;
/**
 * AWS Aurora DSQL feature set (most restrictive)
 */
export declare const DSQL_FEATURES: DialectFeatureSet;
/**
 ...
 *
 * When defining generated columns via the `generatedAs` callback, the first parameter
 * provides typed access to all table columns. Each column is represented as a
 * {@link FluentGenExpr} with a `__columnName` property for tracking which column
 * the expression targets.
 *
 * @template T - Record of column names to ColumnConfig types
 *
 * @example
 ...
 * generatedAs: (t, As) => [
 *     // t.fullName, t.firstName, t.lastName are all FluentGenExpr instances
 ...
 *
 * @remarks
 * Each property on the resulting type is a {@link FluentGenExpr} instance that supports
 * the full chainable expression API including:
 * - String methods: `.concat()`, `.lower()`, `.upper()`, `.trim()`, `.substring()`
 * - Math methods: `.add()`, `.subtract()`, `.multiply()`, `.divide()`
 * - JSON methods: `.jsonExtract()`, `.jsonExtractText()`
 * - Array methods: `.arrayLength()`, `.arrayToString()`
 * - Full-text search: `.toTsvector()`, `.setWeight()`
 * - Type casting: `.cast()`, `.asText()`, `.asInteger()`
 * - Null handling: `.coalesce()`, `.nullIf()`
 *
 * The `__columnName` property carries the actual SQL column name for each property,
 * which may differ from the TypeScript property key if `$columnName` was configured.
 *
 * @since 1.0.0
 * @see {@link FluentGenExpr} for the full chainable expression API
 * @see {@link GeneratedAsFactory} for the factory that consumes these refs
 */
...
export interface CheckConstraintDef {
	...
	readonly $trackingId?: string;
	/** Set tracking ID for rename detection */
	$id(trackingId: string): CheckConstraintDef;
}
...
export interface ConstraintDef {
	...
	readonly $trackingId?: string;
	/** Set tracking ID for rename detection */
	$id(trackingId: string): ConstraintDef;
}
/**
...
/**
 * @deprecated Use the new pgRelations API instead.
 * @see {@link pg-relations}
 */
...
/**
 * @deprecated Use pgRelations API instead.
 * @see {@link pg-relations}
 */
 ...
 * ```typescript
 * // Before (deprecated):
 * one(users, profiles, { foreignKey: 'user_id' })
 *
 * // After:
 * pgRelations(schema, (rel) => ({
 *   profile: rel.users.hasOne(rel.profiles, {
 *     columns: ['id'],
 *     references: ['user_id'],
 *   }),
 * }));
 ...
 * ```typescript
 * // Before (deprecated):
 * many(users, posts, { foreignKey: 'user_id' })
 *
 * // After:
 * pgRelations(schema, (rel) => ({
 *   posts: rel.users.hasMany(rel.posts, {
 *     columns: ['id'],
 *     references: ['user_id'],
 *   }),
 * }));
 ...
 * ```typescript
 * // Before (deprecated):
 * manyToMany(users, roles, userRoles)
 *
 * // After:
 * pgRelations(schema, (rel) => ({
 *   roles: rel.users.manyToMany(rel.roles, {
 *     through: rel.userRoles,
 *     columns: ['id'],
 *     throughColumns: ['user_id'],
 *     references: ['role_id'],
...
export interface SequenceOptions {
	...
	ownedBy?: string;
	/** Tracking ID for rename detection across pulls */
	trackingId?: string;
}
...
export interface ViewConfig {
	$type: "view";
	/** @modern Use $viewName for new code */
	$viewName: string;
	/** @modern Use $definition for new code */
	$definition: string;
	$isMaterialized: false;
	$trackingId?: string;
	$commentText?: string;
	/** @deprecated Use $viewName — kept for backward compatibility */
	name: string;
	/** @deprecated Use $definition — kept for backward compatibility */
	definition: string;
	/** @deprecated Use $isMaterialized — kept for backward compatibility */
	isMaterialized: false;
	/** Returns AST for schema diffing and migration generation */
	toAST(): ParsedView;
	/** Set tracking ID for rename detection */
	$id(trackingId: string): ViewConfig;
	/** Set comment for the view (chainable) */
	$comment(comment: string): ViewConfig;
}
export interface MaterializedViewConfig {
	...
	/** @deprecated Use $isMaterialized — kept for backward compatibility */
	isMaterialized: true;
	/** @deprecated Use $withData — kept for backward compatibility */
	withData?: boolean;
	/** Returns AST for schema diffing and migration generation */
	toAST(): ParsedView;
	/** Set tracking ID for rename detection */
	$id(trackingId: string): MaterializedViewConfig;
	/** Set comment for the materialized view (chainable) */
	$comment(comment: string): MaterializedViewConfig;
}
/**
 ...
 */
export declare function dropViewSQL(view: ViewConfig, ifExists?: boolean): string;
/**
 * Generate DROP MATERIALIZED VIEW SQL
 */
export declare function dropMaterializedViewSQL(view: MaterializedViewConfig, ifExists?: boolean): string;
/**
 * Check if a value is a ViewConfig
 */
export declare function isViewConfig(value: unknown): value is ViewConfig;
/**
 * Check if a value is a MaterializedViewConfig
 */
export declare function isMaterializedViewConfig(value: unknown): value is MaterializedViewConfig;
/** SQL commands a policy can apply to */
export type PolicyCommand = "ALL" | "SELECT" | "INSERT" | "UPDATE" | "DELETE";
export interface PolicyOptions<TTable = unknown> {
	/**
	 ...
	 * - PERMISSIVE (default): combined with OR among permissive policies
	 * - RESTRICTIVE: combined with AND, applied on top of permissive policies
	 * @default true (PERMISSIVE)
	 */
	permissive?: boolean;
	/**
	 * Roles the policy applies to.
	 * Empty array or omitted = PUBLIC (all roles).
	 *
	 * @example
	 * ```typescript
	 * to: ['authenticated']           // Specific role
	 * to: ['admin', 'moderator']      // Multiple roles
	 * // omit for PUBLIC
	 * ```
	 */
	to?: string[];
	/**
	 * USING expression — applied to existing rows (SELECT, UPDATE, DELETE).
	 * Must be a SQL boolean expression.
	 *
	 * @example
	 * ```typescript
	...
	using?: string;
	/**
	 * WITH CHECK expression — applied to new/modified rows (INSERT, UPDATE).
	 * Must be a SQL boolean expression.
	 *
	 * @example
	 * ```typescript
	...
	withCheck?: string;
	/**
	 * Comment for the policy (versioned with the schema).
	 * Will be applied via COMMENT ON POLICY.
	 */
	comment?: string;
}
export interface PolicyConfig<TTable = unknown> {
	/** Policy name */
	$policyName: string;
	/** Full configuration */
	$options: PolicyOptions<TTable>;
	/** Marker for policy type */
	$type: "policy";
	$trackingId?: string;
	$commentText?: string;
	/** Returns AST for schema diffing and migration generation */
	toAST(): ParsedPolicy;
	/** Set tracking ID for rename detection */
	$id(trackingId: string): PolicyConfig<TTable>;
	/** Set comment for the policy (chainable) */
	$comment(comment: string): PolicyConfig<TTable>;
}
/**
 * Generate CREATE POLICY SQL
 */
export declare function generatePolicySQL(config: PolicyConfig): string;
/**
 * Generate DROP POLICY SQL
 */
export declare function dropPolicySQL(config: PolicyConfig, ifExists?: boolean): string;
/**
 * Generate ALTER TABLE ... ENABLE ROW LEVEL SECURITY
 */
export declare function enableRlsSQL(tableName: string): string;
/**
 * Generate ALTER TABLE ... FORCE ROW LEVEL SECURITY
 */
export declare function forceRlsSQL(tableName: string): string;
/**
 * Define a PostgreSQL Row Level Security (RLS) policy
 *
 * @example
 * ```typescript
 * // Allow users to see only their own rows
 * export const usersOwnRows = pgPolicy('users_own_rows', {
 *     on: usersTable,
 *     for: 'SELECT',
 *     to: ['authenticated'],
 *     using: `id = auth.uid()`,
 * }).$id('p1a2b3')
 *
 * // Allow inserts where user_id matches current user
 * export const usersInsert = pgPolicy('users_insert', {
 *     on: usersTable,
 *     for: 'INSERT',
 *     to: ['authenticated'],
 *     withCheck: `user_id = auth.uid()`,
 * }).$id('p4d5e6')
 *
 * // Restrictive policy (combined with AND)
 * export const tenantIsolation = pgPolicy('tenant_isolation', {
 *     on: ordersTable,
 *     permissive: false,
 *     using: `tenant_id = current_setting('app.tenant_id')::uuid`,
 ...
 * }).$id('p7g8h9')
 *
 * // Admin bypass — all commands, no restrictions
 * export const adminBypass = pgPolicy('admin_bypass', {
 *     on: usersTable,
 *     to: ['admin'],
 *     using: `true`,
 *     withCheck: `true`,
 ...
 * ```
 */
export declare function pgPolicy<TTable = unknown>(name: string, options: PolicyOptions<TTable>): PolicyConfig<TTable>;
/**
 * Check if a value is a PolicyConfig
 */
...
/**
 * Nile Multi-Tenant Database Schema Validation Error Codes
 *
 * Complete registry of error codes, warning codes, and informational codes
 * for Nile dialect schema validation. Each code maps to a specific
 * incompatibility, restriction, or informational note related to Nile's
 * multi-tenant PostgreSQL platform.
 *
 * @module nile-schema/errors
 *
 * @remarks
 * **Code ranges:**
 * | Range              | Category                          |
 * |--------------------|-----------------------------------|
 * | NILE-TC-001-003    | Table classification (tenant detection) |
 * | NILE-PK-001-004    | Primary key restrictions          |
 * | NILE-FK-001-004    | Foreign key restrictions          |
 * | NILE-CT-001-005    | Column type restrictions          |
 * | NILE-TF-001-005    | Trigger/function restrictions     |
 * | NILE-SEQ-001-004   | Sequence restrictions             |
 * | NILE-EXT-001-002   | Extension handling                |
 * | NILE-BT-001-004    | Built-in table restrictions       |
 * | NILE-TX-001-003    | Transaction restrictions          |
 * | NILE-ADM-001-004   | Admin/permission restrictions     |
 * | NILE-CON-001-004   | Constraint restrictions           |
 *
 * @since 1.1.0
 */
/**
 * Severity level for a Nile validation message.
 *
 * - `error`: Schema will not work on Nile. Must be fixed.
 * - `warning`: Schema may work but behavior differs or requires attention.
 * - `info`: Informational note about Nile behavior. No action required.
 *
 * @since 1.1.0
 */
export type NileValidationSeverity = "error" | "warning" | "info";
/**
 * A single Nile schema validation message.
 *
 * @example
 * ```typescript
 * const msg: NileValidationMessage = {
 *     code: 'NILE-PK-001',
 *     severity: 'error',
 *     message: 'Table PRIMARY KEY does not include tenant_id. Fix: PRIMARY KEY (tenant_id, id)',
 *     alternative: 'Add tenant_id as the first column in the PRIMARY KEY',
 ...
 * @since 1.1.0
 */
export interface NileValidationMessage {
	/** Unique rule code (e.g. 'NILE-PK-001'). */
	code: string;
	/** Severity level. */
	severity: NileValidationSeverity;
	/** Human-readable description. */
	message: string;
	/** Recommended alternative or workaround. */
	alternative?: string;
	/** Feature category. */
	category?: string;
	/** Table name context. */
	...
	functionName?: string;
	/** Nile documentation URL. */
	docsUrl?: string;
}
/**
 * Aggregated Nile validation result.
 *
 * @example
 * ```typescript
 * const result = createNileValidationResult(messages);
 * if (!result.valid) {
 *     console.log(`${result.errors.length} error(s) found`);
 * }
 * if (result.info.length > 0) {
 *     console.log(`${result.info.length} informational note(s)`);
 * }
 * ```
 *
 * @since 1.1.0
...
export interface NileValidationResult {
	/** True if there are zero errors (warnings and info are OK). */
	valid: boolean;
	/** Error-severity messages. */
	errors: NileValidationMessage[];
	/** Warning-severity messages. */
	warnings: NileValidationMessage[];
	/** Info-severity messages. */
	info: NileValidationMessage[];
	/** All messages combined. */
	all: NileValidationMessage[];
}
/**
 * Auto-fix descriptor for a Nile validation rule.
 *
 * When a schema element triggers a Nile error, the auto-fix describes
 * the transformation needed to make it Nile-compatible.
 *
 * @example
 * ```typescript
 * const fix: NileAutoFix = {
 *     description: 'Add tenant_id to PRIMARY KEY',
 *     originalType: 'PRIMARY KEY (id)',
 *     replacementType: 'PRIMARY KEY (tenant_id, id)',
 *     additionalChanges: [
 *         'Ensure tenant_id column exists as UUID NOT NULL',
 *     ],
 * };
 * ```
 *
 * @since 1.1.0
 */
export interface NileAutoFix {
	/** Human-readable description of the fix. */
	description: string;
	/** The original type or feature being replaced. */
	originalType: string;
	/** The replacement type or feature. */
	replacementType: string;
	/** Additional schema changes required alongside the type replacement. */
	additionalChanges?: string[];
}
/**
 * Static definition of a Nile validation rule.
 *
 * @since 1.1.0
 */
export interface NileErrorDef {
	/** Unique rule code. */
	code: string;
	/** Severity. */
	severity: NileValidationSeverity;
	...
	/** Nile docs URL. */
	docsUrl: string;
	/** Auto-fix descriptor, if automatic transformation is possible. */
	autoFix?: NileAutoFix;
}
/**
 * Nile table classification rules.
 *
 * These rules detect and validate how tables are classified in Nile's
 * multi-tenant architecture: tenant-aware tables (containing `tenant_id`)
 * versus shared tables (no `tenant_id`).
 *
 * @since 1.1.0
 */
export declare const NILE_TC_RULES: Record<string, NileErrorDef>;
/**
 * Nile primary key rules.
 *
 * Tenant-aware tables in Nile require composite primary keys that include
 * `tenant_id` for proper tenant isolation and data distribution.
 *
 * @since 1.1.0
 */
export declare const NILE_PK_RULES: Record<string, NileErrorDef>;
/**
 * Nile foreign key rules.
 *
 * Foreign keys in Nile have specific restrictions based on the
 * tenant/shared classification of the source and target tables.
 * Cross-classification references (tenant-to-shared, shared-to-tenant)
 * are not supported.
 *
 * @since 1.1.0
 */
export declare const NILE_FK_RULES: Record<string, NileErrorDef>;
/**
 * Nile column type rules.
 *
 * Nile supports all PostgreSQL 15 column types, with the exception of
 * sequence-based types on tenant-aware tables. Sequence-based columns
 * (SERIAL, BIGSERIAL, SMALLSERIAL, IDENTITY) are not available for
 * tenant tables because sequences cannot be tenant-isolated.
 *
 * @since 1.1.0
 */
export declare const NILE_CT_RULES: Record<string, NileErrorDef>;
/**
 * Nile trigger and function rules.
 *
 * Nile does not support user-defined triggers, functions, stored
 * procedures, anonymous code blocks, or PL/pgSQL. All procedural
 * logic must be moved to application code.
 *
 * @since 1.1.0
 */
export declare const NILE_TF_RULES: Record<string, NileErrorDef>;
/**
 * Nile sequence rules.
 *
 * Sequences in Nile are only available for shared tables. Tenant-aware
 * tables cannot use sequences because sequences cannot be isolated
 * per tenant.
 *
 * @since 1.1.0
 */
export declare const NILE_SEQ_RULES: Record<string, NileErrorDef>;
/**
 * Nile extension rules.
 *
 * Extensions in Nile are pre-installed and do not require (or allow)
 * explicit CREATE EXTENSION or DROP EXTENSION statements.
 *
 * @since 1.1.0
 */
export declare const NILE_EXT_RULES: Record<string, NileErrorDef>;
/**
 * Nile built-in table rules.
 *
 * Nile provides built-in tables (`tenants`, `users`, etc.) that are
 * automatically created. These tables cannot be created or dropped,
 * and only user-added extension columns can be modified.
 *
 * @since 1.1.0
 */
export declare const NILE_BT_RULES: Record<string, NileErrorDef>;
/**
 * Nile transaction rules.
 *
 * Nile enforces tenant isolation at the transaction level. Writes
 * to tenant tables must occur within a single-tenant context, and
 * mixing tenant and shared table writes in the same transaction is
 * not supported.
 *
 * @since 1.1.0
 */
export declare const NILE_TX_RULES: Record<string, NileErrorDef>;
/**
 * Nile admin and permission rules.
 *
 * Database administration operations (user/role management, database
 * creation, permission grants, and RLS policies) are managed through
 * the Nile Console or API, not via SQL statements.
 *
 * @since 1.1.0
 */
export declare const NILE_ADM_RULES: Record<string, NileErrorDef>;
/**
 * Nile constraint rules.
 *
 * UNIQUE and EXCLUSION constraints on tenant-aware tables must include
 * `tenant_id` to ensure proper tenant-scoped enforcement. Without
 * `tenant_id`, uniqueness or exclusion is enforced across all tenants,
 * which breaks tenant isolation.
 *
 * @since 1.1.0
 */
export declare const NILE_CON_RULES: Record<string, NileErrorDef>;
/**
 * All Nile validation rules combined into a single lookup table.
 *
 * Provides O(1) lookup by rule code across all eleven categories:
 * table classification, primary key, foreign key, column type,
 * trigger/function, sequence, extension, built-in table,
 * transaction, admin, and constraint rules.
 *
 * @example
 * ```typescript
 * const rule = NILE_ALL_RULES['NILE-PK-001'];
 * if (rule) {
 *     console.log(rule.severity); // 'error'
 *     console.log(rule.message);  // 'Table PRIMARY KEY does not include tenant_id...'
 * }
 * ```
 *
 * @since 1.1.0
 */
export declare const NILE_ALL_RULES: Record<string, NileErrorDef>;
/**
 * Create an aggregated validation result from a list of messages.
 *
 * Partitions messages into `errors`, `warnings`, and `info` arrays.
 * The result is `valid` when there are zero error-severity messages.
 *
 * @param messages - Array of validation messages to aggregate
 * @returns Aggregated validation result with categorized messages
 *
 * @example
 * ```typescript
 * const messages: NileValidationMessage[] = [
 *     { code: 'NILE-TC-001', severity: 'info', message: 'Tenant table detected...' },
 *     { code: 'NILE-PK-001', severity: 'error', message: 'PRIMARY KEY missing tenant_id...' },
 * ];
 * const result = createNileValidationResult(messages);
 * console.log(result.valid);          // false
 * console.log(result.errors.length);  // 1
 * console.log(result.info.length);    // 1
 * ```
 *
 * @since 1.1.0
 */
export declare function createNileValidationResult(messages: NileValidationMessage[]): NileValidationResult;
/**
 * Look up a Nile rule definition by code.
 *
 * @param code - The rule code to look up (e.g. 'NILE-PK-001')
 * @returns The rule definition, or `undefined` if the code is not recognized
 *
 * @example
 * ```typescript
 * const rule = lookupNileRule('NILE-FK-003');
 * if (rule) {
 *     console.log(rule.severity); // 'error'
 *     console.log(rule.category); // 'foreign-key'
 * }
 * ```
 *
 * @since 1.1.0
 */
export declare function lookupNileRule(code: string): NileErrorDef | undefined;
/**
 * Create a validation message from a rule code with optional location context.
 *
 * Looks up the rule definition from {@link NILE_ALL_RULES} and merges it
 * with the provided location information. If the code is not recognized,
 * returns a fallback error message.
 *
 * @param code - The rule code (e.g. 'NILE-PK-001')
 * @param location - Optional context about where the issue was found
 * @param location.tableName - The table where the issue was detected
 * @param location.columnName - The column where the issue was detected
 * @param location.constraintName - The constraint where the issue was detected
 * @param location.functionName - The function where the issue was detected
 * @returns A fully populated validation message
 *
 * @example
 * ```typescript
 * const msg = createNileMessage('NILE-PK-001', {
 *     tableName: 'orders',
 *     columnName: 'id',
 * });
 * console.log(msg.code);      // 'NILE-PK-001'
 * console.log(msg.tableName); // 'orders'
 * ```
 *
 * @since 1.1.0
 */
export declare function createNileMessage(code: string, location?: {
	tableName?: string;
	columnName?: string;
	constraintName?: string;
	functionName?: string;
}): NileValidationMessage;
/**
 * Format a single Nile validation message for display.
 *
 * Produces a human-readable, multi-line string with the severity prefix,
 * rule code, message, location context, recommended alternative, and
 * documentation URL.
 *
 ...
 * @example
 * ```typescript
 * const msg = createNileMessage('NILE-FK-001', { tableName: 'orders' });
 * console.log(formatNileMessage(msg));
 * // [ERROR] NILE-FK-001: Foreign key from tenant table to shared table is not supported in Nile
 * //   Location: orders
 * //   Alternative: Remove the FK constraint and enforce referential integrity in application code
 * //   Docs: https://www.thenile.dev/docs/postgres/postgres-compatibility
 * ```
 *
 * @since 1.1.0
 */
export declare function formatNileMessage(msg: NileValidationMessage): string;
/**
 * Format a complete Nile validation result for display.
 *
 * Produces a structured, human-readable report of all errors, warnings,
 * and informational messages. If the schema is fully compatible and has
 * no messages of any severity, outputs a success message.
 *
 * @param result - The aggregated validation result to format
 * @returns A formatted string suitable for console output
 *
 * @example
 * ```typescript
 * const messages = [
 *     createNileMessage('NILE-TC-001', { tableName: 'orders' }),
 *     createNileMessage('NILE-PK-001', { tableName: 'orders' }),
 * ];
 * const result = createNileValidationResult(messages);
 * console.log(formatNileValidationResult(result));
 * // === 1 Error(s) ===
 * // [ERROR] NILE-PK-001: Table PRIMARY KEY does not include tenant_id...
 * //
 * // === 1 Info(s) ===
 * // [INFO] NILE-TC-001: Tenant table detected...
 * ```
 *
 * @since 1.1.0
 */
export declare function formatNileValidationResult(result: NileValidationResult): string;
/**
 * Nile Database Platform Limits and Constants
 *
 * Built-in tables, reserved columns, pre-installed extensions,
 * tenant isolation requirements, blocked features, sequence
 * restrictions, and supported index methods for Nile's
 * multi-tenant PostgreSQL platform.
 *
 * @module nile-schema/limits
 *
 * @since 1.1.0
 */
/**
 * Tables that Nile creates and manages automatically in every database.
 * These tables must not be dropped, renamed, or have their schemas altered
 * outside of the Nile platform.
 *
 * @since 1.1.0
 */
export declare const NILE_BUILTIN_TABLES: ReadonlySet<string>;
/**
 * Columns that Nile manages on each built-in table.
 * These columns are created automatically and must not be dropped or
 * have their types changed by user migrations.
 *
 * @since 1.1.0
 */
export declare const NILE_BUILTIN_COLUMNS: Readonly<Record<string, readonly string[]>>;
/**
 * Column names that Nile reserves for tenant isolation.
 * On tenant-aware tables the `tenant_id` column is system-managed
 * and must not be manually inserted or updated by application code.
 *
 * @since 1.1.0
 */
export declare const NILE_RESERVED_COLUMNS: ReadonlySet<string>;
/**
 * PostgreSQL extensions that are pre-installed on every Nile database.
 * These extensions are available without running `CREATE EXTENSION` and
 * cannot be dropped.
 *
 * @since 1.1.0
 */
export declare const NILE_PREINSTALLED_EXTENSIONS: ReadonlySet<string>;
/**
 * Requirements for the `tenant_id` column on tenant-aware tables.
 * Every tenant table must include a UUID `tenant_id` column that is
 * NOT NULL and participates in the primary key (ideally as the first
 * column in a composite key).
 *
 * @since 1.1.0
 */
export declare const NILE_TENANT_ID_REQUIREMENTS: {
	/** Column name required for tenant isolation. */
	readonly COLUMN_NAME: "tenant_id";
	/** PostgreSQL type of the tenant_id column. */
	readonly COLUMN_TYPE: "uuid";
	/** tenant_id must be NOT NULL. */
	readonly NULLABLE: false;
	/** tenant_id must be part of the primary key. */
	readonly MUST_BE_IN_PK: true;
	/** Recommended ordinal position of tenant_id in a composite PK (0-based). */
	readonly RECOMMENDED_PK_POSITION: 0;
};
/**
 * PostgreSQL features that Nile does not allow.
 * Attempting to use any of these features will result in an error
 * from the Nile platform.
 *
 * @since 1.1.0
 */
export declare const NILE_BLOCKED_FEATURES: ReadonlySet<string>;
/**
 * Restrictions on sequences, SERIAL, and IDENTITY columns in Nile.
 * Sequences and auto-increment columns are only permitted on shared
 * (non-tenant) tables. Tenant-aware tables must not use SERIAL,
 * BIGSERIAL, or GENERATED AS IDENTITY columns because Nile cannot
 * guarantee cross-tenant uniqueness with local sequences.
 *
 * @since 1.1.0
 */
export declare const NILE_SEQUENCE_RESTRICTIONS: {
	/** Whether CREATE SEQUENCE is allowed on tenant-aware tables. */
	readonly ALLOWED_ON_TENANT_TABLES: false;
	/** Whether CREATE SEQUENCE is allowed on shared tables. */
	readonly ALLOWED_ON_SHARED_TABLES: true;
	/** Whether SERIAL / BIGSERIAL columns are allowed on tenant tables. */
	readonly SERIAL_ON_TENANT: false;
	/** Whether SERIAL / BIGSERIAL columns are allowed on shared tables. */
	readonly SERIAL_ON_SHARED: true;
	/** Whether GENERATED AS IDENTITY columns are allowed on tenant tables. */
	readonly IDENTITY_ON_TENANT: false;
	/** Whether GENERATED AS IDENTITY columns are allowed on shared tables. */
	readonly IDENTITY_ON_SHARED: true;
};
/**
 * Index access methods supported by Nile.
 * Unlike AWS DSQL (which only supports B-tree), Nile supports all
 * standard PostgreSQL index methods because it runs on PostgreSQL 15.
 *
 * @since 1.1.0
 */
export declare const NILE_SUPPORTED_INDEX_METHODS: ReadonlySet<string>;
/**
 * The PostgreSQL major version that Nile is built on.
 *
 * @since 1.1.0
 */
export declare const NILE_SUPPORTED_PG_VERSION: "15";
/**
 * Nile Type Compatibility Map
 *
 * Maps every PostgreSQL 15 column type to its Nile support status.
 * Nile supports approximately 95% of PostgreSQL 15 types natively.
 * The only context-dependent restrictions are sequence-based types
 * (SERIAL, BIGSERIAL, SMALLSERIAL) which are blocked on tenant tables
 * but supported on shared tables.
 *
 * @module nile-schema/type-mappings
 *
 * @remarks
 * Unlike AWS Aurora DSQL which blocks ~72% of PostgreSQL types, Nile
 * is nearly fully compatible with PostgreSQL 15. The sole restriction
 * is architectural: tenant tables cannot use sequence-backed types
 * because sequences cannot be reliably partitioned across tenant
 * boundaries. Shared tables have no such restriction.
 *
 * **Status values:**
 * | Status              | Meaning                                      |
 * |---------------------|----------------------------------------------|
 * | `supported`         | Works on all table types without restriction  |
 * | `context-dependent` | Depends on table type (tenant vs shared)      |
 * | `unsupported`       | Never works on Nile                           |
 *
 * @since 1.1.0
 */
/**
 * Nile compatibility status for a PostgreSQL column type.
 *
 * - `supported`: Fully works on all Nile table types (tenant and shared).
 * - `context-dependent`: Works on some table types but not others.
 *   Check `tenantTableStatus` and `sharedTableStatus` for specifics.
 * - `unsupported`: Blocked as a column type on Nile entirely.
 *
 * @since 1.1.0
 */
export type NileTypeStatus = "supported" | "context-dependent" | "unsupported";
/**
 * A single Nile type compatibility mapping.
 *
 * @remarks
 * When `status` is `context-dependent`, the `tenantTableStatus` and
 * `sharedTableStatus` fields indicate per-table-type support. When
 * `status` is `supported`, both table types allow the type unconditionally.
 *
 * @example
 * ```typescript
 * const mapping: NileTypeMapping = {
 *     pgType: 'SERIAL',
 *     status: 'context-dependent',
 *     errorCode: 'NILE-CT-001',
 *     alternative: 'uuid().default(DEFAULT.genRandomUuid())',
 *     category: 'numeric',
 *     note: 'Sequences cannot be partitioned across tenant boundaries.',
 ...
 * @since 1.1.0
 */
export interface NileTypeMapping {
	/** PostgreSQL type name (uppercase canonical). */
	pgType: string;
	/** Nile compatibility status. */
	status: NileTypeStatus;
	/** Nile error/warning code if not fully supported. */
	errorCode?: string;
	/** Recommended alternative for blocked or context-dependent types. */
	alternative?: string;
	/** Type category for grouping. */
	category: string;
	/** Additional notes about the type on Nile. */
	note?: string;
	/** Support status when the type is used on a tenant table. Only set for context-dependent types. */
	tenantTableStatus?: "supported" | "blocked";
	/** Support status when the type is used on a shared table. Only set for context-dependent types. */
	sharedTableStatus?: "supported" | "blocked";
}
/**
 * Complete mapping of PostgreSQL 15 types to Nile compatibility.
 *
 * @remarks
 * Nile supports approximately 95% of PostgreSQL 15 column types.
 * Every type category is fully supported except for sequence-backed
 * auto-increment types on tenant tables:
 *
 * **Fully supported categories:**
 * - Numeric (integer, bigint, real, double precision, numeric, decimal, money)
 ...
 * - Multirange (int4multirange, int8multirange, nummultirange, etc.)
 * - OID / REG* system types
 * - Internal (pg_lsn, pg_snapshot)
 *
 * **Context-dependent:**
 * - Serial types (serial, bigserial, smallserial) -- blocked on tenant tables
 *
 * @example
 * ```typescript
 * import { NILE_TYPE_MAP, getNileTypeMapping } from './type-mappings';
 *
 * // Check a fully supported type
 * const jsonb = NILE_TYPE_MAP['jsonb'];
 * console.log(jsonb.status); // 'supported'
 *
 * // Check a context-dependent type
 * const serial = NILE_TYPE_MAP['serial'];
 * console.log(serial.status);             // 'context-dependent'
 * console.log(serial.tenantTableStatus);  // 'blocked'
 * console.log(serial.sharedTableStatus);  // 'supported'
 * ```
 *
 * @since 1.1.0
 */
export declare const NILE_TYPE_MAP: Record<string, NileTypeMapping>;
/**
 * Look up a Nile type mapping by SQL type string.
 *
 * Case-insensitive, strips parameterized suffixes (e.g., `varchar(255)`)
 * and array brackets (e.g., `integer[]`).
 *
 * @param sqlType - The SQL type string to look up (e.g., `'serial'`, `'VARCHAR(255)'`, `'integer[]'`).
 * @returns The corresponding {@link NileTypeMapping}, or `undefined` if the type is not in the map.
 *
 * @example
 * ```typescript
 * import { getNileTypeMapping } from './type-mappings';
 *
 * const mapping = getNileTypeMapping('SERIAL');
 * // { pgType: 'SERIAL', status: 'context-dependent', ... }
 *
 * const jsonb = getNileTypeMapping('jsonb');
 * // { pgType: 'JSONB', status: 'supported', ... }
 *
 * const unknown = getNileTypeMapping('custom_type');
 * // undefined
 * ```
 *
 * @remarks
 ...
 * | Dialect     | Support  | Notes                     |
 * |-------------|----------|---------------------------|
 * | Nile        | ✅ Full   | Primary use case          |
 * | PostgreSQL  | N/A      | Use native PG validation  |
 * | CockroachDB | N/A      | Use CockroachDB validator |
 * | AWS DSQL    | N/A      | Use DSQL validator        |
 *
 * @since 1.1.0
 */
export declare function getNileTypeMapping(sqlType: string): NileTypeMapping | undefined;
/**
 * Check if a type is supported on Nile, optionally for a specific table type.
 *
 * When called without `tableType`, returns `true` for `supported` types and
 * `false` for `unsupported` types. For `context-dependent` types, returns
 * `true` (the type is usable in at least some context).
 *
 * When called with `tableType`, also checks the per-table-type status for
 * `context-dependent` types. A context-dependent type returns `false` when
 * the specified table type is `blocked`.
 *
 * Unknown types (not in the map) return `true`, assuming they are user-defined
 * enums or composite types.
 *
 * @param sqlType - The SQL type string to check (e.g., `'serial'`, `'jsonb'`).
 * @param tableType - Optional. The Nile table type: `'tenant'` or `'shared'`.
 *   When provided, context-dependent types are evaluated against this table type.
 * @returns `true` if the type is usable in the given context, `false` otherwise.
 *
 * @example
 * ```typescript
 * import { isNileTypeSupported } from './type-mappings';
 *
 * // Fully supported type -- always true
 * isNileTypeSupported('jsonb');                    // true
 * isNileTypeSupported('jsonb', 'tenant');          // true
 *
 * // Context-dependent type -- depends on table type
 * isNileTypeSupported('serial');                   // true (usable somewhere)
 * isNileTypeSupported('serial', 'tenant');         // false (blocked)
 * isNileTypeSupported('serial', 'shared');         // true (allowed)
 *
 * // Unknown type -- assumed user-defined
 * isNileTypeSupported('my_custom_enum');           // true
 * ```
 *
 * @remarks
 * **Dialect Support:**
 ...
 * @since 1.1.0
 */
export declare function isNileTypeSupported(sqlType: string, tableType?: "tenant" | "shared"): boolean;
/**
 * Get all context-dependent types that behave differently based on table type.
 *
 * Returns an array of {@link NileTypeMapping} entries where `status` is
 * `'context-dependent'`. Deduplicates by canonical `pgType` so each
 * PostgreSQL type appears only once (e.g., `serial` and `serial4` both
 * map to `SERIAL` but only one entry is returned).
 *
 * @returns Array of context-dependent type mappings, deduplicated by `pgType`.
 *
 * @example
 * ```typescript
 * import { getNileContextDependentTypes } from './type-mappings';
 *
 * const types = getNileContextDependentTypes();
 * // [
 * //   { pgType: 'SERIAL', status: 'context-dependent', tenantTableStatus: 'blocked', ... },
 * //   { pgType: 'BIGSERIAL', status: 'context-dependent', tenantTableStatus: 'blocked', ... },
 * //   { pgType: 'SMALLSERIAL', status: 'context-dependent', tenantTableStatus: 'blocked', ... },
 * // ]
 *
 * for (const t of types) {
 *     console.log(`${t.pgType}: tenant=${t.tenantTableStatus}, shared=${t.sharedTableStatus}`);
 * }
 * ```
 *
 * @remarks
 ...
 * @since 1.1.0
 */
export declare function getNileContextDependentTypes(): NileTypeMapping[];
/**
 * Nile table classification.
 *
 * - `tenant`: Table contains a `tenant_id` column and participates in
 *   Nile's automatic tenant isolation.
 * - `shared`: Table does not contain `tenant_id` and is visible across
 *   all tenants.
 * - `builtin`: Table is one of the platform-managed tables (`tenants`,
 *   `users`, `tenant_users`) that Nile creates automatically.
 *
 * @since 1.1.0
 */
export type NileTableType = "tenant" | "shared" | "builtin";
/**
 * Input for table classification.
 *
 * Provides the minimum information needed to determine whether a table
 * is tenant-aware, shared, or a Nile built-in table.
 *
 * @example
 * ```typescript
 * const input: NileTableInput = {
 *     name: 'orders',
 *     columns: ['id', 'tenant_id', 'total', 'created_at'],
 *     columnTypes: { id: 'uuid', tenant_id: 'uuid', total: 'numeric', created_at: 'timestamptz' },
 *     columnNullable: { id: false, tenant_id: false, total: false, created_at: false },
 * };
 * ```
 *
 * @since 1.1.0
 */
export interface NileTableInput {
	...
	/** Whether columns are nullable, keyed by column name. */
	columnNullable?: Record<string, boolean>;
}
/**
 * Classification result for a single table.
 *
 * Contains the table name, the determined {@link NileTableType}, and
 * metadata about the `tenant_id` column if present.
 *
 * @example
 * ```typescript
 * const result: NileTableClassification = {
 *     tableName: 'orders',
 *     tableType: 'tenant',
 *     hasTenantId: true,
 *     tenantIdIsUuid: true,
 *     tenantIdIsNotNull: true,
 * };
 * ```
 *
 * @since 1.1.0
 */
export interface NileTableClassification {
	/** Table name. */
	tableName: string;
	/** Classification result. */
	tableType: NileTableType;
	/** Whether tenant_id column is present. */
	hasTenantId: boolean;
	/** Whether tenant_id is UUID type (if present). */
	tenantIdIsUuid?: boolean;
	/** Whether tenant_id is NOT NULL (if present). */
	tenantIdIsNotNull?: boolean;
}
/**
 * Names of Nile built-in tables that the platform creates and manages
 * automatically. These tables must not be dropped, renamed, or have
 * their schemas altered by user migrations.
 *
 * @since 1.1.0
 */
export declare const NILE_BUILTIN_TABLE_NAMES: ReadonlySet<string>;
/**
 * Classify a single table as `tenant`, `shared`, or `builtin`.
 *
 * Classification rules (evaluated in order):
 * 1. If the table name matches a built-in table name, the type is `builtin`.
 * 2. If the table columns include `tenant_id`, the type is `tenant`.
 * 3. Otherwise, the type is `shared`.
 *
 * When the table has a `tenant_id` column and `columnTypes` / `columnNullable`
 * are provided, the result also includes metadata about whether the column
 * uses the correct UUID type and NOT NULL constraint.
 *
 * @param table - The table input to classify.
 * @returns The classification result including table type and tenant_id metadata.
 *
 * @example
 * ```typescript
 * import { classifyNileTable } from './table-classifier';
 *
 * // Built-in table
 * classifyNileTable({ name: 'tenants', columns: ['id', 'name'] });
 * // { tableName: 'tenants', tableType: 'builtin', hasTenantId: false }
 *
 * // Tenant-aware table
 * classifyNileTable({
 *     name: 'orders',
 *     columns: ['id', 'tenant_id', 'total'],
 *     columnTypes: { tenant_id: 'uuid' },
 *     columnNullable: { tenant_id: false },
 * });
 * // { tableName: 'orders', tableType: 'tenant', hasTenantId: true,
 * //   tenantIdIsUuid: true, tenantIdIsNotNull: true }
 *
 * // Shared table
 * classifyNileTable({ name: 'settings', columns: ['key', 'value'] });
 * // { tableName: 'settings', tableType: 'shared', hasTenantId: false }
 * ```
 *
 * @remarks
 * **Dialect Support:**
 ...
 * |-------------|----------|--------------------------------|
 * | Nile        | ✅ Full   | Primary use case               |
 * | PostgreSQL  | N/A      | No multi-tenant classification |
 * | CockroachDB | N/A      | No multi-tenant classification |
 * | AWS DSQL    | N/A      | No multi-tenant classification |
 *
 * @since 1.1.0
 */
export declare function classifyNileTable(table: NileTableInput): NileTableClassification;
/**
 * Classify multiple tables and return the results as a Map keyed by table name.
 *
 * Iterates over each table input, applies {@link classifyNileTable}, and
 * collects results into a `Map<string, NileTableClassification>` where the
 * key is the original table name from the input.
 *
 * @param tables - Array of table inputs to classify.
 * @returns A Map from table name to classification result.
 *
 * @example
 * ```typescript
 * import { classifyNileTables } from './table-classifier';
 *
 * const results = classifyNileTables([
 *     { name: 'tenants', columns: ['id', 'name'] },
 *     { name: 'orders', columns: ['id', 'tenant_id', 'total'] },
 *     { name: 'settings', columns: ['key', 'value'] },
 * ]);
 *
 * results.get('tenants')?.tableType;  // 'builtin'
 * results.get('orders')?.tableType;   // 'tenant'
 * results.get('settings')?.tableType; // 'shared'
 * ```
 *
 * @remarks
 * **Dialect Support:**
 ...
 * @since 1.1.0
 */
export declare function classifyNileTables(tables: NileTableInput[]): Map<string, NileTableClassification>;
/**
 * Check if a table name is a Nile built-in table.
 *
 * Performs a case-insensitive comparison against the known built-in
 * table names: `tenants`, `users`, `tenant_users`.
 *
 * @param tableName - The table name to check.
 * @returns `true` if the table name matches a built-in table (case-insensitive).
 *
 * @example
 * ```typescript
 * import { isNileBuiltinTable } from './table-classifier';
 *
 * isNileBuiltinTable('tenants');       // true
 * isNileBuiltinTable('TENANTS');       // true
 * isNileBuiltinTable('Tenant_Users');  // true
 * isNileBuiltinTable('orders');        // false
 * ```
 *
 * @remarks
 * **Dialect Support:**
 ...
 * |-------------|----------|--------------------------------|
 * | Nile        | ✅ Full   | Primary use case               |
 * | PostgreSQL  | N/A      | No built-in tenant tables      |
 * | CockroachDB | N/A      | No built-in tenant tables      |
 * | AWS DSQL    | N/A      | No built-in tenant tables      |
 *
 * @since 1.1.0
 */
export declare function isNileBuiltinTable(tableName: string): boolean;
/**
 * Check if a table has a `tenant_id` column, making it tenant-aware.
 *
 * Performs a case-insensitive search for a column named `tenant_id`
 * in the table's column list.
 *
 * @param table - The table input to check.
 * @returns `true` if the table has a `tenant_id` column (case-insensitive).
 *
 * @example
 * ```typescript
 * import { isNileTenantTable } from './table-classifier';
 *
 * isNileTenantTable({ name: 'orders', columns: ['id', 'tenant_id', 'total'] });
 * // true
 *
 * isNileTenantTable({ name: 'orders', columns: ['id', 'TENANT_ID', 'total'] });
 * // true
 *
 * isNileTenantTable({ name: 'settings', columns: ['key', 'value'] });
 * // false
 * ```
 *
 * @remarks
 ...
 * |-------------|----------|--------------------------------|
 * | Nile        | ✅ Full   | Primary use case               |
 * | PostgreSQL  | N/A      | No tenant isolation concept    |
 * | CockroachDB | N/A      | No tenant isolation concept    |
 * | AWS DSQL    | N/A      | No tenant isolation concept    |
 *
 * @since 1.1.0
 */
export declare function isNileTenantTable(table: NileTableInput): boolean;
/**
 * Validate a table's classification and tenant_id configuration.
 *
 * Returns validation messages for the following rules:
 *
 * - **NILE-TC-001** (info): Table detected as tenant-aware (has `tenant_id`).
 * - **NILE-TC-002** (error): `tenant_id` column is not UUID type.
 * - **NILE-TC-003** (error): `tenant_id` column is nullable.
 *
 * Only tenant-aware tables produce messages. Built-in and shared tables
 * return an empty array.
 *
 * @param table - The table input to validate.
 * @returns Array of validation messages. Empty if the table is shared, builtin,
 *   or a correctly configured tenant table without type/nullable metadata.
 *
 * @example
 * ```typescript
 * import { validateNileTableClassification } from './table-classifier';
 *
 * // Correctly configured tenant table
 * validateNileTableClassification({
 *     name: 'orders',
 *     columns: ['id', 'tenant_id', 'total'],
 *     columnTypes: { tenant_id: 'uuid' },
 *     columnNullable: { tenant_id: false },
 * });
 * // [{ code: 'NILE-TC-001', severity: 'info', ... }]
 *
 * // tenant_id with wrong type
 * validateNileTableClassification({
 *     name: 'orders',
 *     columns: ['id', 'tenant_id', 'total'],
 *     columnTypes: { tenant_id: 'integer' },
 * });
 * // [
 * //   { code: 'NILE-TC-001', severity: 'info', ... },
 * //   { code: 'NILE-TC-002', severity: 'error', ... },
 * // ]
 *
 ...
 *     name: 'orders',
 *     columns: ['id', 'tenant_id', 'total'],
 *     columnNullable: { tenant_id: true },
 * });
 * // [
 * //   { code: 'NILE-TC-001', severity: 'info', ... },
 * //   { code: 'NILE-TC-003', severity: 'error', ... },
 * // ]
 *
 * // Shared table -- no messages
 * validateNileTableClassification({
 *     name: 'settings',
 *     columns: ['key', 'value'],
 * });
 ...
 * |-------------|----------|--------------------------------|
 * | Nile        | ✅ Full   | Primary use case               |
 * | PostgreSQL  | N/A      | No multi-tenant validation     |
 * | CockroachDB | N/A      | No multi-tenant validation     |
 * | AWS DSQL    | N/A      | No multi-tenant validation     |
 *
 * @since 1.1.0
 */
export declare function validateNileTableClassification(table: NileTableInput): NileValidationMessage[];
/**
 * Input for primary key validation.
 *
 * Describes a single table's primary key configuration so the validator
 * can check Nile multi-tenant PK requirements.
 *
 * @example
 * ```typescript
 * const input: NilePkInput = {
 *     tableName: 'orders',
 ...
 * @since 1.1.0
 */
export interface NilePkInput {
	/** Table name. */
	tableName: string;
	/** Column names in the primary key (ordered). Empty array means no PK. */
	pkColumns: string[];
	/** Whether the table has a tenant_id column. */
	hasTenantId: boolean;
	/** Table type classification. */
	tableType: "tenant" | "shared" | "builtin";
}
/**
 * Validate primary key constraints for a single Nile table.
 *
 * Only tenant tables are validated. Shared and built-in tables are
 * skipped and return an empty array. For tenant tables the following
 * checks are performed in order:
 *
 * 1. **NILE-PK-004** (error) -- No primary key defined (`pkColumns` is empty).
 * 2. **NILE-PK-003** (error) -- Single-column PK that is not `tenant_id`.
 * 3. **NILE-PK-001** (error) -- PK does not include `tenant_id` at all.
 * 4. **NILE-PK-002** (warning) -- `tenant_id` is present but not the first column.
 *
 * @param input - The primary key configuration for a single table.
 * @returns An array of {@link NileValidationMessage} objects. Empty if valid
 *   or if the table is not a tenant table.
 *
 * @example
 * ```typescript
 * import { validateNilePrimaryKey } from './pk-validator';
 *
 * // Valid tenant table -- no messages
 * const msgs1 = validateNilePrimaryKey({
 *     tableName: 'orders',
 *     pkColumns: ['tenant_id', 'id'],
 *     hasTenantId: true,
 *     tableType: 'tenant',
 * });
 * console.log(msgs1.length); // 0
 *
 * // Tenant table missing tenant_id in PK
 * const msgs2 = validateNilePrimaryKey({
 *     tableName: 'orders',
 *     pkColumns: ['id'],
 *     hasTenantId: true,
 *     tableType: 'tenant',
 * });
 * // [{ code: 'NILE-PK-003', severity: 'error', ... }]
 *
 * // Shared table -- always skipped
 * const msgs3 = validateNilePrimaryKey({
 *     tableName: 'config',
 *     pkColumns: ['id'],
 *     hasTenantId: false,
 *     tableType: 'shared',
 * });
 * console.log(msgs3.length); // 0
 * ```
 *
 * @remarks
 * **Dialect Support:**
 ...
 * |-------------|----------|------------------------------------|
 * | Nile        | ✅ Full   | Primary use case                   |
 * | PostgreSQL  | N/A      | No tenant_id requirement           |
 * | CockroachDB | N/A      | Use CockroachDB validator          |
 * | AWS DSQL    | N/A      | Use DSQL validator                 |
 * | SQLite      | N/A      | Not applicable                     |
 * | MySQL       | N/A      | Not applicable                     |
 *
 * @since 1.1.0
 */
export declare function validateNilePrimaryKey(input: NilePkInput): NileValidationMessage[];
/**
 * Validate primary key constraints for multiple Nile tables.
 *
 * Iterates over every input and collects all validation messages
 * produced by {@link validateNilePrimaryKey}. Shared and built-in
 * tables are automatically skipped.
 *
 * @param inputs - An array of {@link NilePkInput} objects, one per table.
 * @returns A flat array of {@link NileValidationMessage} objects from all tables.
 *
 * @example
 * ```typescript
 * import { validateNilePrimaryKeys } from './pk-validator';
 *
 * const messages = validateNilePrimaryKeys([
 *     { tableName: 'orders',   pkColumns: ['tenant_id', 'id'], hasTenantId: true,  tableType: 'tenant' },
 *     { tableName: 'products', pkColumns: ['id'],              hasTenantId: true,  tableType: 'tenant' },
 *     { tableName: 'config',   pkColumns: ['key'],             hasTenantId: false, tableType: 'shared' },
 * ]);
 * // messages contains 1 error for 'products' (NILE-PK-003)
 * ```
 *
 * @remarks
 * **Dialect Support:**
 ...
 * @since 1.1.0
 */
export declare function validateNilePrimaryKeys(inputs: NilePkInput[]): NileValidationMessage[];
/**
 * Input for FK validation.
 *
 * Describes a single foreign key constraint between two tables in a Nile
 * database, including both table classifications (tenant, shared, or builtin)
 * and the columns involved on each side.
 *
 * @example
 * ```typescript
 * const fkInput: NileFkInput = {
 *     constraintName: 'fk_orders_users',
 *     sourceTable: 'orders',
 *     sourceTableType: 'tenant',
 *     sourceColumns: ['tenant_id', 'user_id'],
 *     targetTable: 'users',
 *     targetTableType: 'tenant',
 ...
 * @since 1.1.0
 */
export interface NileFkInput {
	/** Optional constraint name. */
	constraintName?: string;
	/** Source table name. */
	sourceTable: string;
	/** Source table type. */
	sourceTableType: "tenant" | "shared" | "builtin";
	/** Source column names in the FK. */
	sourceColumns: string[];
	/** Target table name. */
	targetTable: string;
	/** Target table type. */
	targetTableType: "tenant" | "shared" | "builtin";
	/** Target column names referenced. */
...
/**
 * Validate a single foreign key constraint against Nile multi-tenant rules.
 *
 * Applies the following rules in order:
 *
 * 1. **NILE-FK-001** (error): A tenant table cannot reference a shared table.
 * 2. **NILE-FK-002** (error): A shared table cannot reference a tenant table.
 * 3. **NILE-FK-003** (error): When both tables are tenant tables, the FK
 *    must include `tenant_id` on both the source and target column lists.
 * 4. **NILE-FK-004** (warning): When both tables are tenant tables and the
 *    FK uses CASCADE or SET NULL actions, a warning is emitted because the
 *    cascade is tenant-scoped.
 *
 * Shared-to-shared FKs produce no messages. FKs involving built-in tables
 * are skipped entirely (Nile manages them internally).
 *
 * @param input - The foreign key definition to validate
 * @returns An array of validation messages. Empty if the FK is valid.
 *
 * @example
 * ```typescript
 * import { validateNileForeignKey, type NileFkInput } from './fk-validator';
 *
 * // Valid: shared-to-shared FK (no messages)
 * const sharedFk: NileFkInput = {
 *     sourceTable: 'config',
 *     sourceTableType: 'shared',
 *     sourceColumns: ['category_id'],
 *     targetTable: 'categories',
 *     targetTableType: 'shared',
 *     targetColumns: ['id'],
 * };
 * const result1 = validateNileForeignKey(sharedFk);
 * console.log(result1.length); // 0
 *
 * // Error: tenant-to-shared FK (blocked)
 * const crossFk: NileFkInput = {
 *     sourceTable: 'orders',
 *     sourceTableType: 'tenant',
 *     sourceColumns: ['category_id'],
 *     targetTable: 'categories',
 ...
 *     targetColumns: ['id'],
 * };
 * const result2 = validateNileForeignKey(crossFk);
 * console.log(result2[0].code); // 'NILE-FK-001'
 *
 * // Error: tenant-to-tenant FK missing tenant_id
 * const missingTenantId: NileFkInput = {
 ...
 *     targetColumns: ['id'],
 * };
 * const result3 = validateNileForeignKey(missingTenantId);
 * console.log(result3[0].code); // 'NILE-FK-003'
 * ```
 *
 * @remarks
 * **Dialect Support:**
 ...
 * |-------------|----------|------------------------------------|
 * | Nile        | ✅ Full   | Primary use case                   |
 * | PostgreSQL  | N/A      | No multi-tenant FK restrictions    |
 * | CockroachDB | N/A      | Use CockroachDB validator          |
 * | AWS DSQL    | N/A      | Use DSQL validator                 |
 * | SQLite      | N/A      | Different FK semantics             |
 * | MySQL       | N/A      | Different FK semantics             |
 *
 * @since 1.1.0
 */
export declare function validateNileForeignKey(input: NileFkInput): NileValidationMessage[];
/**
 * Validate multiple foreign key constraints against Nile multi-tenant rules.
 *
 * Iterates over each FK input and collects all validation messages into a
 * single flat array. This is a convenience wrapper around
 * {@link validateNileForeignKey} for batch validation.
 *
 * @param inputs - Array of foreign key definitions to validate
 * @returns Combined array of validation messages from all FK inputs.
 *   Empty if all FKs are valid.
 *
 * @example
 * ```typescript
 * import { validateNileForeignKeys, type NileFkInput } from './fk-validator';
 *
 * const fks: NileFkInput[] = [
 *     {
 *         constraintName: 'fk_orders_products',
 *         sourceTable: 'orders',
 *         sourceTableType: 'tenant',
 *         sourceColumns: ['tenant_id', 'product_id'],
 ...
 *     },
 *     {
 *         constraintName: 'fk_orders_categories',
 *         sourceTable: 'orders',
 *         sourceTableType: 'tenant',
 *         sourceColumns: ['category_id'],
 *         targetTable: 'categories',
 ...
 * ];
 *
 * const messages = validateNileForeignKeys(fks);
 * // messages[0].code === 'NILE-FK-004' (cascade warning for first FK)
 * // messages[1].code === 'NILE-FK-001' (tenant-to-shared error for second FK)
 * ```
 *
 * @remarks
 * **Dialect Support:**
 ...
 * @since 1.1.0
 */
export declare function validateNileForeignKeys(inputs: NileFkInput[]): NileValidationMessage[];
/**
 * Input for column type validation.
 *
 * Provides all the context needed to determine whether a column type
 * is compatible with Nile. The `tableType` field is critical because
 * sequence-based types are only blocked on tenant-aware tables.
 *
 * @example
 * ```typescript
 * const input: NileColumnTypeInput = {
 *     columnName: 'id',
 *     sqlType: 'SERIAL',
 *     tableName: 'orders',
 *     tableType: 'tenant',
 * };
 *
 * const messages = validateNileColumnType(input);
 * // [{ code: 'NILE-CT-001', severity: 'error', message: '...' }]
 * ```
 *
 * @since 1.1.0
 */
export interface NileColumnTypeInput {
	/** Column name. */
	columnName: string;
	/** SQL type string (e.g. 'SERIAL', 'UUID', 'JSONB'). */
	sqlType: string;
	/** Table name for context. */
	tableName: string;
	/** Table type (tenant/shared/builtin). */
	tableType: "tenant" | "shared" | "builtin";
	/** Whether column has GENERATED AS IDENTITY. */
	hasIdentity?: boolean;
	/** Default expression if any. */
	defaultExpression?: string;
}
/**
 * Serial-family type names (lowercase).
 *
 * These types create implicit sequences under the hood. On Nile,
 * sequences cannot be partitioned across tenant boundaries, so
 * serial-family types are blocked on tenant-aware tables but
 * permitted on shared tables.
 *
 * @example
 * ```typescript
 * import { NILE_SERIAL_TYPES } from './type-validator';
 *
 * NILE_SERIAL_TYPES.has('serial');      // true
 * NILE_SERIAL_TYPES.has('bigserial');   // true
 * NILE_SERIAL_TYPES.has('uuid');        // false
 * ```
 *
 * @since 1.1.0
 */
export declare const NILE_SERIAL_TYPES: ReadonlySet<string>;
/**
 * Column types provided by pre-installed extensions on Nile.
 *
 * These types are available without issuing CREATE EXTENSION because
 * Nile pre-installs the corresponding extensions. When a column uses
 * one of these types, NILE-CT-005 is emitted as an informational note.
 *
 * @example
 * ```typescript
 * import { NILE_EXTENSION_TYPES } from './type-validator';
 *
 * NILE_EXTENSION_TYPES.has('vector');    // true (pgvector)
 * NILE_EXTENSION_TYPES.has('geometry');  // true (PostGIS)
 * NILE_EXTENSION_TYPES.has('citext');    // true
 * NILE_EXTENSION_TYPES.has('uuid');      // false (built-in, not extension)
 * ```
 *
 * @since 1.1.0
 */
export declare const NILE_EXTENSION_TYPES: ReadonlySet<string>;
/**
 * Validate a single column type for Nile compatibility.
 *
 * Checks the column definition against Nile's type restrictions.
 * The only restrictions are on tenant-aware tables:
 *
 * - **NILE-CT-001** (error): SERIAL/BIGSERIAL/SMALLSERIAL on a tenant table.
 * - **NILE-CT-002** (error): GENERATED AS IDENTITY on a tenant table.
 * - **NILE-CT-003** (error): DEFAULT nextval() on a tenant table.
 * - **NILE-CT-004** (info): All PG15 types supported (positive confirmation).
 * - **NILE-CT-005** (info): Extension types available without CREATE EXTENSION.
 * - **NILE-SEQ-003** (info): SERIAL on a shared table is supported.
 *
 * For shared and builtin tables, all PostgreSQL 15 types are supported
 * without restriction.
 *
 * @param input - The column definition to validate
 * @returns Array of validation messages (empty if fully compatible)
 *
 * @example
 * ```typescript
 * import { validateNileColumnType } from './type-validator';
 *
 * // SERIAL on a tenant table -- error
 * const msgs = validateNileColumnType({
 *     columnName: 'id',
 *     sqlType: 'SERIAL',
 *     tableName: 'orders',
 *     tableType: 'tenant',
 * });
 * // [{ code: 'NILE-CT-001', severity: 'error', ... }]
 *
 * // SERIAL on a shared table -- info
 * const msgs2 = validateNileColumnType({
 *     columnName: 'id',
 *     sqlType: 'SERIAL',
 *     tableName: 'config',
 *     tableType: 'shared',
 * });
 * // [{ code: 'NILE-SEQ-003', severity: 'info', ... }]
 *
 * // UUID on any table -- no messages
 * const msgs3 = validateNileColumnType({
 *     columnName: 'id',
 *     sqlType: 'UUID',
 *     tableName: 'orders',
 *     tableType: 'tenant',
 ...
 * |-------------|----------|----------------------------------------|
 * | Nile        | ✅ Full   | Primary use case                       |
 * | PostgreSQL  | N/A      | No sequence restrictions on any table  |
 * | CockroachDB | N/A      | Use CockroachDB type-validator         |
 * | AWS DSQL    | N/A      | Use DSQL type-validator                |
 *
 * @since 1.1.0
 */
export declare function validateNileColumnType(input: NileColumnTypeInput): NileValidationMessage[];
/**
 * Validate multiple column types for Nile compatibility.
 *
 * Convenience wrapper that calls {@link validateNileColumnType} for
 * each input and concatenates all resulting messages.
 *
 * @param inputs - Array of column definitions to validate
 * @returns All validation messages across all columns
 *
 * @example
 * ```typescript
 * import { validateNileColumnTypes } from './type-validator';
 *
 * const messages = validateNileColumnTypes([
 *     { columnName: 'id', sqlType: 'SERIAL', tableName: 'orders', tableType: 'tenant' },
 *     { columnName: 'name', sqlType: 'TEXT', tableName: 'orders', tableType: 'tenant' },
 *     { columnName: 'id', sqlType: 'BIGSERIAL', tableName: 'config', tableType: 'shared' },
 * ]);
 * // Returns: NILE-CT-001 for orders.id, NILE-SEQ-003 for config.id, nothing for orders.name
 * ```
 *
 * @remarks
 * **Dialect Support:**
 ...
 * @since 1.1.0
 */
export declare function validateNileColumnTypes(inputs: NileColumnTypeInput[]): NileValidationMessage[];
/**
 * Quick check: is a column type blocked for the given table type?
 *
 * Returns `true` only if the SQL type is a serial-family type and
 * the table type is `'tenant'`. All other combinations return `false`.
 *
 * This is a fast, boolean-only check. For detailed validation messages,
 * use {@link validateNileColumnType} instead.
 *
 * @param sqlType - The SQL type string (e.g. 'SERIAL', 'bigserial')
 * @param tableType - The Nile table classification
 * @returns `true` if the type is blocked for that table type, `false` otherwise
 *
 * @example
 * ```typescript
 * import { isNileColumnTypeBlocked } from './type-validator';
 *
 * isNileColumnTypeBlocked('SERIAL', 'tenant');  // true
 * isNileColumnTypeBlocked('SERIAL', 'shared');  // false
 * isNileColumnTypeBlocked('UUID', 'tenant');    // false
 * isNileColumnTypeBlocked('JSONB', 'tenant');   // false
 * isNileColumnTypeBlocked('bigserial', 'tenant'); // true
 * isNileColumnTypeBlocked('serial2', 'builtin');  // false
 * ```
 *
 * @remarks
 * **Dialect Support:**
 ...
 * |-------------|----------|----------------------------------------|
 * | Nile        | ✅ Full   | Primary use case                       |
 * | PostgreSQL  | N/A      | No type is blocked                     |
 * | CockroachDB | N/A      | Use CockroachDB type-validator         |
 * | AWS DSQL    | N/A      | Use DSQL type-validator                |
 *
 * @since 1.1.0
 */
export declare function isNileColumnTypeBlocked(sqlType: string, tableType: "tenant" | "shared" | "builtin"): boolean;
/**
 * Get the recommended alternative for a blocked column type.
 *
 * Returns the Relq ORM alternative expression for serial-family types.
 * Returns `undefined` for types that are not blocked on Nile (i.e.,
 * types that do not need an alternative).
 *
 * @param sqlType - The SQL type string (e.g. 'SERIAL', 'bigserial')
 * @returns The alternative expression string, or `undefined` if the type is not blocked
 *
 * @example
 * ```typescript
 * import { getNileTypeAlternative } from './type-validator';
 *
 * getNileTypeAlternative('SERIAL');
 * // 'uuid().default(DEFAULT.genRandomUuid())'
 *
 * getNileTypeAlternative('bigserial');
 * // 'uuid().default(DEFAULT.genRandomUuid())'
 *
 * getNileTypeAlternative('UUID');
 * // undefined
 *
 * getNileTypeAlternative('TEXT');
 * // undefined
 * ```
 *
 * @remarks
 ...
 * @since 1.1.0
 */
export declare function getNileTypeAlternative(sqlType: string): string | undefined;
/**
 * Input descriptor for a trigger, function, procedure, or anonymous code block
 * that should be validated against Nile restrictions.
 *
 * @example
 * ```typescript
 * // Trigger input
 * const input: NileFunctionInput = {
 *     name: 'set_updated_at',
 *     kind: 'trigger',
 *     tableName: 'orders',
 * };
 *
 * // Function input
 * const funcInput: NileFunctionInput = {
 ...
 *
 * // DO block input
 * const doInput: NileFunctionInput = {
 *     name: 'anonymous',
 *     kind: 'do_block',
 * };
 * ```
 ...
 * @since 1.1.0
 */
export interface NileFunctionInput {
	/** Name of the trigger, function, or procedure. For DO blocks, use a descriptive name. */
	name: string;
	/** Procedural language used (e.g. 'plpgsql', 'plpython3u'). Optional. */
	language?: string;
	/** Kind of procedural object being validated. */
	kind: "function" | "procedure" | "trigger" | "do_block";
	/** For triggers: the table the trigger is attached to. */
	tableName?: string;
}
/**
 * Set of all procedural object kinds that Nile blocks.
 *
 * Nile does not support any server-side procedural objects. All four
 * kinds -- functions, procedures, triggers, and anonymous DO blocks --
 * are rejected at the DDL level.
 *
 * @example
 * ```typescript
 * if (NILE_BLOCKED_FUNCTION_KINDS.has(input.kind)) {
 *     // This kind is not supported on Nile
 * }
 * ```
 *
 * @since 1.1.0
 */
export declare const NILE_BLOCKED_FUNCTION_KINDS: ReadonlySet<string>;
/**
 * Set of procedural languages that Nile blocks.
 *
 * Nile does not support any procedural language extension. PL/pgSQL,
 * PL/Python, PL/Perl, PL/Tcl, and C language functions are all
 * rejected. Only built-in SQL expressions and built-in PostgreSQL
 * functions are available.
 *
 * @example
 * ```typescript
 * const lang = input.language?.toLowerCase();
 * if (lang && NILE_BLOCKED_LANGUAGES.has(lang)) {
 *     // This language is not available on Nile
 * }
 * ```
 *
 * @since 1.1.0
 */
export declare const NILE_BLOCKED_LANGUAGES: ReadonlySet<string>;
/**
 * Validate a single trigger, function, procedure, or DO block against Nile restrictions.
 *
 * Nile does not support any form of server-side procedural code. This function
 * checks the `kind` field to emit the appropriate error code, and additionally
 * checks the `language` field for PL/pgSQL usage when no other rule has already
 * been triggered.
 *
 * @param input - The function/trigger/procedure descriptor to validate.
 * @returns An array of {@link NileValidationMessage} entries. Returns an empty
 *   array only if `input.kind` is not in the blocked set and `input.language`
 *   is not a blocked language (which in practice never happens, since all four
 *   kinds are blocked).
 *
 * @example
 * ```typescript
 * import { validateNileFunction } from './function-validator';
 *
 * // Trigger validation
 * const msgs = validateNileFunction({
 *     name: 'set_updated_at',
 *     kind: 'trigger',
 *     tableName: 'orders',
 * });
 * // [{ code: 'NILE-TF-001', severity: 'error', message: 'Trigger "set_updated_at" on table "orders" cannot be created on Nile. Triggers are not supported.' }]
 *
 * // Function with PL/pgSQL
 * const msgs2 = validateNileFunction({
 *     name: 'calc_total',
 *     kind: 'function',
 *     language: 'plpgsql',
 * });
 * // Two messages: NILE-TF-002 (function blocked) and NILE-TF-005 (plpgsql blocked)
 * ```
 *
 * @remarks
 ...
 * |-------------|-------------------------------|----------|
 * | NILE-TF-001 | kind === 'trigger'            | error    |
 * | NILE-TF-002 | kind === 'function'           | error    |
 * | NILE-TF-003 | kind === 'procedure'          | error    |
 * | NILE-TF-004 | kind === 'do_block'           | error    |
 * | NILE-TF-005 | language === 'plpgsql' (extra) | error   |
 *
 * NILE-TF-005 fires only when the language is 'plpgsql' (case-insensitive)
 * AND the input was not already caught by one of the kind-specific rules
 * that would make NILE-TF-005 redundant. In practice, because all four kinds
 * are blocked, NILE-TF-005 fires as an additional diagnostic when the language
 * is explicitly specified as PL/pgSQL on a function or procedure.
 *
 * **Dialect Support:**
 * | Dialect     | Support  | Notes                                  |
 * |-------------|----------|----------------------------------------|
 * | Nile        | N/A      | Target dialect -- all features blocked  |
 * | PostgreSQL  | N/A      | Not applicable                          |
 * | CockroachDB | N/A      | Not applicable                          |
 * | AWS DSQL    | N/A      | Not applicable                          |
 *
 * @since 1.1.0
 */
export declare function validateNileFunction(input: NileFunctionInput): NileValidationMessage[];
/**
 * Validate multiple triggers, functions, procedures, and DO blocks against Nile restrictions.
 *
 * Convenience wrapper that calls {@link validateNileFunction} for each input
 * and returns the concatenated results.
 *
 * @param inputs - Array of function/trigger/procedure descriptors to validate.
 * @returns Flat array of all validation messages across all inputs.
 *
 * @example
 * ```typescript
 * import { validateNileFunctions } from './function-validator';
 *
 * const msgs = validateNileFunctions([
 *     { name: 'set_updated_at', kind: 'trigger', tableName: 'orders' },
 *     { name: 'calc_total', kind: 'function', language: 'plpgsql' },
 *     { name: 'reset_seq', kind: 'do_block' },
 * ]);
 * // msgs contains errors for all three inputs
 * console.log(msgs.length); // 4 (trigger + function + plpgsql + do_block)
 * ```
 *
 * @remarks
 * **Dialect Support:**
 ...
 * @since 1.1.0
 */
export declare function validateNileFunctions(inputs: NileFunctionInput[]): NileValidationMessage[];
/**
 * Input descriptor for a PostgreSQL sequence to be validated against
 * Nile's multi-tenant restrictions.
 *
 * @example
 * ```typescript
 * // Sequence on a tenant table (will produce an error)
 * const input: NileSequenceInput = {
 ...
 * };
 *
 * // Sequence on a shared table (supported)
 * const sharedInput: NileSequenceInput = {
 *     sequenceName: 'config_id_seq',
 *     ownedByTable: 'app_config',
 *     ownedByColumn: 'id',
 *     tableType: 'shared',
 * };
 *
 * // Standalone sequence without table context
 * const standaloneInput: NileSequenceInput = {
 *     sequenceName: 'global_counter_seq',
 * };
 * ```
 *
 * @since 1.1.0
 */
export interface NileSequenceInput {
	/** Name of the PostgreSQL sequence. */
	sequenceName: string;
	/** Table that owns the sequence (via OWNED BY), if known. */
	ownedByTable?: string;
	/** Column that owns the sequence (via OWNED BY), if known. */
	ownedByColumn?: string;
	/** Nile table classification of the owning table. */
	tableType?: "tenant" | "shared" | "builtin";
}
/**
 * Validate a single sequence against Nile's multi-tenant restrictions.
 *
 * Sequences in Nile are only available for shared tables. This function
 * checks the table type of the owning table and emits the appropriate
 * error, warning, or informational message.
 *
 * When a sequence is associated with a tenant table, both NILE-SEQ-001
 * and NILE-SEQ-004 fire (the former for the general restriction, the
 * latter specifically for OWNED BY).
 *
 * @param input - The sequence descriptor to validate.
 * @returns An array of {@link NileValidationMessage} entries. May contain
 *   zero or more messages depending on the sequence's table context.
 *
 * @example
 * ```typescript
 * import { validateNileSequence } from './sequence-validator';
 *
 * // Tenant table -- error
 * const msgs = validateNileSequence({
 *     sequenceName: 'orders_id_seq',
 *     ownedByTable: 'orders',
 *     ownedByColumn: 'id',
 *     tableType: 'tenant',
 * });
 * // msgs[0].code === 'NILE-SEQ-001' (error)
 * // msgs[1].code === 'NILE-SEQ-004' (error, OWNED BY tenant column)
 *
 * // Shared table -- info
 * const msgs2 = validateNileSequence({
 *     sequenceName: 'config_id_seq',
 *     ownedByTable: 'app_config',
 *     ownedByColumn: 'id',
 *     tableType: 'shared',
 * });
 * // msgs2[0].code === 'NILE-SEQ-003' (info)
 *
 * // No table context -- warning
 * const msgs3 = validateNileSequence({
 *     sequenceName: 'global_counter_seq',
 * });
 * // msgs3[0].code === 'NILE-SEQ-002' (warning)
 * ```
 *
 * @remarks
 * **Validation rules applied:**
 * | Code         | Condition                              | Severity |
 * |--------------|----------------------------------------|----------|
 * | NILE-SEQ-001 | tableType === 'tenant'                 | error    |
 * | NILE-SEQ-002 | no tableType or no ownedByTable        | warning  |
 * | NILE-SEQ-003 | tableType === 'shared'                 | info     |
 * | NILE-SEQ-004 | ownedByTable && tableType === 'tenant' | error    |
 *
 * NILE-SEQ-004 fires in addition to NILE-SEQ-001 when the sequence is
 * explicitly owned by a tenant table column.
 *
 * **Dialect Support:**
 * | Dialect     | Support  | Notes                                    |
 * |-------------|----------|------------------------------------------|
 * | Nile        | N/A      | Target dialect -- restrictions validated  |
 * | PostgreSQL  | N/A      | Not applicable                           |
 * | CockroachDB | N/A      | Not applicable                           |
 * | AWS DSQL    | N/A      | Not applicable                           |
 *
 * @since 1.1.0
 */
export declare function validateNileSequence(input: NileSequenceInput): NileValidationMessage[];
/**
 * Validate multiple sequences against Nile's multi-tenant restrictions.
 *
 * Convenience wrapper that calls {@link validateNileSequence} for each input
 * and returns the concatenated results.
 *
 * @param inputs - Array of sequence descriptors to validate.
 * @returns Flat array of all validation messages across all inputs.
 *
 * @example
 * ```typescript
 * import { validateNileSequences } from './sequence-validator';
 *
 * const msgs = validateNileSequences([
 *     { sequenceName: 'orders_id_seq', ownedByTable: 'orders', ownedByColumn: 'id', tableType: 'tenant' },
 *     { sequenceName: 'config_id_seq', ownedByTable: 'app_config', ownedByColumn: 'id', tableType: 'shared' },
 * ]);
 * // msgs contains errors for orders_id_seq and info for config_id_seq
 * ```
 *
 * @remarks
 * **Dialect Support:**
 ...
 * @since 1.1.0
 */
export declare function validateNileSequences(inputs: NileSequenceInput[]): NileValidationMessage[];
/**
 * Validate a SQL default expression for sequence function usage on Nile.
 *
 * Checks whether the expression contains `nextval()`, `currval()`, or
 * `setval()` calls. These functions rely on sequences, which are not
 * available for tenant-aware tables in Nile. If the table type is
 * `'tenant'` and a sequence function is detected, a NILE-CT-003 error
 * is emitted.
 *
 * @param expression - The SQL default expression to check (e.g. `"nextval('orders_id_seq')"`)
 * @param tableType - The Nile table classification: `'tenant'`, `'shared'`, or `'builtin'`
 * @returns An array of {@link NileValidationMessage} entries. Empty if no
 *   sequence function is found or the table type is not `'tenant'`.
 *
 * @example
 * ```typescript
 * import { validateNileSequenceExpression } from './sequence-validator';
 *
 * // nextval on tenant table -- error
 * const msgs = validateNileSequenceExpression("nextval('orders_id_seq')", 'tenant');
 * // [{ code: 'NILE-CT-003', severity: 'error', ... }]
 *
 * // nextval on shared table -- no error
 * const msgs2 = validateNileSequenceExpression("nextval('config_id_seq')", 'shared');
 * // []
 *
 * // Non-sequence expression -- no error
 * const msgs3 = validateNileSequenceExpression("gen_random_uuid()", 'tenant');
 * // []
 * ```
 *
 * @remarks
 ...
 * | Code        | Condition                                     | Severity |
 * |-------------|-----------------------------------------------|----------|
 * | NILE-CT-003 | tenant table + nextval/currval/setval detected | error    |
 *
 * **Dialect Support:**
 * | Dialect     | Support  | Notes                                    |
 * |-------------|----------|------------------------------------------|
 ...
 * @since 1.1.0
 */
export declare function validateNileSequenceExpression(expression: string, tableType: "tenant" | "shared" | "builtin"): NileValidationMessage[];
/**
 * Input descriptor for validating operations on Nile built-in tables.
 *
 * Nile provides built-in tables (`tenants`, `users`, `tenant_users`) that
 * are automatically created and managed. These tables cannot be created or
 * dropped by user DDL, and only user-added extension columns can be modified.
 *
 * @example
 * ```typescript
 * // Attempting to create a built-in table
 * const input: NileBuiltinTableInput = {
 *     tableName: 'tenants',
 *     operation: 'create',
 * };
 *
 * // Attempting to alter a built-in column
 * const alterInput: NileBuiltinTableInput = {
 *     tableName: 'users',
 *     operation: 'alter',
 *     columnName: 'email',
 *     isBuiltinColumn: true,
 * };
 * ```
 *
 * @since 1.1.0
 */
export interface NileBuiltinTableInput {
	/** Name of the table being operated on. */
	tableName: string;
	/** DDL operation being attempted. */
	operation: "create" | "drop" | "alter";
	/** Column name being modified, if applicable. */
	columnName?: string;
	/** Whether the column is a Nile built-in column (as opposed to a user-added extension column). */
	isBuiltinColumn?: boolean;
}
/**
 * SQL patterns that match administrative and procedural commands blocked on Nile.
 *
 * Each entry defines a regular expression pattern, the associated Nile error code,
 * and a human-readable message. These are used by {@link validateNileMiscSql} to
 * scan raw SQL strings for incompatible statements.
 *
 * @remarks
 * Patterns cover:
 * - Trigger, function, procedure, and DO block creation (NILE-TF-*)
 * - PL/pgSQL language usage (NILE-TF-005)
 * - User/role/database creation (NILE-ADM-001, NILE-ADM-002)
 * - GRANT/REVOKE permission management (NILE-ADM-003)
 * - Row-Level Security policies and enablement (NILE-ADM-004)
 *
 * @example
 * ```typescript
 * for (const entry of NILE_BLOCKED_ADMIN_PATTERNS) {
 *     if (entry.pattern.test(sql)) {
 *         console.log(`Blocked: ${entry.code} - ${entry.message}`);
 *     }
 * }
 * ```
 *
 * @since 1.1.0
 */
export declare const NILE_BLOCKED_ADMIN_PATTERNS: ReadonlyArray<{
	pattern: RegExp;
	code: string;
	message: string;
}>;
/**
 * Set of all miscellaneous feature names that Nile blocks or restricts.
 *
 * Used for quick membership checks when categorizing schema elements
 * against Nile's blocked feature list.
 *
 * @example
 * ```typescript
 * if (NILE_BLOCKED_MISC_FEATURES.has('trigger')) {
 *     // Feature is blocked on Nile
 * }
 * ```
 *
 * @since 1.1.0
 */
export declare const NILE_BLOCKED_MISC_FEATURES: ReadonlySet<string>;
/**
 * Validate a DDL operation against a Nile built-in table.
 *
 * Nile provides built-in tables (`tenants`, `users`, `tenant_users`) that
 * are automatically managed. This function checks whether a CREATE, DROP,
 * or ALTER operation is valid for the given table and column.
 *
 * @param input - The built-in table operation descriptor to validate.
 * @returns An array of {@link NileValidationMessage} entries. Returns an
 *   empty array if the table name is not a recognized built-in table.
 *
 * @example
 * ```typescript
 * import { validateNileBuiltinTable } from './misc-validator';
 *
 * // CREATE on built-in table -- error
 * const msgs = validateNileBuiltinTable({
 *     tableName: 'tenants',
 *     operation: 'create',
 * });
 * // [{ code: 'NILE-BT-001', severity: 'error', ... }]
 *
 * // DROP on built-in table -- error
 * const msgs2 = validateNileBuiltinTable({
 *     tableName: 'users',
 *     operation: 'drop',
 * });
 * // [{ code: 'NILE-BT-002', severity: 'error', ... }]
 *
 * // ALTER built-in column -- warning
 * const msgs3 = validateNileBuiltinTable({
 *     tableName: 'users',
 *     operation: 'alter',
 *     columnName: 'email',
 *     isBuiltinColumn: true,
 * });
 * // [{ code: 'NILE-BT-003', severity: 'warning', ... }]
 *
 * // ALTER to drop tenant_id -- error
 * const msgs4 = validateNileBuiltinTable({
 *     tableName: 'tenant_users',
 *     operation: 'alter',
 *     columnName: 'tenant_id',
 *     isBuiltinColumn: true,
 * });
 * // [{ code: 'NILE-BT-003', ... }, { code: 'NILE-BT-004', ... }]
 * ```
 *
 * @remarks
 * **Validation rules applied:**
 * | Code        | Condition                                           | Severity |
 * |-------------|-----------------------------------------------------|----------|
 * | NILE-BT-001 | operation === 'create'                               | error    |
 * | NILE-BT-002 | operation === 'drop'                                 | error    |
 * | NILE-BT-003 | operation === 'alter' && isBuiltinColumn             | warning  |
 * | NILE-BT-004 | columnName === 'tenant_id' && alter involves column  | error    |
 *
 * **Dialect Support:**
 * | Dialect     | Support  | Notes                                  |
 * |-------------|----------|----------------------------------------|
 ...
 * @since 1.1.0
 */
export declare function validateNileBuiltinTable(input: NileBuiltinTableInput): NileValidationMessage[];
/**
 * Validate that a drop or rename operation on tenant_id is blocked.
 *
 * The `tenant_id` column is a system-managed column required for tenant
 * isolation in Nile. It cannot be dropped or renamed on any table.
 *
 * @param tableName - The table containing the tenant_id column.
 * @param operation - The operation being attempted: `'drop'` or `'rename'`.
 * @returns An array containing a single NILE-BT-004 error message.
 *
 * @example
 * ```typescript
 * import { validateNileTenantIdProtection } from './misc-validator';
 *
 * const msgs = validateNileTenantIdProtection('orders', 'drop');
 * // [{ code: 'NILE-BT-004', severity: 'error', message: 'Cannot drop tenant_id ...' }]
 *
 * const msgs2 = validateNileTenantIdProtection('orders', 'rename');
 * // [{ code: 'NILE-BT-004', severity: 'error', message: 'Cannot rename tenant_id ...' }]
 * ```
 *
 * @remarks
 * **Dialect Support:**
 ...
 * @since 1.1.0
 */
export declare function validateNileTenantIdProtection(tableName: string, operation: "drop" | "rename"): NileValidationMessage[];
/**
 * Validate SQL for CREATE EXTENSION and DROP EXTENSION statements on Nile.
 *
 * Extensions in Nile are pre-installed and cannot be created or dropped
 * via SQL. CREATE EXTENSION is silently skipped (informational), and
 * DROP EXTENSION produces a warning since pre-installed extensions cannot
 * be removed.
 *
 * @param sql - The raw SQL string to scan for extension statements.
 * @returns An array of {@link NileValidationMessage} entries for any
 *   detected extension management statements.
 *
 * @example
 * ```typescript
 * import { validateNileExtensions } from './misc-validator';
 *
 * // CREATE EXTENSION -- info (skip)
 * const msgs = validateNileExtensions("CREATE EXTENSION IF NOT EXISTS pgvector;");
 * // [{ code: 'NILE-EXT-001', severity: 'info', ... }]
 *
 * // DROP EXTENSION -- warning
 * const msgs2 = validateNileExtensions("DROP EXTENSION pg_trgm;");
 * // [{ code: 'NILE-EXT-002', severity: 'warning', ... }]
 *
 * // No extension statements -- empty
 * const msgs3 = validateNileExtensions("SELECT * FROM users;");
 * // []
 * ```
 *
 * @remarks
 ...
 * | Code         | Condition           | Severity |
 * |--------------|---------------------|----------|
 * | NILE-EXT-001 | CREATE EXTENSION    | info     |
 * | NILE-EXT-002 | DROP EXTENSION      | warning  |
 *
 * **Dialect Support:**
 * | Dialect     | Support  | Notes                                  |
 * |-------------|----------|----------------------------------------|
 ...
 * @since 1.1.0
 */
export declare function validateNileExtensions(sql: string): NileValidationMessage[];
/**
 * Validate a raw SQL string against all Nile miscellaneous restrictions.
 *
 * Scans the SQL for blocked administrative commands (triggers, functions,
 * procedures, DO blocks, PL/pgSQL, user/role/database creation, GRANT/REVOKE,
 * and RLS policies) as well as extension management statements. Results are
 * deduplicated by error code so the same code does not appear twice for the
 * same SQL input.
 *
 * @param sql - The raw SQL string to validate.
 * @returns An array of {@link NileValidationMessage} entries for all
 *   matched patterns, deduplicated by code.
 *
 * @example
 * ```typescript
 * import { validateNileMiscSql } from './misc-validator';
 *
 * const sql = `
 *     CREATE EXTENSION IF NOT EXISTS pgvector;
 *     CREATE FUNCTION calc() RETURNS void LANGUAGE plpgsql AS $$ BEGIN END; $$;
 *     GRANT SELECT ON users TO app_user;
 * `;
 * const msgs = validateNileMiscSql(sql);
 * // msgs contains:
 * //   NILE-EXT-001 (info, CREATE EXTENSION)
 * //   NILE-TF-002 (error, CREATE FUNCTION)
 * //   NILE-TF-005 (error, LANGUAGE plpgsql)
 * //   NILE-ADM-003 (error, GRANT)
 * // Each code appears at most once.
 * ```
 *
 * @remarks
 * **Deduplication:** If the same error code would fire multiple times
 * (e.g. two GRANT statements both matching NILE-ADM-003), only the first
 * occurrence is included in the result.
 *
 * **Dialect Support:**
 * | Dialect     | Support  | Notes                                  |
 * |-------------|----------|----------------------------------------|
 ...
 * @since 1.1.0
 */
export declare function validateNileMiscSql(sql: string): NileValidationMessage[];
/**
 * Validate transaction isolation concerns for Nile multi-tenant writes.
 *
 * Nile enforces strict transaction boundaries for tenant isolation:
 * - Writes to tenant tables must occur within a single-tenant context.
 * - Mixing tenant and shared table writes in the same transaction is not supported.
 * - DDL changes are distributed atomically across all tenants via pg_karnak.
 *
 * @param hasTenantTableWrites - Whether the transaction writes to tenant-aware tables.
 * @param hasSharedTableWrites - Whether the transaction writes to shared tables.
 * @returns An array of {@link NileValidationMessage} entries with warnings
 *   and informational notes about Nile transaction behavior.
 *
 * @example
 * ```typescript
 * import { validateNileTransactionConcerns } from './misc-validator';
 *
 * // Tenant writes only
 * const msgs = validateNileTransactionConcerns(true, false);
 * // [{ code: 'NILE-TX-001', severity: 'warning', ... },
 * //  { code: 'NILE-TX-003', severity: 'info', ... }]
 *
 * // Mixed writes -- both tenant and shared
 * const msgs2 = validateNileTransactionConcerns(true, true);
 * // [{ code: 'NILE-TX-001', ... },
 * //  { code: 'NILE-TX-002', ... },
 * //  { code: 'NILE-TX-003', ... }]
 *
 * // No writes
 * const msgs3 = validateNileTransactionConcerns(false, false);
 * // [{ code: 'NILE-TX-003', severity: 'info', ... }]
 * ```
 *
 * @remarks
 ...
 * | Code        | Condition                    | Severity |
 * |-------------|------------------------------|----------|
 * | NILE-TX-001 | hasTenantTableWrites         | warning  |
 * | NILE-TX-002 | both tenant and shared writes | warning  |
 * | NILE-TX-003 | always                       | info     |
 *
 * **Dialect Support:**
 * | Dialect     | Support  | Notes                                  |
 * |-------------|----------|----------------------------------------|
 ...
 * @since 1.1.0
 */
export declare function validateNileTransactionConcerns(hasTenantTableWrites: boolean, hasSharedTableWrites: boolean): NileValidationMessage[];
/**
 * Input descriptor for a constraint to be validated against Nile's
 * multi-tenant restrictions.
 *
 * @example
 * ```typescript
 * // UNIQUE constraint on a tenant table (must include tenant_id)
 * const input: NileConstraintInput = {
 ...
 * };
 *
 * // UNIQUE constraint that correctly includes tenant_id
 * const validInput: NileConstraintInput = {
 *     constraintName: 'uq_orders_tenant_order',
 *     constraintType: 'unique',
 *     tableName: 'orders',
 *     tableType: 'tenant',
 ...
 * };
 *
 * // EXCLUSION constraint on a tenant table
 * const exclusionInput: NileConstraintInput = {
 *     constraintName: 'excl_bookings_overlap',
 *     constraintType: 'exclusion',
 *     tableName: 'bookings',
 *     tableType: 'tenant',
 *     columns: ['room_id', 'booking_range'],
 * };
 *
 * // Constraint on a shared table (no restrictions)
 * const sharedInput: NileConstraintInput = {
 *     constraintName: 'uq_config_key',
 *     constraintType: 'unique',
 ...
 * @since 1.1.0
 */
export interface NileConstraintInput {
	/** Optional constraint name. */
	constraintName?: string;
	/** The constraint type. */
	constraintType: "unique" | "exclusion";
	/** Table that owns the constraint. */
	tableName: string;
	/** Nile table classification. */
	tableType: "tenant" | "shared" | "builtin";
	/** Column names included in the constraint (ordered). */
	columns: string[];
}
/**
 * Validate a single UNIQUE or EXCLUSION constraint against Nile's
 * multi-tenant rules.
 *
 * Applies the following rules:
 *
 * 1. **NILE-CON-001** (error): UNIQUE constraint on a tenant table does
 *    not include `tenant_id`. Uniqueness would be enforced across all
 *    tenants, breaking tenant isolation.
 * 2. **NILE-CON-002** (warning): UNIQUE constraint on a tenant table has
 *    `tenant_id` but not as the first column. Reordering improves index
 *    performance.
 * 3. **NILE-CON-003** (error): EXCLUSION constraint on a tenant table does
 *    not include `tenant_id`. The exclusion would span tenants.
 * 4. **NILE-CON-004** (info): Constraint on a shared table has no
 *    tenant-scoping requirements.
 *
 * Built-in table constraints are skipped (Nile manages them internally).
 *
 * @param input - The constraint definition to validate.
 * @returns An array of {@link NileValidationMessage} entries. Empty if
 *   the constraint is valid or the table is built-in.
 *
 * @example
 * ```typescript
 * import { validateNileConstraint } from './constraint-validator';
 *
 * // UNIQUE missing tenant_id on tenant table -- error
 * const msgs = validateNileConstraint({
 ...
 *     columns: ['email'],
 * });
 * // [{ code: 'NILE-CON-001', severity: 'error', ... }]
 *
 * // UNIQUE with tenant_id not first -- warning
 * const msgs2 = validateNileConstraint({
 *     constraintType: 'unique',
 *     tableName: 'orders',
 *     tableType: 'tenant',
 *     columns: ['email', 'tenant_id'],
 * });
 * // [{ code: 'NILE-CON-002', severity: 'warning', ... }]
 *
 * // UNIQUE with tenant_id first -- no messages
 * const msgs3 = validateNileConstraint({
 *     constraintType: 'unique',
 *     tableName: 'orders',
 *     tableType: 'tenant',
 ...
 *
 * // Shared table -- info
 * const msgs4 = validateNileConstraint({
 *     constraintType: 'unique',
 *     tableName: 'config',
 *     tableType: 'shared',
 *     columns: ['key'],
 * });
 * // [{ code: 'NILE-CON-004', severity: 'info', ... }]
 * ```
 *
 * @remarks
 * **Dialect Support:**
 ...
 * |-------------|----------|------------------------------------------|
 * | Nile        | N/A      | Target dialect -- restrictions validated  |
 * | PostgreSQL  | N/A      | No tenant_id requirements                |
 * | CockroachDB | N/A      | Use CockroachDB validator                |
 * | AWS DSQL    | N/A      | Use DSQL validator                       |
 *
 * @since 1.1.0
 */
export declare function validateNileConstraint(input: NileConstraintInput): NileValidationMessage[];
/**
 * Validate multiple constraints against Nile's multi-tenant rules.
 *
 * Convenience wrapper that calls {@link validateNileConstraint} for each
 * input and returns the concatenated results.
 *
 * @param inputs - Array of constraint definitions to validate.
 * @returns Flat array of all validation messages across all inputs.
 *
 * @example
 * ```typescript
 * import { validateNileConstraints } from './constraint-validator';
 *
 * const msgs = validateNileConstraints([
 *     {
 *         constraintType: 'unique',
 *         tableName: 'orders',
 *         tableType: 'tenant',
 ...
 *     },
 * ]);
 * // msgs[0].code === 'NILE-CON-001' (missing tenant_id)
 * // msgs[1].code === 'NILE-CON-004' (shared table info)
 * ```
 *
 * @remarks
 * **Dialect Support:**
 ...
 * @since 1.1.0
 */
export declare function validateNileConstraints(inputs: NileConstraintInput[]): NileValidationMessage[];
/**
 * Quick check: does a constraint need `tenant_id` on Nile?
 *
 * Returns `true` only if the constraint type is `'unique'` or `'exclusion'`
 * and the table type is `'tenant'`. All other combinations return `false`.
 *
 * @param constraintType - The constraint type.
 * @param tableType - The Nile table classification.
 * @returns `true` if tenant_id is required in this constraint.
 *
 * @example
 * ```typescript
 * import { isNileConstraintTenantScoped } from './constraint-validator';
 *
 * isNileConstraintTenantScoped('unique', 'tenant');     // true
 * isNileConstraintTenantScoped('exclusion', 'tenant');  // true
 * isNileConstraintTenantScoped('unique', 'shared');     // false
 * isNileConstraintTenantScoped('exclusion', 'shared');  // false
 * isNileConstraintTenantScoped('unique', 'builtin');    // false
 * ```
 *
 * @remarks
 * **Dialect Support:**
 ...
 * @since 1.1.0
 */
export declare function isNileConstraintTenantScoped(constraintType: "unique" | "exclusion", tableType: "tenant" | "shared" | "builtin"): boolean;
/**
 * Nile SQL Generator
 *
 * Transforms PostgreSQL DDL statements into Nile-compatible DDL
 * by stripping or replacing unsupported features. Unlike DSQL which
 * blocks many type and feature categories, Nile's transformations
 * focus on:
 *
 * - Stripping triggers, functions, procedures, DO blocks
 * - Stripping RLS policies
 * - Stripping CREATE USER/ROLE/DATABASE, GRANT/REVOKE
 * - Skipping CREATE EXTENSION (pre-installed)
 * - Skipping DROP EXTENSION (cannot remove)
 * - Adding comments for stripped features
 *
 * Nile does NOT need type transformations (JSONB, arrays, etc. all work).
 *
 * @module nile-schema/sql-generator
 *
 * @since 1.1.0
 */
/**
 * Result of a Nile SQL transformation.
 *
 * @since 1.1.0
 */
export interface NileTransformResult {
	/** Transformed SQL safe for Nile execution. */
	sql: string;
	/** Transformations applied, each describing what changed. */
	changes: NileTransformChange[];
	/** True if any modifications were made. */
	modified: boolean;
}
/**
 * Description of a single transformation applied to the DDL.
 *
 * @since 1.1.0
 */
export interface NileTransformChange {
	/** Rule code that triggered this change. */
	ruleCode: string;
	/** Human-readable description. */
	description: string;
	/** Original SQL fragment that was replaced. */
	original: string;
	/** Replacement SQL fragment (empty string if stripped). */
	replacement: string;
}
/**
 * Transform a PostgreSQL DDL statement for Nile compatibility.
 *
 * Applies all transformation rules to produce SQL safe for
 * Nile execution. Returns the transformed SQL along with a
 * log of all changes made.
 *
 * @param sql - Raw PostgreSQL DDL string
 * @returns Transformed result with SQL, changes list, and modified flag
 *
 * @example
 * ```typescript
 * const result = transformSqlForNile(
 *     'CREATE TRIGGER update_timestamp BEFORE UPDATE ON todos ...'
 * );
 * // result.sql === '/* Nile: TRIGGER removed — not supported * /'
 ...
 * @since 1.1.0
 */
export declare function transformSqlForNile(sql: string): NileTransformResult;
/**
 * Transform multiple DDL statements for Nile.
 *
 * Splits on semicolons, transforms each, and rejoins.
 *
 * @param sqlStatements - Semicolon-separated SQL statements
 * @returns Combined transform result
 *
 * @since 1.1.0
 */
export declare function transformSqlStatementsForNile(sqlStatements: string): NileTransformResult;
/**
 * Check if a SQL string needs Nile transformation.
 *
 * Lightweight check — does NOT perform transformation,
 * just tests if any rule patterns match.
 *
 * @param sql - SQL string to check
 * @returns True if the SQL contains patterns that need transformation
 *
 * @since 1.1.0
 */
export declare function needsNileTransformation(sql: string): boolean;
/**
 * Format a list of transform changes for CLI display.
 *
 * @param changes - Changes from a transformation
 * @returns Formatted multi-line string
 *
 * @since 1.1.0
 */
export declare function formatNileTransformChanges(changes: NileTransformChange[]): string;
