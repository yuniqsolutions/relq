...
function parseCreateTable(sql) {
    ...
    if (!tableMatch) {
        throw new relq_errors_1.RelqQueryError('Invalid CREATE TABLE statement', { hint: 'SQL must start with CREATE TABLE' });
    }
    ...
    if (!bodyMatch) {
        throw new relq_errors_1.RelqQueryError('Could not parse table body', { hint: 'Ensure table definition includes column list in parentheses' });
    }
