...
exports.BLOCKED_PLPGSQL_CONSTRUCTS = void 0;
exports.validateFunction = validateFunction;
exports.validateFunctions = validateFunctions;
exports.scanFunctionBody = scanFunctionBody;
...
function validateFunction(fn) {
    ...
    if (fn.language?.toLowerCase() === 'plpgsql') {
        messages.push((0, errors_1.createMessage)('CRDB_I600', location));
    }
    if (fn.body && fn.language?.toLowerCase() === 'plpgsql') {
        for (const construct of BLOCKED_CONSTRUCTS) {
            if (construct.pattern.test(fn.body)) {
                messages.push((0, errors_1.createMessage)(construct.code, location));
            }
        }
    }
    return messages;
...
function scanFunctionBody(body, functionName) {
    const messages = [];
...
exports.BLOCKED_PLPGSQL_CONSTRUCTS = [
