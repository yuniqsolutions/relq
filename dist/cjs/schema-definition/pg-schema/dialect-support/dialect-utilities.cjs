...
exports.getDialectFeatures = getDialectFeatures;
exports.validateColumnType = validateColumnType;
exports.validateColumnTypes = validateColumnTypes;
exports.isIndexMethodSupported = isIndexMethodSupported;
exports.getSupportedIndexMethods = getSupportedIndexMethods;
exports.isFeatureSupported = isFeatureSupported;
exports.getUnsupportedFeatures = getUnsupportedFeatures;
exports.compareDialectFeatures = compareDialectFeatures;
exports.getPortableFeatureSet = getPortableFeatureSet;
const dialect_features_1 = require("./dialect-features.cjs");
function getDialectFeatures(dialect) {
    const features = dialect_features_1.DIALECT_FEATURES.get(dialect);
    if (!features) {
...
function validateColumnType(type, dialect) {
...
function validateColumnTypes(types, dialect) {
    return types
        .map(type => validateColumnType(type, dialect))
        .filter(result => !result.supported);
}
function isIndexMethodSupported(method, dialect) {
    const features = getDialectFeatures(dialect);
    return features.supportedIndexMethods.has(method);
}
function getSupportedIndexMethods(dialect) {
    return getDialectFeatures(dialect).supportedIndexMethods;
}
function isFeatureSupported(feature, dialect) {
    const features = getDialectFeatures(dialect);
    return features.supports[feature];
}
function getUnsupportedFeatures(dialect) {
    const features = getDialectFeatures(dialect);
    const unsupported = [];
...
function compareDialectFeatures(dialect1, dialect2) {
...
function getPortableFeatureSet() {
