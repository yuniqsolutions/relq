...
exports.transformSqlForDsql = transformSqlForDsql;
exports.transformSqlStatementsForDsql = transformSqlStatementsForDsql;
exports.needsDsqlTransformation = needsDsqlTransformation;
exports.formatDsqlTransformChanges = formatDsqlTransformChanges;
...
function transformSqlForDsql(sql) {
    const changes = [];
    let current = sql;
...
function transformSqlStatementsForDsql(sqlStatements) {
...
function needsDsqlTransformation(sql) {
    for (const rule of TRANSFORM_RULES) {
...
function formatDsqlTransformChanges(changes) {
    if (changes.length === 0)
