...
exports.DSQL_TYPE_MAP = void 0;
exports.getDsqlTypeMapping = getDsqlTypeMapping;
exports.isDsqlTypeSupported = isDsqlTypeSupported;
exports.getDsqlUnsupportedTypesByCategory = getDsqlUnsupportedTypesByCategory;
exports.DSQL_TYPE_MAP = {
...
function getDsqlTypeMapping(sqlType) {
    const normalized = sqlType
        .toLowerCase()
        .replace(/\s*\(.*\)/, '')
        .replace(/\s*\[\s*\]/, '')
        .trim();
    return exports.DSQL_TYPE_MAP[normalized];
}
function isDsqlTypeSupported(sqlType) {
    const mapping = getDsqlTypeMapping(sqlType);
    if (!mapping)
        return true;
    return mapping.status === 'supported';
}
function getDsqlUnsupportedTypesByCategory() {
    const groups = {};
    const seen = new Set();
    for (const mapping of Object.values(exports.DSQL_TYPE_MAP)) {
