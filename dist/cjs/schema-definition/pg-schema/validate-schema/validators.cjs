...
function validateSchemaForDialect(schema, options) {
    const { dialect, includeInfo = false, checkPortability = false, allowedCustomTypes = [] } = options;
    const features = (0, dialect_support_1.getDialectFeatures)(dialect);
    const errors = [];
    const warnings = [];
    const info = [];
    const summary = {
...
function validateColumn(column, location, features, errors, warnings, allowedCustomTypes) {
    ...
    const typeResult = (0, dialect_support_1.validateColumnType)(colType, features.dialect);
...
function validateIndex(index, tableName, features, errors, warnings) {
    ...
    if (index.using) {
        ...
        if (!(0, dialect_support_1.isIndexMethodSupported)(method, features.dialect)) {
            errors.push({
                severity: 'error',
