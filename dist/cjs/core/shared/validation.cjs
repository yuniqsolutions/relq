...
function validateData(tableName, data, operation, mappings, config) {
    ...
    if (!mapping) {
        (0, methods_1.debugLog)(config, `No mapping found for table: ${tableName}. Available:`, Array.from(mappings.keys()));
        return;
    }
    const errors = [];
    ...
    (0, methods_1.debugLog)(config, `Validating ${operation} on ${tableName}. Fields:`, Object.keys(data));
    for (const [propName, value] of Object.entries(data)) {
        ...
        if (validateFn) {
            ...
            (0, methods_1.debugLog)(config, `Domain validation for '${propName}' with value '${value}': isValid=${isValid}`);
            if (!isValid) {
    ...
    if (errors.length > 0) {
        ...
        (0, methods_1.debugLog)(config, message);
        if (onError === 'throw') {
...
function validateComposite(propName, value, fields, errors, config) {
    const validation = config?.validation;
    const validateLength = validation?.validateLength !== false;
    const validateTypes = validation?.validateTypes !== false;
    for (const [fieldName, colDef] of Object.entries(fields)) {
        ...
        (0, methods_1.debugLog)(config, `Composite field '${propName}.${fieldName}': value=${JSON.stringify(fieldValue)}, colType=${colType}, nullable=${isNullable}`);
