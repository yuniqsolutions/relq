...
function getSourceColumnName(tableDef, columnKey, relationDef) {
    if (relationDef.$columns && relationDef.$columns.length > 0) {
        return relationDef.$columns[0].column;
    }
    if (tableDef && tableDef.$columns && tableDef.$columns[columnKey]) {
        const colDef = tableDef.$columns[columnKey];
        return colDef.$sqlName || columnKey;
    }
    return camelToSnake(columnKey);
}
function getTargetColumnName(relationDef, targetTableDef) {
    if (relationDef.$references && relationDef.$references.length > 0) {
        return relationDef.$references[0].column;
    }
    return 'id';
}
