...
function isTenantTable(columns) {
    return columns.some(col => col.name === 'tenant_id'
        && col.dataType === 'uuid'
        && !col.isNullable);
}
async function introspectNile(connection, options) {
    const { includeFunctions = false, includeTriggers = false, onProgress, onDetailedProgress, } = options || {};
    ...
    try {
        ...
        `);
        onDetailedProgress?.({ step: 'sequences', count: sequencesResult.rows.length, status: 'done' });
        const sequences = sequencesResult.rows.map(row => ({
            name: row.name,
        ...
        for (const col of columnsResult.rows) {
            ...
            if (dataType === 'ARRAY' && col.udt_name) {
                ...
                dataType = `${mapInternalType(baseType)}[]`;
            }
        ...
        for (const row of tablesResult.rows) {
            ...
            if (tableName.startsWith('_relq') || tableName.startsWith('_kuery'))
                continue;
            if (NILE_SYSTEM_TABLES.includes(tableName))
                continue;
            ...
            const tenantType = isTenantTable(tableColumns) ? 'nile:tenant' : 'nile:shared';
            const table = {
                name: tableName,
                ...
                partitionKey: parseArrayColumn(row.partition_key),
                comment: row.table_comment || null,
            };
            table.nileTableType = tenantType;
            tables.push(table);
        }
