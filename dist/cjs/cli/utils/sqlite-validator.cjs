...
exports.UNSUPPORTED_SQL = exports.UNSUPPORTED_DATA_TYPES = exports.PG_TO_SQLITE_TRANSFORMS = exports.SQLITE_TYPE_AFFINITY = void 0;
exports.validateSqlForSQLite = validateSqlForSQLite;
exports.getSQLiteWarnings = getSQLiteWarnings;
exports.validateSchemaForSQLite = validateSchemaForSQLite;
exports.transformPgToSQLite = transformPgToSQLite;
exports.formatSQLiteErrors = formatSQLiteErrors;
exports.hasSQLiteIncompatibilities = hasSQLiteIncompatibilities;
exports.SQLITE_TYPE_AFFINITY = {
...
exports.PG_TO_SQLITE_TRANSFORMS = {
    SERIAL: {
...
function validateSqlForSQLite(sql, dialect = 'sqlite', location) {
    const errors = [];
...
function getSQLiteWarnings(sql, location) {
    const warnings = [];
    for (const [name, config] of Object.entries(exports.UNSUPPORTED_SQL)) {
        if (!config.isWarning)
            continue;
...
function validateSchemaForSQLite(schema, dialect = 'sqlite') {
    const errors = [];
    const warnings = [];
...
function transformPgToSQLite(sql) {
    let result = sql;
    const transforms = [];
    for (const [name, config] of Object.entries(exports.PG_TO_SQLITE_TRANSFORMS)) {
...
function formatSQLiteErrors(result, dialect = 'sqlite') {
    const lines = [];
...
function hasSQLiteIncompatibilities(sql) {
