...
exports.PLANETSCALE_UNSUPPORTED = exports.UNSUPPORTED_SQL = exports.UNSUPPORTED_DATA_TYPES = exports.PG_TO_MYSQL_TRANSFORMS = void 0;
exports.validateSqlForMySQL = validateSqlForMySQL;
exports.validateSchemaForMySQL = validateSchemaForMySQL;
exports.transformPgToMySQL = transformPgToMySQL;
exports.formatMySQLErrors = formatMySQLErrors;
exports.hasMySQLIncompatibilities = hasMySQLIncompatibilities;
exports.PG_TO_MYSQL_TRANSFORMS = {
    SERIAL: {
...
exports.PLANETSCALE_UNSUPPORTED = {
    FOREIGN_KEY: {
...
function validateSqlForMySQL(sql, dialect = 'mysql', location) {
    const errors = [];
    ...
    if (dialect === 'planetscale') {
        for (const [name, config] of Object.entries(exports.PLANETSCALE_UNSUPPORTED)) {
            if (config.pattern.test(sql)) {
                const match = sql.match(config.pattern);
                errors.push({
...
function validateSchemaForMySQL(schema, dialect = 'mysql') {
    const errors = [];
    const warnings = [];
...
function transformPgToMySQL(sql) {
    let result = sql;
    const transforms = [];
    for (const [name, config] of Object.entries(exports.PG_TO_MYSQL_TRANSFORMS)) {
...
function formatMySQLErrors(result, dialect = 'mysql') {
    const lines = [];
...
function hasMySQLIncompatibilities(sql, dialect = 'mysql') {
    for (const [, config] of Object.entries(exports.UNSUPPORTED_DATA_TYPES)) {
        if (config.dialects && !config.dialects.includes(dialect))
            continue;
        if (config.pattern.test(sql))
            return true;
    ...
    if (dialect === 'planetscale') {
        for (const [, config] of Object.entries(exports.PLANETSCALE_UNSUPPORTED)) {
            if (config.pattern.test(sql))
                return true;
        }
    }
