...
function normalizeIndexColumns(columns) {
    ...
    if (typeof columns === 'string') {
        return columns.replace(/^\{|\}$/g, '').split(',').map(c => c.trim()).filter(Boolean);
    }
    if (Array.isArray(columns)) {
        ...
        for (const col of columns) {
            ...
            if (s.startsWith('{') && s.endsWith('}')) {
                result.push(...s.slice(1, -1).split(',').map(c => c.trim()).filter(Boolean));
            }
            else if (s) {
                result.push(s);
            }
...
async function introspectedToParsedSchema(schema) {
    ...
    for (const t of schema.tables || []) {
        ...
        const fkSignatures = new Map();
        for (const c of t.constraints || []) {
            if (c.type === 'FOREIGN KEY' && c.definition) {
                const sig = c.definition.replace(/\s+/g, ' ').trim().toUpperCase();
                const existing = fkSignatures.get(sig);
                if (existing) {
                    const autoPattern = new RegExp(`^${t.name}_\\w+_fkey$`, 'i');
                    if (autoPattern.test(existing) && !autoPattern.test(c.name)) {
                        fkSignatures.set(sig, c.name);
                    }
                    continue;
                }
                fkSignatures.set(sig, c.name);
        ...
        const fkKeepNames = new Set(fkSignatures.values());
        for (const c of t.constraints || []) {
            if (processedConstraints.has(c.name))
                continue;
            if (c.type === 'FOREIGN KEY' && c.definition) {
                const sig = c.definition.replace(/\s+/g, ' ').trim().toUpperCase();
                if (fkSignatures.has(sig) && !fkKeepNames.has(c.name)) {
                    continue;
                }
            }
            ...
            if (c.type === 'FOREIGN KEY') {
                if (c.referencedTable && c.referencedColumns?.length > 0) {
        ...
        const indexes = (t.indexes || [])
            ...
            .map(i => ({
            name: i.name,
            columns: normalizeIndexColumns(i.columns),
            isUnique: i.isUnique,
            method: i.type,
...
const IMPLICIT_PRECISION_TYPES = new Map([
    ['integer', 32],
    ['int4', 32],
    ['bigint', 64],
    ['int8', 64],
...
function extractTypeParams(col) {
    ...
    if (col.precision != null) {
        const normalizedType = (col.dataType || '').toLowerCase().replace(/\[\]$/, '').trim();
        const udtNorm = (col.udtName || '').toLowerCase().trim();
        const implicitPrecision = IMPLICIT_PRECISION_TYPES.get(normalizedType) ?? IMPLICIT_PRECISION_TYPES.get(udtNorm);
        if (implicitPrecision != null && col.precision === implicitPrecision && !col.scale) {
            return undefined;
        }
