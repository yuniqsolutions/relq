...
exports.computeTrackingIdEdits = computeTrackingIdEdits;
exports.computeTopLevelEdits = computeTopLevelEdits;
...
function findExportCallExpression(source, exportName) {
    for (const decl of source.getVariableDeclarations()) {
        ...
        if (!init)
            continue;
        let node = init;
        while (ts_morph_1.Node.isCallExpression(node) || ts_morph_1.Node.isPropertyAccessExpression(node)) {
            const parent = node.getParent();
            if (!parent)
                break;
            if (ts_morph_1.Node.isCallExpression(parent) || ts_morph_1.Node.isPropertyAccessExpression(parent)) {
                node = parent;
            }
            else {
                break;
...
function findTrackingIdProperty(source, exportName) {
    for (const decl of source.getVariableDeclarations()) {
        ...
        if (!init)
            continue;
        const defineTableCalls = init.getDescendantsOfKind(ts_morph_1.SyntaxKind.CallExpression).filter(c => {
            const e = c.getExpression();
            return ts_morph_1.Node.isIdentifier(e) && e.getText() === 'defineTable';
        });
        const call = defineTableCalls[0];
        if (!call) {
            const e = ts_morph_1.Node.isCallExpression(init) ? init.getExpression() : undefined;
            if (e && ts_morph_1.Node.isIdentifier(e) && e.getText() === 'defineTable') {
...
function findTrackingIdInCall(call) {
    ...
    for (const prop of optionsArg.getProperties()) {
        if (ts_morph_1.Node.isPropertyAssignment(prop) && prop.getName() === '$trackingId') {
...
function findSequenceTrackingId(source, exportName) {
    for (const decl of source.getVariableDeclarations()) {
        ...
        if (!init)
            continue;
        const seqCalls = [init, ...init.getDescendantsOfKind(ts_morph_1.SyntaxKind.CallExpression)].filter(n => {
            if (!ts_morph_1.Node.isCallExpression(n))
                return false;
            const e = n.getExpression();
            return ts_morph_1.Node.isIdentifier(e) && e.getText() === 'pgSequence';
        });
...
function findColumnExpression(source, exportName, columnKey) {
    for (const decl of source.getVariableDeclarations()) {
        ...
        for (const prop of columnsArg.getProperties()) {
            if (ts_morph_1.Node.isPropertyAssignment(prop) && prop.getName() === columnKey) {
...
function computeTrackingIdEdits(filePath, content, issues, parsedSchema, exportMap) {
...
function findColumnByDbName(source, exportName, dbName) {
    for (const decl of source.getVariableDeclarations()) {
        ...
        for (const prop of columnsArg.getProperties()) {
            ...
            if (!val)
                continue;
            const strings = val.getDescendantsOfKind(ts_morph_1.SyntaxKind.StringLiteral);
...
function computeTopLevelEdits(filePath, content, entityType, entities, exportMap, issues) {
    const source = createSourceFile(filePath, content);
    const edits = [];
    for (const issue of issues) {
