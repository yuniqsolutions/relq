...
function formatDefaultValue(value, columnType) {
    ...
    if (upper === 'UUID_GENERATE_V4()') {
        needsDefaultImport = true;
        return 'DEFAULT.uuidGenerateV4()';
    }
    if (upper === 'UUID_GENERATE_V1()') {
        needsDefaultImport = true;
    ...
    if (/^md5\(\(?gen_random_uuid\(\)\)?::text\)$/i.test(trimmed)) {
        needsDefaultImport = true;
        return 'DEFAULT.md5RandomUuid()';
    }
    ...
    const isStringColumn = normalizedColType ? [
        'varchar', 'character varying', 'char', 'character', 'bpchar',
        'text', 'citext', 'name',
    ].includes(normalizedColType) || normalizedColType.startsWith('varchar(') || normalizedColType.startsWith('character varying(') || normalizedColType.startsWith('char(') : false;
    if (/^-?\d+$/.test(trimmed)) {
        if (isStringColumn) {
            return `'${trimmed}'`;
        }
