...
exports.TENANT_WARNINGS = exports.UNSUPPORTED_SQL = exports.NILE_PREINSTALLED_EXTENSIONS = exports.NILE_BUILTIN_TABLES = void 0;
exports.classifyTable = classifyTable;
exports.validateSqlForNile = validateSqlForNile;
exports.validateSchemaForNile = validateSchemaForNile;
exports.validateTenantIdInConstraint = validateTenantIdInConstraint;
exports.formatNileErrors = formatNileErrors;
exports.hasNileIncompatibilities = hasNileIncompatibilities;
exports.filterSqlForNile = filterSqlForNile;
exports.NILE_BUILTIN_TABLES = ['tenants', 'users', 'tenant_users'];
...
function validateSqlForNile(sql, location) {
    const errors = [];
...
function validateSchemaForNile(schema) {
    const errors = [];
    const warnings = [];
    ...
    if (schema.extensions && schema.extensions.length > 0) {
        for (const ext of schema.extensions) {
            const isPreinstalled = exports.NILE_PREINSTALLED_EXTENSIONS.some(preinstalled => preinstalled.toLowerCase() === ext.toLowerCase());
            if (isPreinstalled) {
                warnings.push({
                    category: 'EXTENSION',
                    feature: 'EXTENSION',
    ...
    if (schema.tables) {
        for (const table of schema.tables) {
            if (exports.NILE_BUILTIN_TABLES.includes(table.name.toLowerCase())) {
                warnings.push({
                    category: 'TENANT',
...
function validateTenantIdInConstraint(tableName, constraintType, columns, isTenantTable) {
    if (!isTenantTable)
        return null;
...
function formatNileErrors(result) {
    const lines = [];
...
function hasNileIncompatibilities(sql) {
