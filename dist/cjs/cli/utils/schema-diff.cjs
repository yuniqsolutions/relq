...
function formatDiff(diff) {
    const lines = [];
    if (!diff.hasChanges) {
        lines.push(spinner_1.colors.green('✓ No changes detected'));
        return lines.join('\n');
    }
    const { summary } = diff;
    const parts = [];
    if (summary.tablesAdded > 0)
        parts.push(spinner_1.colors.green(`+${summary.tablesAdded} tables`));
    if (summary.tablesRemoved > 0)
        parts.push(spinner_1.colors.red(`-${summary.tablesRemoved} tables`));
    if (summary.tablesModified > 0)
        parts.push(spinner_1.colors.yellow(`~${summary.tablesModified} tables`));
    lines.push(`Changes: ${parts.join(', ') || 'none'}`);
    lines.push('');
    for (const table of diff.tables) {
        const icon = table.type === 'added' ? spinner_1.colors.green('+') :
            table.type === 'removed' ? spinner_1.colors.red('-') :
                spinner_1.colors.yellow('~');
        lines.push(`${icon} ${spinner_1.colors.bold(table.name)}`);
        for (const col of table.columns || []) {
            const colIcon = col.type === 'added' ? spinner_1.colors.green('  +') :
                col.type === 'removed' ? spinner_1.colors.red('  -') :
                    spinner_1.colors.yellow('  ~');
            if (col.changes && col.changes.length > 0) {
                const changeStr = col.changes.map(c => `${c.field}: ${c.from} → ${c.to}`).join(', ');
                lines.push(`${colIcon} ${col.name} (${spinner_1.colors.muted(changeStr)})`);
            }
            else {
        ...
        for (const idx of table.indexes || []) {
            const idxIcon = idx.type === 'added' ? spinner_1.colors.green('  +') :
                spinner_1.colors.red('  -');
            lines.push(`${idxIcon} index: ${idx.name}`);
        }
    }
    for (const ext of diff.extensions) {
        const icon = ext.type === 'added' ? spinner_1.colors.green('+') : spinner_1.colors.red('-');
...
function formatSummary(diff) {
    const { summary } = diff;
    if (!diff.hasChanges) {
        return spinner_1.colors.green('No changes');
    }
    const parts = [];
