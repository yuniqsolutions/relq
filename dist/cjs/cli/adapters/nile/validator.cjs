...
exports.validateTableForNile = validateTableForNile;
...
function validateTableForNile(table) {
    const issues = [];
    const location = `${table.schema}.${table.name}`;
    if (features_1.NILE_RESERVED_TABLES.includes(table.name.toLowerCase())) {
        issues.push({
            severity: 'error',
            category: 'CONSTRAINT',
    ...
    if (features_1.NILE_RESERVED_SCHEMAS.includes(table.schema.toLowerCase())) {
        issues.push({
            severity: 'error',
            category: 'CONSTRAINT',
...
function validateSqlForNile(sql) {
    const issues = [];
    const upperSql = sql.toUpperCase();
    for (const operation of features_1.NILE_UNSUPPORTED_OPERATIONS) {
