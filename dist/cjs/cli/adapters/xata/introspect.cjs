...
exports.introspectXata = introspectXata;
const features_1 = require("./features.cjs");
async function introspectXata(fetchSchema, databaseName, options) {
    ...
    for (const table of schema.tables) {
        if (features_1.XATA_INTERNAL_TABLES.includes(table.name)) {
            continue;
        }
...
function convertXataTable(table) {
    ...
    for (const col of table.columns) {
        if (features_1.XATA_RESERVED_COLUMNS.includes(col.name)) {
            continue;
        }
...
function convertXataColumn(col, tableName, position) {
    let type = features_1.XATA_TO_POSTGRES_TYPE_MAP[col.type] || col.type;
    let baseType = type;
