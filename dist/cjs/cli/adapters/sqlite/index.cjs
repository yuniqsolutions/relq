...
exports.SQLITE_FEATURES = exports.SQLiteAdapter = void 0;
...
class SQLiteAdapter {
    ...
    family = 'sqlite';
    displayName = 'SQLite';
    features = features_1.SQLITE_FEATURES;
    defaultPort = 0;
    defaultUser = '';
    quoteChar = '"';
    ...
    async getDatabaseVersion(connection) {
        ...
        try {
            ...
            return await (0, introspect_1.getSQLiteVersion)(executeQuery);
        }
        finally {
            db.close();
        }
    ...
    async introspect(connection, options) {
        ...
        try {
            ...
            return await (0, introspect_1.introspectSQLite)(executeQuery, databaseName, options);
        }
        finally {
            db.close();
        }
    ...
    async listTables(connection, schema) {
        ...
        try {
            ...
            return await (0, introspect_1.listTables)(executeQuery);
        }
        finally {
            db.close();
        }
    ...
    mapTypeToInternal(friendlyType) {
        ...
        return features_1.POSTGRES_TO_SQLITE_TYPE_MAP[type] || friendlyType;
    }
    ...
    async openDatabase(config) {
        ...
        if (typeof Bun !== 'undefined') {
            try {
                const { Database } = await Promise.resolve(`${'bun:sqlite'}`).then(s => __importStar(require(s)));
                const filename = config.memory ? ':memory:' : (config.filename || ':memory:');
...
Object.defineProperty(exports, "SQLITE_FEATURES", { enumerable: true, get: function () { return features_2.SQLITE_FEATURES; } });
