    ...
    try {
        ...
        if (schemaExists && localSnapshot && !force) {
            ...
            if (allChanges.length === 0) {
                ...
                spin.start('Syncing types from database...');
                ...
                if (typesResult.generated && typesResult.typesCount > 0) {
                    ...
                    if (typesResult.kept.length > 0)
                        parts.push(`${typesResult.kept.length} local-only`);
                    const detail = parts.length > 0 ? ` (${parts.join(', ')})` : '';
                    spin.stop(`Synced ${typesResult.typesCount} type(s) from remote${detail}`);
                    hasSynced = true;
                }
                else {
        ...
        if (typesResult.generated) {
            ...
            if (typesResult.typesCount > 0) {
                const parts = [];
                if (typesResult.added.length > 0)
                    parts.push(colors_1.colors.green(`${typesResult.added.length} added`));
                if (typesResult.updated.length > 0)
                    parts.push(colors_1.colors.yellow(`${typesResult.updated.length} updated`));
                if (typesResult.kept.length > 0)
                    parts.push(colors_1.colors.cyan(`${typesResult.kept.length} local-only`));
                const detail = parts.length > 0 ? ` — ${parts.join(', ')}` : '';
                spin.stop(`Written ${colors_1.colors.cyan(typesFilePath)} ${colors_1.colors.muted(`(${typesResult.typesCount} types, ${(0, format_1.formatBytes)(typesFileSize)})`)}${detail}`);
            }
            else {
