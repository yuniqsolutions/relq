...
function checkEnvVars() {
    ...
    let detectedDialect;
    if (connUrl) {
        detectedDialect = (0, dialect_router_1.detectDialectFromUrl)(connUrl);
    }
...
async function askDialect(detectedDialect) {
    ...
    if (p.isCancel(result))
        (0, ui_1.fatal)('Operation cancelled');
    return result;
}
...
exports.default = (0, citty_1.defineCommand)({
    ...
    async run({ args }) {
        ...
        p.intro(colors_1.colors.cyan('relq init'));
        ...
        if (existingConfig) {
            ...
            if (existingConfig.directory !== cwd) {
                p.log.info(`Found relq.config.ts in: ${colors_1.colors.cyan(existingConfig.directory)}`);
            ...
            if (packageRoot && packageRoot !== cwd) {
                ...
                p.log.info(`Found package.json in: ${colors_1.colors.cyan(packageRoot)}`);
                p.log.info('Initializing at project root...');
            }
        }
        if (existingConfig && existingConfig.directory === projectRoot) {
            ...
            if (relqFolderExists) {
                p.log.success(colors_1.colors.green('Relq is already initialized.'));
                p.log.info(`  ${colors_1.colors.dim('-')} relq.config.ts`);
                p.log.info(`  ${colors_1.colors.dim('-')} .relq/ folder`);
                p.log.info(`Use ${colors_1.colors.cyan('relq status')} to see current state`);
                p.log.info(`Use ${colors_1.colors.cyan('relq pull')} to sync with database`);
                p.outro('');
                return;
            }
            else {
                (0, ui_1.warning)('Found relq.config.ts but .relq/ folder is missing');
                ...
                if (p.isCancel(upgrade) || !upgrade) {
                    p.outro(colors_1.colors.dim('Cancelled.'));
                    return;
                }
                const missingFields = [];
                ...
                if (missingFields.length > 0) {
                    (0, ui_1.warning)('Your config is missing required fields:');
                    for (const field of missingFields) {
                        p.log.info(`  ${colors_1.colors.dim('-')} ${field}`);
                    }
                    p.log.info(colors_1.colors.dim("Let's fill in the missing fields:"));
        ...
        try {
            ...
            if (envCheck.found) {
                ...
                if (envCheck.hasConnUrl) {
                    ...
                    p.log.info(`  ${colors_1.colors.cyan('Connection URL')}`);
                    p.log.info(`    DATABASE_URL: ${url?.substring(0, 40)}...`);
                    if (envCheck.detectedDialect) {
                        p.log.info(`    ${colors_1.colors.green('Detected dialect:')} ${envCheck.detectedDialect}`);
                    }
                }
                if (envCheck.hasIndividual) {
                    p.log.info(`  ${colors_1.colors.cyan('PostgreSQL Config')}`);
                ...
                if (envCheck.hasAws) {
                    p.log.info(`  ${colors_1.colors.cyan('AWS DSQL')}`);
                ...
                if (envCheck.hasTurso) {
                    p.log.info(`  ${colors_1.colors.cyan('Turso/libSQL')}`);
                ...
                if (envCheck.hasMysql) {
                    p.log.info(`  ${colors_1.colors.cyan('MySQL/PlanetScale')}`);
                ...
                if (envCheck.hasXata) {
                    p.log.info(`  ${colors_1.colors.cyan('Xata')}`);
            ...
            if (useEnv) {
                ...
                if (dialect === 'sqlite') {
                    ...
                    if (p.isCancel(hostResult))
                        (0, ui_1.fatal)('Operation cancelled');
                    host = hostResult;
                    const connAdapter = await (0, registry_1.getAdapter)(dialect);
