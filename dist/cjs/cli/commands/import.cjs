...
async function importCommand(sqlFilePath, options = {}, projectRoot = process.cwd()) {
    ...
    if (importConfig?.connection && (0, config_1.isAwsDsqlConfig)(importConfig.connection)) {
        const sqlErrors = (0, dsql_validator_1.validateSqlForDsql)(sqlContent, 'import SQL');
        const schemaValidation = (0, dsql_validator_1.validateSchemaForDsql)({
        ...
        if (allErrors.length > 0) {
            ...
            (0, git_utils_1.fatal)('Imported SQL contains DSQL incompatibilities', 'Fix the issues above before importing into an AWS DSQL project.');
        }
        if (schemaValidation.warnings.length > 0) {
            console.log('');
            console.log((0, dsql_validator_1.formatDsqlErrors)({ valid: true, errors: [], warnings: schemaValidation.warnings }));
        }
    }
