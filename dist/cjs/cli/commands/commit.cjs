...
async function commitCommand(context) {
    ...
    const dsqlConfig = await (0, config_1.loadConfig)();
    if (dsqlConfig?.connection && (0, config_1.isAwsDsqlConfig)(dsqlConfig.connection) && sql) {
        const dsqlErrors = (0, dsql_validator_1.validateSqlForDsql)(sql, 'commit SQL');
        if (dsqlErrors.length > 0) {
            ...
            (0, cli_utils_1.fatal)('Commit SQL contains DSQL incompatibilities', 'Fix the issues above before committing.');
        }
    }
