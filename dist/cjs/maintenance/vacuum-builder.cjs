    ...
    toString() {
        ...
        if (this.tables.length > 0) {
            const tableStrs = this.tables.map(table => {
                let tableStr = pg_format_1.default.ident(table);
                if (this.columns.has(table)) {
                    const cols = this.columns.get(table);
                    tableStr += ` (${cols.map(c => pg_format_1.default.ident(c)).join(', ')})`;
                }
                return tableStr;
            });
            sql += ' ' + tableStrs.join(', ');
...
exports.VacuumBuilder = VacuumBuilder;
...
exports.AnalyzeBuilder = AnalyzeBuilder;
