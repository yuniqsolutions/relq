...
function parseConnectionUrl(url, dialectHint) {
    const detectedDialect = (0, types_1.detectDialectFromUrl)(url);
    const dialect = dialectHint ?? detectedDialect ?? types_1.DEFAULT_DIALECT;
...
function buildConnectionUrl(config, dialect) {
    if (config.url) {
        return config.url;
    }
...
function getDatabaseName(config, dialect) {
    switch (dialect) {
        case 'postgres':
        case 'cockroachdb':
        case 'nile':
