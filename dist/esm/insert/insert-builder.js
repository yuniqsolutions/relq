...
export class InsertBuilder {
    ...
    buildConflictClause() {
        ...
        let clause = this.conflictColumns && this.conflictColumns.length > 0
            ? format(' ON CONFLICT (%I)', this.conflictColumns.map(c => this.resolveColumnName(c)))
            : ' ON CONFLICT';
        if (this.conflictBuilder.action === 'update') {
            const updateSql = buildConflictUpdateSql(this.conflictBuilder.updateData, this.tableName, (col) => this.resolveColumnName(col));
