...
export function transformSqlForNile(sql) {
    const changes = [];
    let current = sql;
...
export function transformSqlStatementsForNile(sqlStatements) {
...
export function needsNileTransformation(sql) {
    for (const rule of TRANSFORM_RULES) {
...
export function formatNileTransformChanges(changes) {
    if (changes.length === 0)
