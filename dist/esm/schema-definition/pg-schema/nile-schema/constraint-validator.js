...
export function validateNileConstraint(input) {
    const messages = [];
    const location = {
    ...
    if (input.tableType === 'shared') {
        messages.push({
            ...createNileMessage('NILE-CON-004', location),
    ...
    if (input.constraintType === 'unique') {
        if (!hasTenantId) {
            ...
            messages.push({
                ...createNileMessage('NILE-CON-001', location),
        ...
        if (input.columns[0] !== 'tenant_id') {
            messages.push({
                ...createNileMessage('NILE-CON-002', location),
    ...
    if (input.constraintType === 'exclusion') {
        if (!hasTenantId) {
            messages.push({
                ...createNileMessage('NILE-CON-003', location),
...
export function validateNileConstraints(inputs) {
    const messages = [];
...
export function isNileConstraintTenantScoped(constraintType, tableType) {
