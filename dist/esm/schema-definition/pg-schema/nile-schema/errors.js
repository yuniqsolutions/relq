...
export function createNileValidationResult(messages) {
...
export function lookupNileRule(code) {
    return NILE_ALL_RULES[code];
}
export function createNileMessage(code, location) {
    const def = NILE_ALL_RULES[code];
    if (!def) {
        return {
            code,
            severity: 'error',
...
export function formatNileMessage(msg) {
...
export function formatNileValidationResult(result) {
    const lines = [];
