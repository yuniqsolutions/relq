export const NILE_BUILTIN_TABLE_NAMES = new Set([
    'tenants',
    'users',
    'tenant_users',
]);
...
export function classifyNileTable(table) {
    ...
    if (NILE_BUILTIN_TABLE_NAMES.has(lowerName)) {
        return {
...
export function classifyNileTables(tables) {
...
export function isNileBuiltinTable(tableName) {
    return NILE_BUILTIN_TABLE_NAMES.has(tableName.toLowerCase());
}
export function isNileTenantTable(table) {
    return table.columns.some(c => c.toLowerCase() === TENANT_ID_COLUMN);
}
export function validateNileTableClassification(table) {
    const messages = [];
    const classification = classifyNileTable(table);
