...
export function getNileTypeMapping(sqlType) {
    const normalized = sqlType
        .toLowerCase()
        .replace(/\s*\(.*\)/, '')
        .replace(/\s*\[\s*\]/, '')
        .trim();
    return NILE_TYPE_MAP[normalized];
}
export function isNileTypeSupported(sqlType, tableType) {
...
export function getNileContextDependentTypes() {
    const result = [];
    const seen = new Set();
    for (const mapping of Object.values(NILE_TYPE_MAP)) {
