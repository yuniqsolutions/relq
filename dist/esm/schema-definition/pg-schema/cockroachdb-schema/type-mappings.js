...
export function getTypeMapping(sqlType) {
    const normalized = sqlType.toLowerCase().trim();
    if (CRDB_TYPE_MAP[normalized]) {
        return CRDB_TYPE_MAP[normalized];
    }
    const baseType = normalized.replace(/\(.*\)$/, '').trim();
    if (CRDB_TYPE_MAP[baseType]) {
        return CRDB_TYPE_MAP[baseType];
    }
    const nonArray = baseType.replace(/\[\]$/, '').trim();
    if (CRDB_TYPE_MAP[nonArray]) {
        return CRDB_TYPE_MAP[nonArray];
    }
    return undefined;
}
export function isTypeSupported(sqlType) {
    const mapping = getTypeMapping(sqlType);
    if (!mapping)
        return true;
    return mapping.status !== 'unsupported';
}
export function getUnsupportedTypesByCategory() {
    const result = new Map();
    for (const mapping of Object.values(CRDB_TYPE_MAP)) {
...
export function getTypesWithDifferences() {
    return Object.values(CRDB_TYPE_MAP).filter(m => m.status === 'behavioral-difference' || m.status === 'warning');
}
