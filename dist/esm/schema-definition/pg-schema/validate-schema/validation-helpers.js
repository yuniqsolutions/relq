import { validateColumnType } from "../dialect-support/index.js";
import { validateSchemaForDialect } from "./validators.js";
export function validateTableForDialect(table, dialect) {
    return validateSchemaForDialect({ tables: { [table.$name]: table } }, { dialect });
}
export function isColumnTypeSupported(columnType, dialect) {
    const result = validateColumnType(columnType, dialect);
    return result.supported;
}
export function getSchemaUnsupportedFeatures(schema, dialect) {
    const result = validateSchemaForDialect(schema, { dialect });
