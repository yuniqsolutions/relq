...
export function generateConstraintCode(constraints, useCamelCase, getTrackingId) {
    const lines = [];
    for (const c of constraints) {
        ...
        const trackingId = c.trackingId || (getTrackingId ? getTrackingId('k') : undefined);
        const idSuffix = trackingId ? `.$id('${trackingId}')` : '';
        if (c.type === 'PRIMARY KEY' && c.columns.length > 1) {
            const nameOpt = c.name ? `name: '${c.name}', ` : '';
            lines.push(`        constraint.primaryKey({ ${nameOpt}columns: [${cols.join(', ')}]${commentOpt} })${idSuffix}`);
        }
        else if (c.type === 'UNIQUE' && c.columns.length > 1) {
            const nameOpt = c.name ? `name: '${c.name}', ` : '';
            lines.push(`        constraint.unique({ ${nameOpt}columns: [${cols.join(', ')}]${commentOpt} })${idSuffix}`);
        }
...
export function generateCheckConstraintsOption(constraints, useCamelCase, getTrackingId) {
    ...
    const lines = checks.map(c => {
        ...
        if (c.expressionAst) {
            try {
                ...
                return `        check.constraint('${c.name}', ${typedExpr})${idSuffix}`;
            }
            catch {
