...
export async function syncTypesFromDb(connection, typesFilePath) {
    const dbTypes = await getTypesFromDb(connection);
    if (dbTypes.length === 0 && !fs.existsSync(typesFilePath)) {
        const content = generateTypesFile([]);
        fs.writeFileSync(typesFilePath, content);
        return { typesCount: 0, generated: true, added: [], updated: [], kept: [] };
    }
    if (dbTypes.length === 0) {
        return { typesCount: 0, generated: false, added: [], updated: [], kept: [] };
    }
    const localTypes = fs.existsSync(typesFilePath)
        ? parseTypesFile(fs.readFileSync(typesFilePath, 'utf-8'))
        : [];
    const dbTypeMap = new Map(dbTypes.map(t => [t.name, t]));
    const localTypeMap = new Map(localTypes.map(t => [t.name, t]));
    const merged = [];
    const added = [];
    const updated = [];
    const kept = [];
    for (const dbType of dbTypes) {
        merged.push(dbType);
        const localType = localTypeMap.get(dbType.name);
        if (!localType) {
            added.push(dbType.name);
        }
        else if (localType.source.trim() !== dbType.source.trim()) {
            updated.push(dbType.name);
        }
    }
    for (const localType of localTypes) {
        if (!dbTypeMap.has(localType.name)) {
            merged.push({
                name: localType.name,
                kind: localType.kind,
                source: localType.source,
            });
            kept.push(localType.name);
        }
    }
    merged.sort((a, b) => a.name.localeCompare(b.name));
    const content = generateTypesFile(merged);
    fs.writeFileSync(typesFilePath, content);
    return {
        typesCount: merged.length,
        generated: true,
        added,
        updated,
        kept,
