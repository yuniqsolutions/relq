...
export function mergeTrackingIdsFromParsed(target, source) {
    ...
    for (const table of target.tables) {
        ...
        const sourceConMap = new Map(sourceTable.constraints.map(c => [c.name, c]));
        for (const con of table.constraints) {
            const sourceCon = sourceConMap.get(con.name);
            if (sourceCon?.trackingId && !con.trackingId) {
                con.trackingId = sourceCon.trackingId;
            }
            else if (!con.trackingId && con.type === 'FOREIGN KEY' && con.references) {
                const sig = `FK:${con.columns.sort().join(',')}→${con.references.table}`;
                for (const [, sCon] of sourceConMap) {
                    if (sCon.trackingId && sCon.type === 'FOREIGN KEY' && sCon.references) {
                        const sSig = `FK:${sCon.columns.sort().join(',')}→${sCon.references.table}`;
                        if (sig === sSig) {
                            con.trackingId = sCon.trackingId;
                            break;
                        }
                    }
                }
...
export function copyTrackingIdsToNormalized(parsedSchema, normalizedSchema) {
    ...
    for (const table of parsedSchema.tables) {
        ...
        for (const con of table.constraints) {
            if (con.trackingId) {
                constraintMap.set(con.name, con.trackingId);
            }
        }
    ...
    for (const table of normalizedSchema.tables) {
        ...
        for (const con of table.constraints || []) {
            const conTrackingId = parsed.constraints.get(con.name);
            if (conTrackingId) {
                con.trackingId = conTrackingId;
            }
        }
    }
}
...
function generateColumnCode(col, useCamelCase, enumNames, domainNames, checkOverride, genericTypeName) {
    ...
    if (enumNames.has(normalizedType) || enumNames.has(col.type)) {
        ...
        if (useCamelCase && colName !== col.name) {
            ...
            if (col.typeParams?.precision != null) {
                const bn = builderInfo.builderName;
                if (bn === 'numeric' || bn === 'decimal') {
                    typeBuilder += `.precision(${col.typeParams.precision})`;
                    if (col.typeParams.scale != null && col.typeParams.scale > 0) {
                        typeBuilder += `.scale(${col.typeParams.scale})`;
                    }
                }
            }
        }
...
function generateTableCode(table, useCamelCase, enumNames, domainNames, columnTypeMap = {}) {
    ...
    if (compositePKs.length > 0 || compositeUniques.length > 0 || excludeConstraints.length > 0) {
        const constraintLines = generateConstraintCode([...compositePKs, ...compositeUniques, ...excludeConstraints], useCamelCase, (p) => generateTrackingId(p));
        if (constraintLines.length > 0) {
            optionParts.push(`    constraints: (table, constraint) => [\n${constraintLines.join(',\n')},\n    ]`);
        }
    }
    const checkConstraintsOption = generateCheckConstraintsOption(table.constraints, useCamelCase, (p) => generateTrackingId(p));
    ...
    if (generatedCols.length > 0) {
        ...
        for (const col of generatedCols) {
            ...
            if (ast) {
                try {
                    const builderExpr = astToBuilder(ast, {
                        prefix: 't',
                        useCamelCase,
                        useTableRef: true,
                        chainable: true,
...
function generateSequenceCode(seq, useCamelCase) {
    ...
    if (seq.minValue !== undefined && !isDefaultLimit(seq.minValue, 'min')) {
        opts.push(`minValue: ${seq.minValue}`);
    }
    if (seq.maxValue !== undefined && !isDefaultLimit(seq.maxValue, 'max')) {
...
function generatePolicyCode(policy, useCamelCase, tableNames) {
    const policyName = useCamelCase ? toCamelCase(policy.name) : policy.name;
    const tableName = useCamelCase ? toCamelCase(policy.table) : policy.table;
    const parts = [];
...
function generateRelationsCode(schema, camelCase) {
        ...
        for (const constraint of table.constraints) {
            if (constraint.type === 'FOREIGN KEY' && constraint.references) {
                if (constraint.columns.length === 1 && constraint.references.columns.length === 1) {
                    const existingFK = foreignKeys.find(fk => fk.fromTable === table.name &&
                        fk.fromColumn === constraint.columns[0] &&
                        fk.toTable === constraint.references.table);
                    if (existingFK) {
                        if (!existingFK.constraintName) {
                            existingFK.constraintName = getExplicitFKName(constraint.name, table.name, constraint.columns[0]);
                        }
                        if (!existingFK.onDelete && constraint.references.onDelete) {
                            existingFK.onDelete = constraint.references.onDelete;
                        }
                        if (!existingFK.onUpdate && constraint.references.onUpdate) {
                            existingFK.onUpdate = constraint.references.onUpdate;
                        }
                        if (!existingFK.match && constraint.references.match) {
                            existingFK.match = constraint.references.match;
                        }
                        if (!existingFK.deferrable && constraint.references.deferrable) {
                            existingFK.deferrable = constraint.references.deferrable;
                        }
                        if (!existingFK.initiallyDeferred && constraint.references.initiallyDeferred) {
                            existingFK.initiallyDeferred = constraint.references.initiallyDeferred;
                        }
                    }
                    else {
                        foreignKeys.push({
    ...
    for (const fk of foreignKeys) {
        ...
        if (fk.isComposite && fk.fromColumns && fk.toColumns) {
            const fromCols = fk.fromColumns.map(c => `'${camelCase ? toCamelCase(c) : c}'`).join(', ');
