...
import { compareCheckConstraints as astCompareCheckConstraints, compareFunctionBodies as astCompareFunctionBodies, extractColumnFromCheck as astExtractColumnFromCheck, } from "./pg-parser.js";
function arraysEqual(a, b) {
    if (a.length !== b.length)
        return false;
    return a.every((val, idx) => val === b[idx]);
}
export async function compareSchemas(before, after) {
    const changes = [];
...
function compareExtensions(before, after) {
    ...
    for (const ext of afterSet) {
        if (!beforeSet.has(ext)) {
            changes.push(createChange('CREATE', 'EXTENSION', ext, null, { name: ext }));
        }
    }
    for (const ext of beforeSet) {
        if (!afterSet.has(ext)) {
            changes.push(createChange('DROP', 'EXTENSION', ext, { name: ext }, null));
        }
    }
    return changes;
}
function compareEnums(before, after) {
    ...
    for (const [name, enumDef] of afterMap) {
        if (!beforeMap.has(name)) {
            changes.push(createChange('CREATE', 'ENUM', name, null, enumDef));
        }
        else {
            ...
            for (let i = 0; i < enumDef.values.length; i++) {
                ...
                if (!beforeValues.has(value)) {
                    ...
                    changes.push(createChange('CREATE', 'ENUM_VALUE', value, null, {
                        enumName: name,
                        value,
                        after: afterValue,
                    }, name));
    ...
    for (const [name, enumDef] of beforeMap) {
        if (!afterMap.has(name)) {
            changes.push(createChange('DROP', 'ENUM', name, enumDef, null));
        }
    }
    return changes;
}
function compareDomains(before, after) {
    ...
    for (const [name, domain] of afterMap) {
        if (!beforeMap.has(name)) {
            changes.push(createChange('CREATE', 'DOMAIN', name, null, {
                name: domain.name,
...
function compareCompositeTypes(before, after) {
    ...
    for (const [name, typeDef] of afterMap) {
        if (!beforeMap.has(name)) {
            changes.push(createChange('CREATE', 'COMPOSITE_TYPE', name, null, typeDef));
        }
    }
    for (const [name, typeDef] of beforeMap) {
        if (!afterMap.has(name)) {
            changes.push(createChange('DROP', 'COMPOSITE_TYPE', name, typeDef, null));
        }
    }
    return changes;
}
...
async function compareTables(before, after) {
    ...
    for (const [name, table] of beforeMap) {
        if (!afterMap.has(name)) {
            changes.push(createChange('DROP', 'TABLE', name, tableToChangeData(table), null));
        }
    }
    return changes;
}
...
async function checkDefinitionsEqual(def1, def2) {
    ...
    if (!def1 || !def2)
        return false;
    const astResult = await astCompareCheckConstraints(def1, def2);
    if (astResult) {
        return true;
    }
...
async function compareConstraints(before, after, tableName) {
    ...
    const extractColumnFromCheckDef = (definition) => {
        const astResult = astExtractColumnFromCheck(definition);
        if (astResult)
            return astResult;
    ...
    for (const [col, afterCon] of afterChecksByCol) {
        if (beforeChecksByCol.has(col)) {
            ...
            if (!await checkDefinitionsEqual(beforeCon.definition, afterCon.definition)) {
                changes.push(createChange('ALTER', 'CHECK', afterCon.name, {
                    name: beforeCon.name,
...
function compareViews(before, after) {
    ...
    for (const [name, view] of afterMap) {
        if (!beforeMap.has(name)) {
            changes.push(createChange('CREATE', 'VIEW', name, null, view));
        }
        else {
            const beforeView = beforeMap.get(name);
            if (beforeView.definition !== view.definition) {
                changes.push(createChange('ALTER', 'VIEW', name, beforeView, view));
            }
        }
    }
    for (const [name, view] of beforeMap) {
        if (!afterMap.has(name)) {
            changes.push(createChange('DROP', 'VIEW', name, view, null));
        }
    }
    return changes;
}
async function compareFunctions(before, after) {
    ...
    for (const [name, func] of afterMap) {
        if (!beforeMap.has(name)) {
            changes.push(createChange('CREATE', 'FUNCTION', name, null, {
                name: func.name,
            ...
            const bodiesMatch = await astCompareFunctionBodies(beforeFunc.definition, func.definition || '');
            if (!bodiesMatch || beforeFunc.returnType !== func.returnType) {
                changes.push(createChange('ALTER', 'FUNCTION', name, {
    ...
    for (const [name, func] of beforeMap) {
        if (!afterMap.has(name)) {
            changes.push(createChange('DROP', 'FUNCTION', name, func, null));
        }
    }
    return changes;
}
...
export { compareExtensions, compareEnums, compareDomains, compareTables };
