...
export function validateTrackingIds(schema) {
    const issues = [];
...
export function formatTrackingIdIssues(issues) {
    if (issues.length === 0)
        return '';
    ...
    lines.push(`${colors.red('✖')}  ${colors.bold(`Tracking ID ${label} (${total})`)}`);
    lines.push(colors.dim('───────────────────────────────────'));
    if (duplicates.length > 0) {
        lines.push('');
        lines.push(`  ${colors.red(`Duplicates (${duplicates.length})`)}`);
        lines.push('');
        ...
        for (const [table, tableIssues] of byTable) {
            lines.push(`  ${colors.bold(table)}`);
            for (const d of tableIssues) {
                const name = d.location.includes('.') ? d.location.split('.').slice(1).join('.') : d.location;
                lines.push(`    ${colors.red('✗')} ${d.entity} ${colors.cyan(`"${name}"`)}  ${colors.dim(`$id("${d.trackingId}")`)}`);
            }
        }
    }
    if (missing.length > 0) {
        lines.push('');
        lines.push(`  ${colors.yellow(`Missing (${missing.length})`)}`);
        lines.push('');
        const byTable = new Map();
        ...
        for (const [table, tableIssues] of byTable) {
            ...
            for (const m of tableIssues) {
                ...
                lines.push(`    ${colors.yellow('○')} ${m.entity} ${colors.cyan(`"${name}"`)}`);
            }
        }
    }
    lines.push('');
    lines.push(colors.dim('───────────────────────────────────'));
    lines.push(`  Run ${colors.cyan('relq fix')} to auto-repair all issues.`);
    return lines.join('\n');
}
