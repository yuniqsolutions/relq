...
export function tableToAST(table) {
    ...
    for (const uc of table.$uniqueConstraints || []) {
        constraints.push({
            ...
            type: 'UNIQUE',
            columns: uc.columns,
            trackingId: uc.$trackingId || uc.trackingId || uc.name || `${table.$name}_${uc.columns.join('_')}_key`,
        });
    }
    for (const cc of table.$checkConstraints || []) {
        constraints.push({
            ...
            trackingId: cc.$trackingId || cc.trackingId || cc.name || `${table.$name}_check`,
        });
    }
...
function extractDefaultValue(defaultVal, columnType, isArray) {
    ...
    if (typeof defaultVal === 'object' && typeof defaultVal.text === 'string' && Array.isArray(defaultVal.values)) {
...
export function sequenceToAST(seq) {
    if (!isSequenceConfig(seq))
        return null;
    let ownedBy;
    const rawOwned = seq.options?.ownedBy;
    if (rawOwned) {
        if (typeof rawOwned === 'string') {
            const parts = rawOwned.split('.');
            if (parts.length >= 2) {
                ownedBy = { table: parts[0], column: parts.slice(1).join('.') };
            }
        }
        else if (typeof rawOwned === 'object' && rawOwned.table && rawOwned.column) {
            ownedBy = { table: rawOwned.table, column: rawOwned.column };
        }
    }
    return {
        name: seq.name,
...
export function policyToAST(policy) {
...
export function parsedColumnToColumnInfo(col) {
    return {
        name: col.name,
...
export function parsedIndexToIndexInfo(idx) {
    return {
        name: idx.name,
        columns: idx.columns,
...
export function parsedConstraintToConstraintInfo(con) {
    let definition = '';
...
export function parsedTableToTableInfo(table) {
    return {
        name: table.name,
        schema: table.schema || 'public',
