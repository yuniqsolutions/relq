...
export function validateSqlForDsql(sql, location) {
    const errors = [];
...
export function formatDsqlErrors(result) {
    const lines = [];
...
export function hasDsqlIncompatibilities(sql) {
    for (const config of Object.values(UNSUPPORTED_DATA_TYPES)) {
        if (config.pattern.test(sql))
            return true;
    }
    for (const config of Object.values(UNSUPPORTED_SQL)) {
        if (config.pattern.test(sql))
            return true;
    }
    return false;
