import { UNSUPPORTED_DATA_TYPES, UNSUPPORTED_SQL_FEATURES, PLPGSQL_LIMITATIONS } from "./features.js";
export function validateSqlForCockroachDB(sql) {
    const issues = [];
    for (const typeCheck of UNSUPPORTED_DATA_TYPES) {
    ...
    for (const feature of UNSUPPORTED_SQL_FEATURES) {
    ...
    if (/\bCREATE\s+(OR\s+REPLACE\s+)?FUNCTION\b/i.test(sql)) {
        for (const limitation of PLPGSQL_LIMITATIONS) {
...
export function validateSchemaForCockroachDB(schema) {
    const issues = [];
...
export function validateTableForCockroachDB(table) {
    const issues = [];
