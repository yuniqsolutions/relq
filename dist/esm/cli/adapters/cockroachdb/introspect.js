...
export async function introspectCockroachDB(connection, options) {
    ...
    try {
        ...
        const database = dbResult.rows[0]?.current_database || connection.database || 'defaultdb';
        const versionResult = await pool.query('SELECT version()');
        const versionStr = versionResult.rows[0]?.version || '';
        const versionMatch = versionStr.match(/CockroachDB\s+\w+\s+(v[\d.]+)/i);
...
export async function introspectTable(connection, tableName, schema = 'public') {
    const { Pool } = await import('pg');
    const { buildPoolConfig } = await import("../../../config/config.js");
    const poolConfig = await buildPoolConfig({
        url: connection.url,
...
export async function listTables(connection, schema = 'public') {
    const { Pool } = await import('pg');
    const { buildPoolConfig } = await import("../../../config/config.js");
    const poolConfig = await buildPoolConfig({
        url: connection.url,
...
async function introspectTables(pool, options) {
    const schemaFilter = options?.schemas?.length
        ? `AND n.nspname = ANY($1)`
        : `AND n.nspname NOT LIKE 'pg_%' AND n.nspname != 'information_schema' AND n.nspname != 'crdb_internal'`;
    ...
    const columnsResult = await pool.query(`
        SELECT
            ...
            CASE
                ...
                THEN true
                WHEN t.typname IN ('int2', 'int4', 'int8') AND
                     pg_catalog.pg_get_expr(d.adbin, d.adrelid) LIKE 'unique_rowid%'
                THEN true
                ELSE false
