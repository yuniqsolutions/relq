import { XATA_FEATURES, XATA_LIMITS, POSTGRES_TO_XATA_TYPE_MAP } from "./features.js";
import { introspectXata } from "./introspect.js";
export class XataAdapter {
    dialect = 'xata';
    family = 'xata';
    displayName = 'Xata';
    features = XATA_FEATURES;
    defaultPort = 443;
    defaultUser = '';
    quoteChar = '"';
    client = null;
    ...
    async introspect(connection, options) {
        ...
        return await introspectXata(fetchSchema, databaseName, options);
    }
    ...
    validate(schema) {
        ...
        if (schema.tables.length > XATA_LIMITS.maxTablesPerDatabase) {
            issues.push({
                severity: 'error',
                category: 'FEATURE',
                feature: 'table_count',
                location: 'database',
                message: `Xata supports maximum ${XATA_LIMITS.maxTablesPerDatabase} tables per database`,
            });
        }
    ...
    validateTableInternal(table) {
        const issues = [];
        if (table.columns.length > XATA_LIMITS.maxColumnsPerTable) {
            issues.push({
                severity: 'error',
                category: 'FEATURE',
                feature: 'column_count',
                location: table.name,
                message: `Xata supports maximum ${XATA_LIMITS.maxColumnsPerTable} columns per table`,
            });
        }
        for (const column of table.columns) {
            const type = column.type.toLowerCase();
            if (!this.isTypeSupported(type)) {
                const xataType = POSTGRES_TO_XATA_TYPE_MAP[type];
                if (xataType) {
                    issues.push({
                        severity: 'warning',
                        category: 'DATA_TYPE',
    ...
    isTypeSupported(sqlType) {
        ...
        const supportedTypes = Object.keys(POSTGRES_TO_XATA_TYPE_MAP);
        const unsupported = [
    ...
    mapTypeToFriendly(internalType) {
        return POSTGRES_TO_XATA_TYPE_MAP[internalType.toLowerCase()] || internalType;
    }
    mapTypeToInternal(friendlyType) {
        const entries = Object.entries(POSTGRES_TO_XATA_TYPE_MAP);
...
export { XATA_FEATURES } from "./features.js";
