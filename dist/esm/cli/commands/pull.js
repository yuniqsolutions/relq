    ...
    try {
        ...
        if (typesResult.generated) {
            ...
            if (typesResult.typesCount > 0) {
                const parts = [];
                if (typesResult.added.length > 0)
                    parts.push(colors.green(`${typesResult.added.length} added`));
                if (typesResult.updated.length > 0)
                    parts.push(colors.yellow(`${typesResult.updated.length} updated`));
                if (typesResult.kept.length > 0)
                    parts.push(colors.cyan(`${typesResult.kept.length} local-only`));
                const detail = parts.length > 0 ? ` — ${parts.join(', ')}` : '';
                spin.stop(`Written ${colors.cyan(typesFilePath)} ${colors.muted(`(${typesResult.typesCount} types, ${formatBytes(typesFileSize)})`)}${detail}`);
            }
            else {
