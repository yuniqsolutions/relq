...
import { validateSchemaForDsql, formatDsqlErrors } from "../utils/dsql-validator.js";
...
export async function addCommand(context) {
    ...
    if (fileChange) {
        ...
        if (config?.connection && isAwsDsqlConfig(config.connection) && currentSchema) {
            const dsqlValidation = validateSchemaForDsql({
            ...
            if (!dsqlValidation.valid) {
                console.log('');
                console.log(formatDsqlErrors(dsqlValidation));
                fatal('Schema contains DSQL incompatibilities', 'Fix the issues above before staging changes.');
            }
            if (dsqlValidation.warnings.length > 0) {
                console.log(formatDsqlErrors({ valid: true, errors: [], warnings: dsqlValidation.warnings }));
            }
        }
        if (currentSchema && snapshot) {
            const snapshotAsDbSchema = snapshotToDatabaseSchema(snapshot);
            const schemaChanges = await compareSchemas(snapshotAsDbSchema, currentSchema);
            ...
            if (tableData?.columns) {
                for (const col of tableData.columns) {
                    if (col.references?.table) {
                        ...
                        console.log(`   ${colors.green(fkDisplay)}`);
                    }
                }
            }
            if (tableData?.constraints) {
                for (const con of tableData.constraints) {
                    if (con.definition && con.definition.startsWith('CHECK')) {
                        console.log(`   ${colors.green(`+ CHECK ${con.name} on ${tableData.name}`)}`);
                    }
                }
            }
        }
