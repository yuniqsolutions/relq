...
import { isAwsDsqlConfig, loadConfig } from "../../config/config.js";
import { validateSqlForDsql, validateSchemaForDsql, formatDsqlErrors } from "../utils/dsql-validator.js";
...
export async function importCommand(sqlFilePath, options = {}, projectRoot = process.cwd()) {
    ...
    if (importConfig?.connection && isAwsDsqlConfig(importConfig.connection)) {
        const sqlErrors = validateSqlForDsql(sqlContent, 'import SQL');
        const schemaValidation = validateSchemaForDsql({
            ...
            triggers: triggers.map(t => ({ name: t.name })),
            sequences: parsedSchema.sequences?.map(s => ({ name: s.name })) || [],
            extensions: parsedSchema.extensions || [],
        });
        const allErrors = [...sqlErrors, ...schemaValidation.errors];
        if (allErrors.length > 0) {
            ...
            fatal('Imported SQL contains DSQL incompatibilities', 'Fix the issues above before importing into an AWS DSQL project.');
        }
        if (schemaValidation.warnings.length > 0) {
            console.log('');
            console.log(formatDsqlErrors({ valid: true, errors: [], warnings: schemaValidation.warnings }));
        }
    }
