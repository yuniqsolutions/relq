...
export class ConnectedSelectBuilder {
    ...
    addTypeSafeJoin(joinType, tableOrAlias, callback) {
        ...
        if (!conditionInternals.hasConditions() && schema && relations) {
            const fkResolution = resolveForeignKey(relations, schema, this.schemaKey || this.tableName, rightTableKey);
            if (fkResolution) {
                ...
                throw new RelqQueryError(`Cannot auto-resolve FK relationship between "${this.schemaKey || this.tableName}" and "${rightTableKey}". ` +
