...
export interface RelqSafetyConfig {
	...
	/**
	 * Suppress warnings about tables defined with `const` instead of `export const`.
	 *
	 * - `false` / `undefined` (default): Show warning each time unexported tables are detected
	 * - `true`: Never show the warning
	 * - `string[]`: Suppress warnings only for specific table names (SQL names, e.g. `'migration_items'`)
	 */
	suppressUnexportedWarning?: boolean | string[];
}
/**
...
export interface RelqConfig<TTables extends Record<string, TableDefinition<Record<string, ColumnConfig>>> = Record<string, TableDefinition<Record<string, ColumnConfig>>>> {
	...
	includeFunctions?: boolean;
	/**
	 * Include Row Level Security (RLS) POLICIEs in schema generation.
	 * @default false
	 */
...
export interface BaseRelqConfig<TTables = Record<string, any>> {
	 ...
	 */
	extensions?: string[];
	/** Safety and warning configuration */
	safety?: RelqSafetyConfig;
}
/**
 ...
 */
type RelqConfig$1<TTables = Record<string, any>> = PostgresRelqConfig<TTables> | CockroachDBRelqConfig<TTables> | NileRelqConfig<TTables> | DsqlRelqConfig<TTables> | SQLiteRelqConfig<TTables> | TursoRelqConfig<TTables> | MySQLRelqConfig<TTables> | MariaDBRelqConfig<TTables> | PlanetScaleRelqConfig<TTables> | XataRelqConfig<TTables>;
/**
 ...
 *
 * Uses a discriminated union on the `dialect` field so TypeScript narrows
 * `connection` to the correct type automatically.  When `dialect` is omitted
 * it defaults to `'postgres'`.
 *
 * Errors point to the actual problem (e.g. "unknown property") instead of
 * falling through to the last overload with a misleading dialect mismatch.
 *
 * @example
 * ```typescript
 * // PostgreSQL (default — dialect is optional)
 ...
 * export default defineConfig({
 *     dialect: 'postgres',
 *     connection: { host: 'localhost', database: 'mydb', ssl: 'require' },
 * });
 *
 * // AWS Aurora DSQL
 * export default defineConfig({
 ...
 *     connection: {
 *         database: 'postgres',
 *         aws: { hostname: 'abc.dsql.us-east-1.on.aws', region: 'us-east-1' },
 *     },
 * });
 *
 * // SQLite
 ...
 * export default defineConfig({
 *     dialect: 'turso',
 *     connection: { url: 'libsql://mydb.turso.io', authToken: process.env.TURSO_AUTH_TOKEN },
 * });
 *
 * // MySQL
 * export default defineConfig({
 ...
 * ```
 */
export declare function defineConfig<T extends RelqConfig$1>(config: T): T;
/**
 * Extract the dialect from a config object
 */
export type ExtractDialect<T extends RelqConfig$1> = T extends {
	dialect: infer D extends DialectName;
} ? D : "postgres";
/**
 * Check if config is for a specific dialect
 */
export declare function isDialect<D extends DialectName>(config: RelqConfig$1, dialect: D): config is RelqConfigForDialect<D>;
/**
 * Get the resolved dialect from config (defaults to 'postgres')
 */
export declare function getDialect(config: RelqConfig$1): DialectName;
/**
 ...
 * ```
 */
export declare function migrateLegacyConfig(config: unknown): RelqConfig$1;
/**
...
export {
	PoolConfig$1 as PoolConfig,
	RelqConfig$1 as RelqConfig,
};
